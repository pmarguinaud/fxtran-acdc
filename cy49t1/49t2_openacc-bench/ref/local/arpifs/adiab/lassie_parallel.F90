...
IF (LPARALLELMETHOD ('OPENMP','LASSIE_PARALLEL:SITNU')) THEN
  ...
  IF (LSYNCHOST ('LASSIE_PARALLEL:SITNU')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENMPSINGLECOLUMN','LASSIE_PARALLEL:SITNU')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LASSIE_PARALLEL:SITNU')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('LASSIE_PARALLEL:SITNU:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         YSTACK, Z_YDVARS_DIV_T0, Z_YDVARS_SP_DL, Z_YDVARS_SP_DM, Z_YDVARS_T_DL, &
    !$ACC&         Z_YDVARS_T_DM) &
    !$ACC&PRIVATE (JBLK) &
...
IF (YDDYNA%LSPRT) THEN
  ...
  IF (YDDYNA%LTWOTL) THEN
    ...
    IF (LPARALLELMETHOD ('OPENMP','LASSIE_PARALLEL:GPRCP')) THEN
      ...
      IF (LSYNCHOST ('LASSIE_PARALLEL:GPRCP')) THEN
    ...
    ELSEIF (LPARALLELMETHOD ('OPENMPSINGLECOLUMN','LASSIE_PARALLEL:GPRCP')) THEN
    ...
    ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LASSIE_PARALLEL:GPRCP')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK ('LASSIE_PARALLEL:GPRCP:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
        ...
        !$ACC PARALLEL LOOP GANG &
        !$ACC&PRESENT (PTOD0, YDCPG_OPTS, YDCST, YDGEOMETRY, YSTACK, ZR9) &
        !$ACC&PRIVATE (JBLK) &
        !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
