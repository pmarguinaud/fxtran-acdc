...
IF (LPARALLELMETHOD ('OPENMP','CTVTOT_EXPL_PARALLEL:0')) THEN
  ...
  IF (LSYNCHOST ('CTVTOT_EXPL_PARALLEL:0')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENMPSINGLECOLUMN','CTVTOT_EXPL_PARALLEL:0')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CTVTOT_EXPL_PARALLEL:0')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CTVTOT_EXPL_PARALLEL:0:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&PRESENT (YDCPG_OPTS, YDCST, YDGEOMETRY, YSTACK, ZR, Z_YDVARS_T_T0) &
    !$ACC&PRIVATE (JBLK) &
...
ENDSUBROUTINE CTVTOT_EXPL_PARALLEL
