      ...
      if ($opts{inlineDeclarations})
        {
          for my $use (@use)
            {
              &Fxtran::Decl::use ($d1, $use->cloneNode (1));
              &removeStmt ($use);
            }
          for my $decl (@decl)
            {
              &Fxtran::Decl::declare ($d1, $decl->cloneNode (1));
              &removeStmt ($decl);
            }
          for my $include (@include)
            {
              &Fxtran::Decl::include ($d1, $include->cloneNode (1));
              &removeStmt ($include);
            }
        }
      else
