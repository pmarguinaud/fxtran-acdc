...
ELSEIF (CDINT/='LAITVSPCQM  ') THEN
  CALL ABOR1_ACC ("unknown type of QM limiter for GFL interpolation: "//CDINT)
ENDIF
...
IF (CDINT=='LAITQM(SLD) '.OR.CDINT=='LAITQMH(SLD)'.OR.CDINT&
  ...
  IF (CDINT=='LAITQM3D(SLD') THEN
    CALL ABOR1_ACC ('LAITRE_GFL: LAITQM3D(SLD) IS NOT YET AVAILABLE')
  ENDIF
...
ELSEIF (CDINT=='LAITVSPCQM  ') THEN
  ...
  IF (.NOT.PRESENT (PXSPSL))  THEN
      CALL ABOR1_ACC ('LAITRE_GFL: MISSING ARGUMENT PXSPSL FOR INTERPOLATION TYPE LAITVSPCQM')
  ENDIF
...
ELSE
  CALL ABOR1_ACC ('LAITRE_GFL: UNKNOWN INTERPOLATION TYPE: '//CDINT)
ENDIF




ENDSUBROUTINE LAITRE_GFL_OPENACC
