...
REAL (KIND=JPRB)::ZNH1M (KPROMA, KFLEV)
REAL (KIND=JPRB)::ZNH1L (KPROMA, KFLEV)
REAL (KIND=JPRB)::ZINM (KPROMA, 0:KFLEV+1)
REAL (KIND=JPRB)::ZINL (KPROMA, 0:KFLEV+1)
...
IF (LLNHEE) THEN
  ...
  IF (.NOT.(PRESENT (PRNHPPI).AND.PRESENT (PQCHAL).AND.PRESENT (PQCHAM)))  THEN
      CALL ABOR1_ACC (' GPGRGEO_EXPL: missing input PRNHPPI, PQCHAL, PQCHAM !!!')
  ENDIF
...
ENDSUBROUTINE GPGRGEO_EXPL_OPENACC
