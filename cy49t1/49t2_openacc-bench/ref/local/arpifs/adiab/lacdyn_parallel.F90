...
IF (LPARALLELMETHOD ('OPENMP','LACDYN_PARALLEL:LASURE')) THEN
  ...
  IF (LSYNCHOST ('LACDYN_PARALLEL:LASURE')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENMPSINGLECOLUMN','LACDYN_PARALLEL:LASURE')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LACDYN_PARALLEL:LASURE')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('LACDYN_PARALLEL:LASURE:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&PRESENT (YDCPG_OPTS, YDGEOMETRY, YDMODEL, YSTACK, ZBDT, ZREDIV, Z_YDVARS_GEOMETRY_GMAPPA_T0, &
...
ELSE
  ...
  IF (LPARALLELMETHOD ('OPENMP','LACDYN_PARALLEL:ZMIXNL')) THEN
    ...
    IF (LSYNCHOST ('LACDYN_PARALLEL:ZMIXNL')) THEN
  ...
  ELSEIF (LPARALLELMETHOD ('OPENMPSINGLECOLUMN','LACDYN_PARALLEL:ZMIXNL')) THEN
  ...
  ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LACDYN_PARALLEL:ZMIXNL')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LACDYN_PARALLEL:ZMIXNL:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      ...
      !$ACC PARALLEL LOOP GANG &
      !$ACC&PRESENT (YDCPG_OPTS, YSTACK, ZMIXNL) &
      !$ACC&PRIVATE (JBLK) &
      !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
CALL LATTEX_PARALLEL (YDMODEL%YRCST, YDGEOMETRY, YDVARS, YDCPG_TND, YDCPG_SL1, YDCPG_SL2&
&, YDCPG_BNDS, YDCPG_OPTS, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH, YDMODEL%YRML_GCONF&
&, YDMODEL%YRML_DYN, ZDTS2, ZBT, YL_ZBDT, YDMODEL%YRML_DYN%YRDYN%RESGP, YDMODEL%YRML_DYN&
&%YRDYN%RESGM, YDCPG_DYN0%F_OROGL, YDCPG_DYN0%F_OROGM, YDCPG_DYN0%F_PHIF, YL_ZGAGT0L, YL_ZGAGT0M&
&, YL_ZTOD0, ZSPDS0, ZLPD0, YL_ZGAGT9L, YL_ZGAGT9M, YL_ZTOD9, ZSPDS9, ZLPD9, YDCPG_DYN0%XYB&
&%F_RDELP, YDCPG_DYN0%CTY%F_EVEL, YDCPG_DYN0%F_NHX, YDCPG_DYN0%F_GWT, YDCPG_DYN0%F_GWFT,&
& YDCPG_DYN9%F_NHX, YDCPG_DYN9%F_GWT, YL_ZMIXNL, YDCPG_TNDSI_DDH)
