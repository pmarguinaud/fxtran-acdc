...
IF (LDFINDVSEP) THEN
  ...
  DO JROF=KST, KEND
    ...
    DO WHILE (.NOT.LLDONE)
      IVSEP=IVSEP+1
      ISEP=KLEV (JLON, IVSEP+1)-KLEV (JLON, 1)

      DO JLEV=2, YDGEOMETRY%YRDIMV%NFLEVG-IVSEP
        ISEP=MIN (ISEP,(KLEV (JLON, JLEV+IVSEP)-KLEV (JLON, JLEV)))
      ENDDO
    ...
    IF (IMINSEP<4) THEN
      CALL ABOR1_ACC ('LARCINA: UNABLE TO COMPUTE NVSEPC')
    ENDIF

    KVSEPC (JLON)=MAX (KVSEPC (JLON), IVSEP)
  ENDDO
  ...
  DO JROF=KST, KEND
    ...
    IF (IMINSEP<2) THEN
      CALL ABOR1_ACC ('LARCINA: UNABLE TO COMPUTE NVSEPL')
    ENDIF

    KVSEPL (JLON)=MAX (KVSEPL (JLON), IVSEP)
  ENDDO
