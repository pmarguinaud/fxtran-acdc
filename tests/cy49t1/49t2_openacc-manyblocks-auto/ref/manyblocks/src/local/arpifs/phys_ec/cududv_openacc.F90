...
REAL (KIND=JPRB), INTENT (IN)::PUU 
REAL (KIND=JPRB), INTENT (IN)::PUD 
REAL (KIND=JPRB), INTENT (IN)::PVU 
REAL (KIND=JPRB), INTENT (IN)::PVD 
...
fxtran_acdc_temp (REAL(KIND=JPRB), PTENU_PTR, (KLON, KLEV))
fxtran_acdc_temp (REAL(KIND=JPRB), PTENV_PTR, (KLON, KLEV))
...
fxtran_acdc_assoc (PTENU_PTR, PTENU)

fxtran_acdc_assoc (PTENV_PTR, PTENV)
...
DO JK=KTOPM2, KLEV
  ...
  IF (LDCUM_PTR (JL)) THEN
    ZMFUU (JL, JK)=PMFU_PTR (JL, JK)*(PUU_PTR (JL, JK)-ZIMP*PUEN_PTR (JL, IK))
    ZMFUV (JL, JK)=PMFU_PTR (JL, JK)*(PVU_PTR (JL, JK)-ZIMP*PVEN_PTR (JL, IK))
    ZMFDU (JL, JK)=PMFD_PTR (JL, JK)*(PUD_PTR (JL, JK)-ZIMP*PUEN_PTR (JL, IK))
    ZMFDV (JL, JK)=PMFD_PTR (JL, JK)*(PVD_PTR (JL, JK)-ZIMP*PVEN_PTR (JL, IK))
  ENDIF
...
IF (YDECUMF%RMFSOLUV==0.0_JPRB) THEN
  ...
  DO JK=KTOPM2, KLEV
    ...
    IF (LDCUM_PTR (JL)) THEN
      PTENU_PTR (JL, JK)=PTENU_PTR (JL, JK)+ZDUDT (JL, JK)
      PTENV_PTR (JL, JK)=PTENV_PTR (JL, JK)+ZDVDT (JL, JK)
    ENDIF
...
ELSE
  ...
  DO JK=KTOPM2, KLEV
    ...
    IF (LLCUMBAS (JL, JK)) THEN
      ...
      ZMFUU (JL, JK)=-ZZP*(PMFU_PTR (JL, JK)+PMFD_PTR (JL, JK))
      ...
      ZU=ZZP*(PUEN_PTR (JL, IM)-PUEN_PTR (JL, JK))
      ZV=ZZP*(PVEN_PTR (JL, IM)-PVEN_PTR (JL, JK))
      ZDUDT (JL, JK)=(ZDUDT (JL, JK)+PTENU_PTR (JL, JK)*YDECUMF%RMFSOLRHS)*PTSPHY+PUEN_PTR (JL, JK)-ZU
      ZDVDT (JL, JK)=(ZDVDT (JL, JK)+PTENV_PTR (JL, JK)*YDECUMF%RMFSOLRHS)*PTSPHY+PVEN_PTR (JL, JK)-ZV
    ENDIF
  ...
  IF (LLPERT_CUDUDV.OR.LLPERT_CUDUDVS.OR.LLPPAR_CUDUV) THEN
    ...
    IF ((KTYPE_PTR (JL)==1.AND.(LLPERT_CUDUDV.OR.LLPPAR_CUDUV)).OR.(KTYPE_PTR&
      & (JL)==2.AND.(LLPERT_CUDUDVS.OR.LLPPAR_CUDUV))) THEN

      IF (KTYPE_PTR (JL)==1) THEN

        IF (.NOT.LLPPAR_CUDUV) THEN
          ZXU=PGP2DSPP_PTR (JL, IPCUDU)
          ZXV=PGP2DSPP_PTR (JL, IPCUDV)
          ZXU_MAG=PN1%XMAG (1)
          ZXV_MAG=PN1%XMAG (2)
        ELSE
      ...
      IF (KTYPE_PTR (JL)==2) THEN
        ...
        IF (.NOT.LLPPAR_CUDUV) THEN
          ZXU=PGP2DSPP_PTR (JL, IPCUDUS)
          ZXV=PGP2DSPP_PTR (JL, IPCUDVS)
          ZXU_MAG=PN2%XMAG (1)
          ZXV_MAG=PN2%XMAG (2)
        ELSE
          ZXU=YDPERTPAR%CUDU_MOD
  ...
  DO JK=KTOPM2, KLEV
    ...
    IF (LLCUMBAS (JL, JK)) THEN
      ...
      IF ((LLPERT_CUDUDV).OR.(LLPPAR_CUDUV)) THEN
        PTENU_PTR (JL, JK)=PTENU_PTR (JL, JK)*(1.0_JPRB-YDECUMF%RMFSOLRHS&
        &)+ZRDU *(ZR1 (JL, JK)-PUEN_PTR (JL, JK))*ZTSPHY
        PTENV_PTR (JL, JK)=PTENV_PTR (JL, JK)*(1.0_JPRB-YDECUMF%RMFSOLRHS&
        &)+ZRDV *(ZR2 (JL, JK)-PVEN_PTR (JL, JK))*ZTSPHY
      ELSE
        PTENU_PTR (JL, JK)=PTENU_PTR (JL, JK)*(1.0_JPRB-YDECUMF&
        &%RMFSOLRHS)+(ZR1 (JL, JK)-PUEN_PTR (JL, JK))*ZTSPHY
        PTENV_PTR (JL, JK)=PTENV_PTR (JL, JK)*(1.0_JPRB-YDECUMF&
        &%RMFSOLRHS)+(ZR2 (JL, JK)-PVEN_PTR (JL, JK))*ZTSPHY
      ENDIF
