...
<end-subroutine-stmt>END SUBROUTINE</end-subroutine-stmt></program-unit>");
  ...
  &Fxtran::Decl::use ($pu, 'USE FIELD_MODULE, ONLY : FIELD_BASIC');
  ...
  &Fxtran::Scope::removeWhiteLines ($s);
...
sub simplifyReadWriteAssignemnts
{
  ...
  &Fxtran::Construct::removeEmptyConstructs ($d);
...
sub makeSync
{
  ...
  &Fxtran::Decl::use ($d, 'USE FIELD_MODULE, ONLY : FIELD_BASIC');

  &Fxtran::Scope::removeWhiteLines ($d);
