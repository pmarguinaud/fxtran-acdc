...
ELSE
  ...
  DO JB=1,N_SPBAND
    ...
    CALL FIT1(ZDE1(:,:,JB),FCM_P_AI(JB,:),FCM_Q_AI(JB,:),ZEOAI(:,:,JB))
    CALL FIT1(ZRE1(:,:,JB),FCM_P_AL(JB,:),FCM_Q_AL(JB,:),ZEOAL(:,:,JB))
    CALL FIT1(ZDE1(:,:,JB),FCM_P_DI(JB,:),FCM_Q_DI(JB,:),ZEODI(:,:,JB))
    CALL FIT1(ZRE1(:,:,JB),FCM_P_DL(JB,:),FCM_Q_DL(JB,:),ZEODL(:,:,JB))
    CALL FIT1(ZDE1(:,:,JB),FCM_P_GI(JB,:),FCM_Q_GI(JB,:),ZGI  (:,:,JB))
    CALL FIT1(ZRE1(:,:,JB),FCM_P_GL(JB,:),FCM_Q_GL(JB,:),ZGL  (:,:,JB))
...
DO JLEV=KTDIA,KLEV
	DO JLON=KIDIA,KFDIA
		!IF ( LLMASKS(JLON) ) THEN
      ZSIZE_=PSIZE(JLON,JLEV)
      ZP_=PP(0)   +ZSIZE_*(PP(1)+ZSIZE_*(PP(2)+ZSIZE_*PP(3)))
      ZQ_=1.0_JPRB+ZSIZE_*(PQ(1)+ZSIZE_*(PQ(2)+ZSIZE_*PQ(3)))
      POUT(JLON,JLEV)=ZP_/ZQ_
    !ENDIF
  ENDDO
ENDDO
