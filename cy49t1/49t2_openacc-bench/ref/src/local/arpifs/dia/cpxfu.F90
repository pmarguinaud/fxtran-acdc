...
!$ACDC pointerparallel --gpumemstat --stack84 --inline-contained --use-acpy --post-parallel synchost,nullify
...
!   R. El Khatib 08-Aug-2023 Bugfix : PVMIN/PVMAX are not scalars but arrays, to be scanned up to KFDIA only
!      R. Honnert   Aug-2023 Added thermal vertical velocity
!     ------------------------------------------------------------------
...
IF (LXMWINDCLS .AND. NMEANSTEPS/=0) THEN
  ...
  IF (LXCLS) THEN
    CALL MEANWIND_XFU (YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NMEANSTEPS, YDXFU%UCLS, &
                     & YDXFU%VCLS, PXUCLS, PXVCLS, YDXFU%RMWINDCALC, ZUMEAN, ZVMEAN)
  ENDIF                                                   
                                                          
  IF (LXNUVCLS) THEN                                      
    CALL MEANWIND_XFU (YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, NMEANSTEPS, YDXFU%NUCLS, &
                     & YDXFU%NVCLS, PXNUCLS, PXNVCLS, YDXFU%RMNWINDCALC, ZNUMEAN, ZNVMEAN)
  ENDIF
...
IF (YDXFU%YCLPHMF%LACTIVE) CALL COPY1_XFU(YDXFU%CLPHMF,YDMF_PHYS_OUT%CLPHMF)
IF (YDXFU%YVMAXTH%LACTIVE) CALL COPY1_XFU(YDXFU%VMAXTH,YDMF_PHYS_OUT%VMAXTH)
...
IF (YDXFU%YSNWDE%LACTIVE) CALL COPY1_XFU (YDXFU%SNWDE, YDMF_PHYS_OUT%SNWDE )
...
ELSEIF (PRESENT (PSMIN) .AND. PRESENT (PVMAX)) THEN
  PXFU (YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA) = MIN (PSMIN, MAX (PVMAX(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA), PXFU (YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)))
ELSEIF (PRESENT (PVMIN) .AND. PRESENT (PSMAX)) THEN
  PXFU (YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA) = MIN (PVMIN(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA), MAX (PSMAX, PXFU (YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA)))
ELSEIF (PRESENT (PVMIN)) THEN
  PXFU (YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA) = MIN (PVMIN(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA), PXFU (YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA))
ELSEIF (PRESENT (PVMAX)) THEN
  PXFU (YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA) = MAX (PVMAX(YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA), PXFU (YDCPG_BNDS%KIDIA:YDCPG_BNDS%KFDIA))
