...
IF (LDTF1) THEN
  ...
  IF (YDDYNA%LTWOTL) THEN
    ...
    ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','GPTF1_YDVARS_PARALLEL:LTWOTL')) THEN
      ...
      !$OMP & MAP(PRESENT:YDCPG_OPTS, YFXTRAN_ACDC_STACK, Z_YDVARS_U_T0, Z_YDVARS_U_T9,  & 
      !$OMP &          Z_YDVARS_V_T0, Z_YDVARS_V_T9)  & 
      !$OMP & PRIVATE (JBLK)  & 
    ...
    IF (YDDYNA%LGRADSP) THEN
      ...
      IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','GPTF1_YDVARS_PARALLEL:LGRADSP')) THEN
        ...
        CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_SGRTL_T0, YDVARS%SGRTL%FT0)
        CALL SGET_HOST_DATA_RDWR (Z_YDVARS_SGRTL_T9, YDVARS%SGRTL%FT9)
        CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_SGRTM_T0, YDVARS%SGRTM%FT0)
      ...
      ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','GPTF1_YDVARS_PARALLEL:LGRADSP')) THEN
        ...
        IF (LHOOK) CALL DR_HOOK ('GPTF1_YDVARS_PARALLEL:LGRADSP:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
        CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_SGRTL_T0, YDVARS%SGRTL%FT0)
        CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_SGRTL_T9, YDVARS%SGRTL%FT9)
        CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_SGRTM_T0, YDVARS%SGRTM%FT0)
        CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_SGRTM_T9, YDVARS%SGRTM%FT9)
        IF (LHOOK) CALL DR_HOOK ('GPTF1_YDVARS_PARALLEL:LGRADSP:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
        IF (LHOOK) CALL DR_HOOK ('GPTF1_YDVARS_PARALLEL:LGRADSP:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
        
        !$OMP TARGET TEAMS DISTRIBUTE  & 
        !$OMP & MAP(PRESENT:YDCPG_OPTS, YFXTRAN_ACDC_STACK, Z_YDVARS_SGRTL_T0, Z_YDVARS_SGRTL_T9,  & 
        !$OMP &          Z_YDVARS_SGRTM_T0, Z_YDVARS_SGRTM_T9)  & 
        !$OMP & PRIVATE (JBLK)  & 
        !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
