...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LRAY.AND.YDMODEL%YRML_PHY_MF%YRPHY%NRAY==2) THEN
  CALL APL_ARPEGE_RADIATION_ACRANEB2 (YDVARS, YDCPG_OPTS, YDCFU, YDCPG_MISC, YDMF_PHYS&
  &, YDMF_PHYS_BASE_STATE, YDMF_PHYS_SURF, YDMODEL, YDCPG_BNDS, PCLCT_RAD, PQV, ZFRSODS&
  &, PAER, PQO3, PRDG_MU0LU, PALBDIR, PDECRD, PQCO2, PRDG_MU0, PFLU_EMIS)
ELSEIF (YDMODEL%YRML_PHY_MF%YRPHY%LRAYFM) THEN
  ...
  ELSE
    CALL APL_ARPEGE_RADIATION_CALLRAD_F (YDCPG_GPAR, YDCPG_OPTS&
    &, YDMODEL, YDCPG_BNDS, PTRSODIR, PTRSODIF)
  ENDIF

  CALL APL_ARPEGE_RADIATION_RADHEAT (YDVARS, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_BASE_STATE&
  &, YDMODEL, YDCPG_BNDS, PTRSODIR, PTRSODIF, PTENT, PQV, PTRSOD, ZFRSODS, PCTRSO, PSUDU&
  &, PALBD, PALBP, PSFSWDIF, PRDG_SOLO, PRDG_MU0, PSFSWDIR, PFLU_EMIS, PCEMTR)

  IF (YDMODEL%YRML_PHY_MF%YRPHY%LDAYD) THEN
    CALL APL_ARPEGE_RADIATION_ACDAYD (YDVARS, YDMF_PHYS_BASE_STATE&
    &, YDCPG_OPTS, YDMF_PHYS, YDMODEL, YDCPG_BNDS, PRDG_MU0)
  ENDIF


  IF (YDMODEL%YRML_PHY_MF%YRPHY0%GRSO<1._JPRB) THEN
    CALL APL_ARPEGE_RADIATION_ACRSO (YDVARS, YDMF_PHYS_BASE_STATE&
    &, YDCPG_OPTS, YDMF_PHYS, YDMODEL, YDCPG_BNDS, PRDG_MU0)
  ENDIF


  IF (.NOT.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
    CALL APL_ARPEGE_RADIATION_LMSE_F (YDCPG_OPTS, YDMF_PHYS, YDMODEL, YDCPG_BNDS, PALBP, PALBD)
  ENDIF


  IF (.NOT.YDCPG_OPTS%LCONFX) THEN
    CALL APL_ARPEGE_RADIATION_SUNSHINE (YDMF_PHYS_SURF, YDMF_PHYS, YDMODEL, YDCPG_BNDS)
  ENDIF
...
CALL APL_ARPEGE_RADIATION_DIRECT (YDCPG_OPTS, YDMF_PHYS, YDCPG_BNDS, PRDG_MU0)
