...
IF (LPARALLELMETHOD ('OPENMP','MF_PHYS_SAVE_PHSURF_PART1_PARALLEL:PART1')) THEN
  ...
  IF (LSYNCHOST ('MF_PHYS_SAVE_PHSURF_PART1_PARALLEL:PART1')) THEN
    IF (LHOOK) CALL DR_HOOK ('MF_PHYS_SAVE_PHSURF_PART1_PARALLEL&
...
ELSEIF (LPARALLELMETHOD ('OPENMPSINGLECOLUMN','MF_PHYS_SAVE_PHSURF_PART1_PARALLEL:PART1')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','MF_PHYS_SAVE_PHSURF_PART1_PARALLEL:PART1')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('MF_PHYS_SAVE_PHSURF_PART1_PARALLEL:PART1:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      YLSTACK%U4 = stack_u4 (YSTACK, JBLK-YDCPG_OPTS%JBLKMIN+1, YDCPG_OPTS%KGPBLKS)
        ...
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPRO.OR.YDMODEL%YRML_PHY_MF%YRPHY%LGPCMT) THEN
          CALL ACPY (JLON, PSAV_UDAL (:,:, JBLK), PUAL_T0 (:,:, JBLK))
          CALL ACPY (JLON, PSAV_UDOM (:,:, JBLK), PUOM_T0 (:,:, JBLK))

          IF (YDMODEL%YRML_PHY_MF%YRPHY%LCDDPRO) THEN
            CALL ACPY (JLON, PSAV_DDAL (:,:, JBLK), PDAL_T0 (:,:, JBLK))
            CALL ACPY (JLON, PSAV_DDOM (:,:, JBLK), PDOM_T0 (:,:, JBLK))
          ENDIF
        ...
        IF (YDMODEL%YRML_GCONF%YGFL%YUNEBH%LACTIVE)  THEN
            CALL ACPY (JLON, PSAV_UNEBH (:,:, JBLK), PUNEBH_T0 (:,:, JBLK))
        ENDIF
  

        IF (YDMODEL%YRML_GCONF%YGFL%YUEN%LACTIVE)  THEN
            CALL ACPY (JLON, PSAV_ENTCH (:,:, JBLK), PUEN_T0 (:,:, JBLK))
        ENDIF
...
ENDSUBROUTINE MF_PHYS_SAVE_PHSURF_PART1_PARALLEL
