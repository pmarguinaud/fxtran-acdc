...
ELSE
  CALL LARCINB_LRHS_CURV_F (LLMOM, LLHOI2, LDSLPHY, LL_LIN, LD2TLFF1, LLNOSLHD, YDCPG_SL1,&
  & IWKM, IWK, KASLB1, YDCPG_GMVF_SI, YDGEOMETRY, YDCPG_GMVF, YDCPG_DDH_TND, YDCPG_GMVF_NL&
  &, YDLDDH, YDML_DYN, YDCPG_BNDS, PUPF, PLSCAW, PVPF, PUFZ, KNOWENO, PRSCAW, KL0, PVFZ)
ENDIF

LLMOM=.NOT.YDML_DYN%YRDYN%LSLHDHEAT
CALL LARCINB_LAITRE_GMV (LLMOM, LLHOI2, LL_LIN, LLNOSLHD, YDCPG_DDH_TND, YDCPG_SL1, YDLDDH, IWKH&
&, IWK, KASLB1, YDML_DYN, YDGEOMETRY, YDCPG_GMVF, YDCPG_BNDS, PLSCAW, PRSCAW, KL0, KNOWENO)

IF (YDML_DYN%YRDYN%NTLAG>=3) THEN
  CALL LARCINB_NTLAG_3 (YDCPG_DDH_TND, YDCPG_SL1, YDCPG_GMVF_NL, YDLDDH, KASLB1,&
  & YDML_DYN, YDCPG_GMVF_SI, YDGEOMETRY, YDCPG_BNDS, YDCPG_GMVF, PLSCAW, KL0)
ENDIF


IF (LDSLPHY) THEN
  CALL LARCINB_LDSLPHY (YDCPG_SL1, KASLB1, YDML_DYN, YDGEOMETRY, YDCPG_BNDS, PLSCAW, PTPF, KL0)
ENDIF
...
DO JFLD=1, SIZE (YDVARS%GFL_PTR)
  ...
  IF (YDVARS%GFL_PTR (JFLD)%YCOMP%LADV.AND.(.NOT.YDVARS%GFL_PTR (JFLD)%YCOMP%LMGRID)) THEN
    ...
    IF (YDVARS%GFL_PTR (JFLD)%YCOMP%CSLINT (1:3)=="LIN") THEN
      CALL LARCINB_LIN (LLCOMAD, YDCPG_SL1, JFLD, YDCPG_GFLF, KASLB1,&
    ...
    ELSEIF (YDML_GCONF%YGFL%NUMFLDS_SL1>0.AND.YDML_GCONF%YGFL%NUMFLDS_SPL&
      ...
      CALL LARCINB_LAITVSPCQM (LLMOM, YDVARS, YDCPG_SL1, JFLD, KHVI, YDCPG_GFLF, KASLB1, YDML_DYN&
      &, YDML_GCONF, IWKGFL, YDGEOMETRY, YDCPG_BNDS, PLSCAW, PRSCAW, KL0, KNOWENO)
    ELSE
      CALL LARCINB_LAITRE_GFL (LLMOM, YDVARS, YDCPG_SL1, JFLD, KHVI, YDCPG_GFLF, KASLB1, YDML_DYN&
      &, YDML_GCONF, IWKGFL, YDGEOMETRY, YDCPG_BNDS, PLSCAW, PRSCAW, KL0, KNOWENO)
    ENDIF

    CALL LARCINB_RRINTOPT (YDCPG_DDH_TND, JFLD, YDLDDH, YDCPG_GFLF, YDML_DYN, YDGEOMETRY, YDCPG_BNDS)
    ...
    IF (.NOT.(YDML_DYN%YRDYN%NSPLTHOI==0.OR.YDVARS%GFL_PTR (JFLD)%YCOMP%LTDIABLIN)) THEN
      ...
      CALL LARCINB_LTDIABLIN (LLMOM, CLINT, YDVARS, YDCPG_SL1, JFLD, KHVI, YDCPG_GFLF, IWK, KASLB1&
      &, YDML_DYN, YDML_GCONF, YDGEOMETRY, YDCPG_BNDS, PLSCAW, PRSCAW, KL0, KNOWENO)
    ENDIF
    ...
    IF (YDVARS%GFL_PTR (JFLD)%YCOMP%LTDIABLIN) THEN
      ...
      CALL LARCINB_LLCOMAD (LLCOMAD, YDCPG_SL1, JFLD, YDCPG_GFLF,&
  ...
  IF (YDVARS%GFL_PTR (JFLD)%YCOMP%LADV) THEN
    ...
    IF (YDVARS%GFL_PTR (JFLD)%YCOMP%LPHY.AND.LDSLPHY) THEN
      ...
      CALL LARCINB_LLCOMAD_2 (LLCOMAD, YDCPG_GFLPF, YDCPG_SL1, JFLD&
      &, KASLB1, YDML_DYN, YDGEOMETRY, YDCPG_BNDS, PLSCAW, KL0)
    ENDIF
...
IF (YDML_DYN%YRDYN%NVLAG==2) THEN
  CALL LARCINB_NVLAG_2 (LLMOM, LL_LIN, LLNOSLHD, YDCPG_SL1, KASLB1, YDML_DYN&
  &, YDGEOMETRY, YDCPG_BNDS, IWNT, PLSCAW, PRSCAW, KL0, PCF, KNOWENO)
ENDIF


IF (YDML_DYN%YRDYN%NVLAG==3) THEN
  CALL LARCINB_NVLAG_3 (YDCPG_SL1, KASLB1, YDML_DYN, YDGEOMETRY&
  &, YDCPG_BNDS, PLSCAW, PCF_NL, KL0, PCF_SI, PCF)
ENDIF


IF (YDML_DYN%YRDYNA%NQMGMV==-4) THEN
  CALL LARCINB_NQMGMV (YDCPG_SL1, KASLB1, YDML_DYN, YDGEOMETRY, YDCPG_BNDS, PLSCAW, PDP, KLH0)
ELSE
  ...
  CALL LARCINB_LCOMAD_SP (YDCPG_SL1, IQM, KASLB1, YDML_DYN, YDGEOMETRY&
  &, YDCPG_BNDS, IWNT, PLSCAW, PRSCAW, PDP, KLH0)
ENDIF
