...
DO JFLD=1, SIZE (YDCPG_GMV9%GMV)
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:ZERO_GMV9')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_GMV_P, YDCPG_GMV9%GMV (JFLD)%F_P)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:ZERO_GMV9')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDCPG_GMV9_GMV_P, YDCPG_GMV9%GMV (JFLD)%F_P)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YFXTRAN_ACDC_STACK, Z_YDCPG_GMV9_GMV_P)  & 
    !$OMP & PRIVATE (JBLK)  & 
...
IF (YDMODEL%YRML_DYN%YRDYNA%LSLINL) THEN
  ...
  DO JFLD=1, SIZE (YDCPG_GMV9_SI%GMV)
    ...
    IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:ZERO_GMV9_SI')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_SI_GMV_P, YDCPG_GMV9_SI%GMV (JFLD)%F_P)
    ...
    ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:ZERO_GMV9_SI')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9_SI:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDCPG_GMV9_SI_GMV_P, YDCPG_GMV9_SI%GMV (JFLD)%F_P)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9_SI:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9_SI:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      
      !$OMP TARGET TEAMS DISTRIBUTE  & 
      !$OMP & MAP(PRESENT:YDCPG_OPTS, YFXTRAN_ACDC_STACK, Z_YDCPG_GMV9_SI_GMV_P)  & 
      !$OMP & PRIVATE (JBLK)  & 
      !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (YDMODEL%YRML_DYN%YRDYNA%LPC_CHEAP.AND.YDMODEL%YRML_DYN%YRDYNA%LSETTLS) THEN
  ...
  DO JFLD=1, SIZE (YDCPG_GMV9_NL%GMV)
    ...
    IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:ZERO_GMV9_SL')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_NL_GMV_P, YDCPG_GMV9_NL%GMV (JFLD)%F_P)
    ...
    ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:ZERO_GMV9_SL')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9_SL:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDCPG_GMV9_NL_GMV_P, YDCPG_GMV9_NL%GMV (JFLD)%F_P)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9_SL:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_GMV9_SL:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      
      !$OMP TARGET TEAMS DISTRIBUTE  & 
      !$OMP & MAP(PRESENT:YDCPG_OPTS, YFXTRAN_ACDC_STACK, Z_YDCPG_GMV9_NL_GMV_P)  & 
      !$OMP & PRIVATE (JBLK)  & 
      !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (LDSLPHY) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:ZERO_UV9')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (ZUP9, YL_ZUP9)
    CALL SGET_HOST_DATA_RDWR (ZVP9, YL_ZVP9)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:ZERO_UV9')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_UV9:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDWR (ZUP9, YL_ZUP9)
    CALL SGET_DEVICE_DATA_RDWR (ZVP9, YL_ZVP9)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_UV9:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZERO_UV9:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YFXTRAN_ACDC_STACK, ZUP9, ZVP9)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:ROTUVGEO')) THEN
  ...
  CALL SGET_HOST_DATA_RDWR (ZP, YL_ZP)
  CALL SGET_HOST_DATA_RDWR (ZQ, YL_ZQ)
  CALL SGET_HOST_DATA_RDONLY (Z_YDA_PCCO_P, YDA_PCCO%F_P)
  CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_GNORDL_P, YDVARS%GEOMETRY%GNORDL%FT0)
  CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_GNORDM_P, YDVARS%GEOMETRY%GNORDM%FT0)
  ...
  IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:ROTUVGEO')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (ZP, YL_ZP)
    CALL SGET_HOST_DATA_RDWR (ZQ, YL_ZQ)
    CALL SGET_HOST_DATA_RDWR (Z_YDA_PCCO_P, YDA_PCCO%F_P)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GEOMETRY_GNORDL_P, YDVARS%GEOMETRY%GNORDL%FT0)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GEOMETRY_GNORDM_P, YDVARS%GEOMETRY%GNORDM%FT0)
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:ROTUVGEO')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ROTUVGEO:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
  CALL SGET_DEVICE_DATA_RDWR (ZP, YL_ZP)
  CALL SGET_DEVICE_DATA_RDWR (ZQ, YL_ZQ)
  CALL SGET_DEVICE_DATA_RDONLY (Z_YDA_PCCO_P, YDA_PCCO%F_P)
  CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_GNORDL_P, YDVARS%GEOMETRY%GNORDL%FT0)
  CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_GNORDM_P, YDVARS%GEOMETRY%GNORDM%FT0)
  IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ROTUVGEO:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ROTUVGEO:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
  
  !$OMP TARGET TEAMS DISTRIBUTE  & 
  !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZP, ZQ, Z_YDA_PCCO_P,  & 
  !$OMP &          Z_YDVARS_GEOMETRY_GNORDL_P, Z_YDVARS_GEOMETRY_GNORDM_P)  & 
  !$OMP & PRIVATE (JBLK)  & 
  !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    !$OMP PARALLEL DO SIMD  & 
    !$OMP & PRIVATE (JROF, YLCPG_BNDS, YLSTACK)       
...
IF (YDCPG_OPTS%LRPLANE) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:LRPLANE')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (ZGMDTX, YL_ZGMDTX)
    CALL SGET_HOST_DATA_RDWR (ZGMDTY, YL_ZGMDTY)
    CALL SGET_HOST_DATA_RDWR (ZINEZV, YL_ZINEZV)
    CALL SGET_HOST_DATA_RDWR (Z_YDA_PCCO_P, YDA_PCCO%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDA_PSCO_P, YDA_PSCO%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_GM_P, YDVARS%GEOMETRY%GM%FT0)
    CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_RINDX_P, YDVARS%GEOMETRY%RINDX%FT0)
    CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_RINDY_P, YDVARS%GEOMETRY%RINDY%FT0)
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:LRPLANE')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (ZINEZV, YL_ZINEZV)
      CALL SGET_HOST_DATA_RDWR (Z_YDA_PCCO_P, YDA_PCCO%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDA_PSCO_P, YDA_PSCO%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GEOMETRY_GM_P, YDVARS%GEOMETRY%GM%FT0)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GEOMETRY_RINDX_P, YDVARS%GEOMETRY%RINDX%FT0)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GEOMETRY_RINDY_P, YDVARS%GEOMETRY%RINDY%FT0)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:LRPLANE')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LRPLANE:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDWR (ZGMDTX, YL_ZGMDTX)
    CALL SGET_DEVICE_DATA_RDWR (ZGMDTY, YL_ZGMDTY)
    CALL SGET_DEVICE_DATA_RDWR (ZINEZV, YL_ZINEZV)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDA_PCCO_P, YDA_PCCO%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDA_PSCO_P, YDA_PSCO%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_GM_P, YDVARS%GEOMETRY%GM%FT0)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_RINDX_P, YDVARS%GEOMETRY%RINDX%FT0)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_RINDY_P, YDVARS%GEOMETRY%RINDY%FT0)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LRPLANE:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LRPLANE:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZGMDTX,  & 
    !$OMP &          ZGMDTY, ZINEZV, Z_YDA_PCCO_P, Z_YDA_PSCO_P, Z_YDVARS_GEOMETRY_GM_P,  & 
    !$OMP &          Z_YDVARS_GEOMETRY_RINDX_P, Z_YDVARS_GEOMETRY_RINDY_P)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:ZVWEI')) THEN
  ...
  CALL SGET_HOST_DATA_RDWR (ZVWEI, YL_ZVWEI)
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:ZVWEI')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZVWEI:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
  CALL SGET_DEVICE_DATA_RDWR (ZVWEI, YL_ZVWEI)
  IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZVWEI:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ZVWEI:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
  
  !$OMP TARGET TEAMS DISTRIBUTE  & 
  !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YFXTRAN_ACDC_STACK, ZVWEI)  & 
  !$OMP & PRIVATE (JBLK)  & 
  !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (YDMODEL%YRML_DYN%YRDYN%NVLAG==2) THEN
  ...
  ELSE
    ...
    IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:NVLAG2')) THEN
      ...
      CALL SGET_HOST_DATA_RDONLY (ZC9, YL_ZC9)
      CALL SGET_HOST_DATA_RDONLY (ZDP9, YL_ZDP9)
      CALL SGET_HOST_DATA_RDONLY (ZVWEI, YL_ZVWEI)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_SP_T1, YDVARS%SP%FT1)
      ...
      IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:NVLAG2')) THEN
        ...
        CALL SGET_HOST_DATA_RDWR (ZC9, YL_ZC9)
        CALL SGET_HOST_DATA_RDWR (ZDP9, YL_ZDP9)
        CALL SGET_HOST_DATA_RDWR (ZVWEI, YL_ZVWEI)
        CALL SGET_HOST_DATA_RDWR (Z_YDVARS_SP_T1, YDVARS%SP%FT1)
    ...
    ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:NVLAG2')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:NVLAG2:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
      CALL SGET_DEVICE_DATA_RDONLY (ZC9, YL_ZC9)
      CALL SGET_DEVICE_DATA_RDONLY (ZDP9, YL_ZDP9)
      CALL SGET_DEVICE_DATA_RDONLY (ZVWEI, YL_ZVWEI)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_SP_T1, YDVARS%SP%FT1)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:NVLAG2:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:NVLAG2:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      
      !$OMP TARGET TEAMS DISTRIBUTE  & 
      !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YFXTRAN_ACDC_STACK, ZC9, ZDP9, ZVWEI, Z_YDVARS_SP_T1)  & 
      !$OMP & PRIVATE (JBLK)  & 
      !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
ELSEIF (YDMODEL%YRML_DYN%YRDYN%NVLAG==3) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:NVLAG3')) THEN
    ...
    CALL SGET_HOST_DATA_RDONLY (ZC9, YL_ZC9)
    CALL SGET_HOST_DATA_RDONLY (ZC9_SI, YL_ZC9_SI)
    CALL SGET_HOST_DATA_RDONLY (ZDP9, YL_ZDP9)
    CALL SGET_HOST_DATA_RDWR (ZSPT1, YL_ZSPT1)
    CALL SGET_HOST_DATA_RDWR (ZSPTO, YL_ZSPTO)
    CALL SGET_HOST_DATA_RDONLY (ZVWEI, YL_ZVWEI)
    CALL SGET_HOST_DATA_RDWR (Z_YDCPG_SL2_SPSI, YDCPG_SL2%F_SPSI)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_SP_T1, YDVARS%SP%FT1)
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:NVLAG3')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (ZC9, YL_ZC9)
      CALL SGET_HOST_DATA_RDWR (ZC9_SI, YL_ZC9_SI)
      CALL SGET_HOST_DATA_RDWR (ZDP9, YL_ZDP9)
      CALL SGET_HOST_DATA_RDWR (ZSPT1, YL_ZSPT1)
      CALL SGET_HOST_DATA_RDWR (ZSPTO, YL_ZSPTO)
      CALL SGET_HOST_DATA_RDWR (ZVWEI, YL_ZVWEI)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:NVLAG3')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:NVLAG3:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDONLY (ZC9, YL_ZC9)
    CALL SGET_DEVICE_DATA_RDONLY (ZC9_SI, YL_ZC9_SI)
    CALL SGET_DEVICE_DATA_RDONLY (ZDP9, YL_ZDP9)
    CALL SGET_DEVICE_DATA_RDWR (ZSPT1, YL_ZSPT1)
    CALL SGET_DEVICE_DATA_RDWR (ZSPTO, YL_ZSPTO)
    CALL SGET_DEVICE_DATA_RDONLY (ZVWEI, YL_ZVWEI)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDCPG_SL2_SPSI, YDCPG_SL2%F_SPSI)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_SP_T1, YDVARS%SP%FT1)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:NVLAG3:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:NVLAG3:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZC9, ZC9_SI,  & 
    !$OMP &          ZDP9, ZSPT1, ZSPTO, ZVWEI, Z_YDCPG_SL2_SPSI, Z_YDVARS_SP_T1)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
    ...
    DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
      ...
      !$OMP PARALLEL DO SIMD  & 
      !$OMP & PRIVATE (JLEV, JROF, LLSLINLC2, YLCPG_BNDS, YLSTACK)       
...
ELSE
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:ROTUV')) THEN
    ...
    CALL SGET_HOST_DATA_RDONLY (ZP, YL_ZP)
    CALL SGET_HOST_DATA_RDONLY (ZQ, YL_ZQ)
    CALL SGET_HOST_DATA_RDONLY (ZUP9, YL_ZUP9)
    CALL SGET_HOST_DATA_RDONLY (ZVP9, YL_ZVP9)
    CALL SGET_HOST_DATA_RDWR (Z_YDA_PUP9_P, YDA_PUP9%F_P)
    CALL SGET_HOST_DATA_RDWR (Z_YDA_PVP9_P, YDA_PVP9%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GMV9_SI_U_P, YDCPG_GMV9_SI%U%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GMV9_SI_V_P, YDCPG_GMV9_SI%V%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:ROTUV')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (Z_YDA_PUP9_P, YDA_PUP9%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDA_PVP9_P, YDA_PVP9%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_SI_U_P, YDCPG_GMV9_SI%U%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_SI_V_P, YDCPG_GMV9_SI%V%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_SL2_USI, YDCPG_SL2%F_USI)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_SL2_VSI, YDCPG_SL2%F_VSI)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_U_T1, YDVARS%U%FT1)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_V_T1, YDVARS%V%FT1)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:ROTUV')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ROTUV:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDONLY (ZP, YL_ZP)
    CALL SGET_DEVICE_DATA_RDONLY (ZQ, YL_ZQ)
    CALL SGET_DEVICE_DATA_RDONLY (ZUP9, YL_ZUP9)
    CALL SGET_DEVICE_DATA_RDONLY (ZVP9, YL_ZVP9)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDA_PUP9_P, YDA_PUP9%F_P)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDA_PVP9_P, YDA_PVP9%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GMV9_SI_U_P, YDCPG_GMV9_SI%U%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GMV9_SI_V_P, YDCPG_GMV9_SI%V%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
    ...
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZP, ZQ, ZUP9,  & 
    !$OMP &          ZVP9, Z_YDA_PUP9_P, Z_YDA_PVP9_P, Z_YDCPG_GMV9_SI_U_P, Z_YDCPG_GMV9_SI_V_P,  & 
    !$OMP &          Z_YDCPG_GMV9_U_P, Z_YDCPG_GMV9_V_P, Z_YDCPG_SL2_USI, Z_YDCPG_SL2_VSI,  & 
    !$OMP &          Z_YDVARS_U_T1, Z_YDVARS_V_T1)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:TEMP_EQ')) THEN
  ...
  CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GMV9_SI_T_P, YDCPG_GMV9_SI%T%F_P)
  CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GMV9_T_P, YDCPG_GMV9%T%F_P)
  ...
  IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:TEMP_EQ')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_SI_T_P, YDCPG_GMV9_SI%T%F_P)
    CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_T_P, YDCPG_GMV9%T%F_P)
    CALL SGET_HOST_DATA_RDWR (Z_YDCPG_SL2_TSI, YDCPG_SL2%F_TSI)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_T_T1, YDVARS%T%FT1)
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:TEMP_EQ')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:TEMP_EQ:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
  CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GMV9_SI_T_P, YDCPG_GMV9_SI%T%F_P)
  CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GMV9_T_P, YDCPG_GMV9%T%F_P)
  ...
  !$OMP TARGET TEAMS DISTRIBUTE  & 
  !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, Z_YDCPG_GMV9_SI_T_P,  & 
  !$OMP &          Z_YDCPG_GMV9_T_P, Z_YDCPG_SL2_TSI, Z_YDVARS_T_T1)  & 
  !$OMP & PRIVATE (JBLK)  & 
  !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
DO JFLD=1, SIZE (YDVARS%GFL_PTR)
  ...
  IF (YDMODEL%YRML_DYN%YRDYNA%LSLDIA.AND.(YDVARS%GFL_PTR (JFLD)%YCOMP%CNAME&
    ...
    IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:DPDETA0')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GFL_PTR_T1, YDVARS%GFL_PTR (JFLD)%FT1)
    ...
    ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:DPDETA0')) THEN
      ...
      CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_GFL_PTR_T1, YDVARS%GFL_PTR (JFLD)%FT1)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:DPDETA0:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:DPDETA0:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      
      !$OMP TARGET TEAMS DISTRIBUTE  & 
      !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YFXTRAN_ACDC_STACK, ZDP9, Z_YDVARS_GEOMETRY_OROG_P,  & 
      !$OMP &          Z_YDVARS_GFL_PTR_T1)  & 
      !$OMP & PRIVATE (JBLK)  & 
      !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
  ...
  IF (YDVARS%GFL_PTR (JFLD)%YCOMP%LADV) THEN
    ...
    IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:CVGQ')) THEN
      ...
      CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GFL9_GFL_P, YDCPG_GFL9%GFL (JFLD)%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GFL_PTR_T1, YDVARS%GFL_PTR (JFLD)%FT1)
      ...
      IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:CVGQ')) THEN
        ...
        CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GFL9_GFL_P, YDCPG_GFL9%GFL (JFLD)%F_P)
        CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GFL_PTR_T1, YDVARS%GFL_PTR (JFLD)%FT1)
    ...
    ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:CVGQ')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:CVGQ:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
      CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GFL9_GFL_P, YDCPG_GFL9%GFL (JFLD)%F_P)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_GFL_PTR_T1, YDVARS%GFL_PTR (JFLD)%FT1)
      ...
      !$OMP TARGET TEAMS DISTRIBUTE  & 
      !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDVARS, YFXTRAN_ACDC_STACK, Z_YDCPG_GFL9_GFL_P,  & 
      !$OMP &          Z_YDVARS_GFL_PTR_T1, Z_YDVARS_Q_T0)  & 
      !$OMP & PRIVATE (JBLK)  & 
      !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (YDMODEL%YRML_GCONF%YGFL%YCVGQ%LSP) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:YCVGQ')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_CVGQ_T1, YDVARS%CVGQ%FT1)
    CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_Q_T1, YDVARS%Q%FT1)
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:YCVGQ')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_CVGQ_T1, YDVARS%CVGQ%FT1)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_Q_T1, YDVARS%Q%FT1)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:YCVGQ')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:YCVGQ:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_CVGQ_T1, YDVARS%CVGQ%FT1)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_Q_T1, YDVARS%Q%FT1)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:YCVGQ:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:YCVGQ:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YFXTRAN_ACDC_STACK, Z_YDVARS_CVGQ_T1,  & 
    !$OMP &          Z_YDVARS_Q_T1)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (YDMODEL%YRML_DYN%YRDYNA%L_RDRY_VD) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:L_RDRY_VD')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (ZRT1, YL_ZRT1)
    CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_T_T1, YDVARS%T%FT1)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:L_RDRY_VD')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:L_RDRY_VD:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDWR (ZRT1, YL_ZRT1)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_T_T1, YDVARS%T%FT1)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:L_RDRY_VD:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:L_RDRY_VD:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZRT1, Z_YDVARS_T_T1)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (YDMODEL%YRML_DYN%YRDYN%LADVF.AND..NOT.(YDMODEL%YRML_DYN%YRDYNA&
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:LACONE')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
    CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_PCF_U_PC_PH, YDVARS%PCF_U%FPC_PH)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_PCF_V_PC_PH, YDVARS%PCF_V%FPC_PH)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_U_T1, YDVARS%U%FT1)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_V_T1, YDVARS%V%FT1)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:LACONE')) THEN
    ...
    CALL SGET_DEVICE_DATA_RDONLY (ZQ, YL_ZQ)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDA_PCCO_P, YDA_PCCO%F_P)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_PCF_U_PC_PH, YDVARS%PCF_U%FPC_PH)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_PCF_V_PC_PH, YDVARS%PCF_V%FPC_PH)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_U_T1, YDVARS%U%FT1)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_V_T1, YDVARS%V%FT1)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LACONE:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LACONE:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZP, ZQ, Z_YDA_PCCO_P,  & 
    !$OMP &          Z_YDCPG_GMV9_U_P, Z_YDCPG_GMV9_V_P, Z_YDVARS_PCF_U_PC_PH,  & 
    !$OMP &          Z_YDVARS_PCF_V_PC_PH, Z_YDVARS_U_T1, Z_YDVARS_V_T1)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (YDMODEL%YRML_DYN%YRDYN%L2TLFF.AND..NOT.(YDMODEL%YRML_DYN%YRDYNA&
  ...
  ELSE
    ...
    IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:LARCHE')) THEN
      ...
      CALL SGET_HOST_DATA_RDONLY (ZP, YL_ZP)
      CALL SGET_HOST_DATA_RDONLY (ZQ, YL_ZQ)
      CALL SGET_HOST_DATA_RDWR (ZUT1, YL_ZUT1)
      CALL SGET_HOST_DATA_RDONLY (ZUZ9, YL_ZUZ9)
      CALL SGET_HOST_DATA_RDWR (ZVT1, YL_ZVT1)
      CALL SGET_HOST_DATA_RDONLY (ZVZ9, YL_ZVZ9)
      CALL SGET_HOST_DATA_RDWR (Z_YDA_PCCO_P, YDA_PCCO%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDA_PSCO_P, YDA_PSCO%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
      ...
      CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_RCOLON_P, YDVARS%GEOMETRY%RCOLON%FT0)
      CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_RSILON_P, YDVARS%GEOMETRY%RSILON%FT0)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_U_T1, YDVARS%U%FT1)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_V_T1, YDVARS%V%FT1)
      ...
      IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:LARCHE')) THEN
        ...
        CALL SGET_HOST_DATA_RDWR (ZQ, YL_ZQ)
        CALL SGET_HOST_DATA_RDWR (ZUT1, YL_ZUT1)
        CALL SGET_HOST_DATA_RDWR (ZUZ9, YL_ZUZ9)
        CALL SGET_HOST_DATA_RDWR (ZVT1, YL_ZVT1)
        CALL SGET_HOST_DATA_RDWR (ZVZ9, YL_ZVZ9)
        CALL SGET_HOST_DATA_RDWR (Z_YDA_PCCO_P, YDA_PCCO%F_P)
        CALL SGET_HOST_DATA_RDWR (Z_YDA_PSCO_P, YDA_PSCO%F_P)
        CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
        CALL SGET_HOST_DATA_RDWR (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
        ...
        CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GEOMETRY_RCOLON_P, YDVARS%GEOMETRY%RCOLON%FT0)
        CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GEOMETRY_RSILON_P, YDVARS%GEOMETRY%RSILON%FT0)
        CALL SGET_HOST_DATA_RDWR (Z_YDVARS_U_T1, YDVARS%U%FT1)
        CALL SGET_HOST_DATA_RDWR (Z_YDVARS_V_T1, YDVARS%V%FT1)
    ...
    ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:LARCHE')) THEN
      ...
      CALL SGET_DEVICE_DATA_RDONLY (ZP, YL_ZP)
      CALL SGET_DEVICE_DATA_RDONLY (ZQ, YL_ZQ)
      CALL SGET_DEVICE_DATA_RDWR (ZUT1, YL_ZUT1)
      CALL SGET_DEVICE_DATA_RDONLY (ZUZ9, YL_ZUZ9)
      CALL SGET_DEVICE_DATA_RDWR (ZVT1, YL_ZVT1)
      CALL SGET_DEVICE_DATA_RDONLY (ZVZ9, YL_ZVZ9)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDA_PCCO_P, YDA_PCCO%F_P)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDA_PSCO_P, YDA_PSCO%F_P)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
      ...
      CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_RCOLON_P, YDVARS%GEOMETRY%RCOLON%FT0)
      CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_RSILON_P, YDVARS%GEOMETRY%RSILON%FT0)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_U_T1, YDVARS%U%FT1)
      CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_V_T1, YDVARS%V%FT1)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LARCHE:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LARCHE:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      ...
      !$OMP TARGET TEAMS DISTRIBUTE  & 
      !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZP, ZQ, ZUT1,  & 
      !$OMP &          ZUZ9, ZVT1, ZVZ9, Z_YDA_PCCO_P, Z_YDA_PSCO_P, Z_YDCPG_GMV9_U_P,  & 
      !$OMP &          Z_YDCPG_GMV9_V_P, Z_YDVARS_GEOMETRY_GECLO_P, Z_YDVARS_GEOMETRY_GEMU_P,  & 
      !$OMP &          Z_YDVARS_GEOMETRY_GESLO_P, Z_YDVARS_GEOMETRY_GNORDL_P,  & 
      !$OMP &          Z_YDVARS_GEOMETRY_GNORDL_T0, Z_YDVARS_GEOMETRY_GNORDM_P,  & 
      !$OMP &          Z_YDVARS_GEOMETRY_GNORDM_T0, Z_YDVARS_GEOMETRY_GSQM2_P,  & 
      !$OMP &          Z_YDVARS_GEOMETRY_RCOLON_P, Z_YDVARS_GEOMETRY_RSILON_P,  & 
      !$OMP &          Z_YDVARS_U_T1, Z_YDVARS_V_T1)  & 
      !$OMP & PRIVATE (JBLK)  & 
      !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
      ...
      DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
        ...
        !$OMP PARALLEL DO SIMD  & 
        !$OMP & PRIVATE (JLEV, JROF, YLCPG_BNDS, YLSTACK, ZPP1, ZPP9, ZQQ1, ZQQ9)       
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:ROTUVGEO')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ROTUVGEO:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_HOST_DATA_RDWR (ZP2, YL_ZP2)
    CALL SGET_HOST_DATA_RDWR (ZQ2, YL_ZQ2)
    CALL SGET_HOST_DATA_RDONLY (Z_YDA_PCCO_P, YDA_PCCO%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
    CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_GNORDL_P, YDVARS%GEOMETRY%GNORDL%FT0)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:ROTUVGEO')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:ROTUVGEO:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDWR (ZP2, YL_ZP2)
    CALL SGET_DEVICE_DATA_RDWR (ZQ2, YL_ZQ2)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDA_PCCO_P, YDA_PCCO%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GMV9_U_P, YDCPG_GMV9%U%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_GMV9_V_P, YDCPG_GMV9%V%F_P)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_GNORDL_P, YDVARS%GEOMETRY%GNORDL%FT0)
    ...
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZP2, ZQ2,  & 
    !$OMP &          Z_YDA_PCCO_P, Z_YDCPG_GMV9_U_P, Z_YDCPG_GMV9_V_P, Z_YDVARS_GEOMETRY_GNORDL_P,  & 
    !$OMP &          Z_YDVARS_GEOMETRY_GNORDM_P, Z_YDVARS_PCF_U_PC_PH, Z_YDVARS_PCF_V_PC_PH,  & 
    !$OMP &          Z_YDVARS_U_T1, Z_YDVARS_V_T1)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
...
IF (YDMODEL%YRML_DYN%YRDYNA%LRDBBC) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LAPINEB_PARALLEL:LRDBBC')) THEN
    ...
    CALL SGET_HOST_DATA_RDONLY (ZDBBC9, YL_ZDBBC9)
    CALL SGET_HOST_DATA_RDONLY (ZDPHI9, YL_ZDPHI9)
    CALL SGET_HOST_DATA_RDONLY (ZGWS9, YL_ZGWS9)
    CALL SGET_HOST_DATA_RDONLY (ZOROGL_SR, YL_ZOROGL_SR)
    CALL SGET_HOST_DATA_RDONLY (ZOROGM_SR, YL_ZOROGM_SR)
    CALL SGET_HOST_DATA_RDONLY (ZUST1, YL_ZUST1)
    CALL SGET_HOST_DATA_RDONLY (ZVST1, YL_ZVST1)
    CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_SL2_DBBC1, YDCPG_SL2%F_DBBC1)
    CALL SGET_HOST_DATA_RDONLY (Z_YDCPG_SL2_DPHI1, YDCPG_SL2%F_DPHI1)
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('LAPINEB_PARALLEL:LRDBBC')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (ZDBBC9, YL_ZDBBC9)
      CALL SGET_HOST_DATA_RDWR (ZDPHI9, YL_ZDPHI9)
      CALL SGET_HOST_DATA_RDWR (ZGWS9, YL_ZGWS9)
      CALL SGET_HOST_DATA_RDWR (ZOROGL_SR, YL_ZOROGL_SR)
      CALL SGET_HOST_DATA_RDWR (ZOROGM_SR, YL_ZOROGM_SR)
      CALL SGET_HOST_DATA_RDWR (ZUST1, YL_ZUST1)
      CALL SGET_HOST_DATA_RDWR (ZVST1, YL_ZVST1)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_SL2_DBBC1, YDCPG_SL2%F_DBBC1)
      CALL SGET_HOST_DATA_RDWR (Z_YDCPG_SL2_DPHI1, YDCPG_SL2%F_DPHI1)
      CALL SGET_HOST_DATA_RDWR (Z_YDVARS_SVD_T1, YDVARS%SVD%FT1)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LAPINEB_PARALLEL:LRDBBC')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LRDBBC:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDONLY (ZDBBC9, YL_ZDBBC9)
    CALL SGET_DEVICE_DATA_RDONLY (ZDPHI9, YL_ZDPHI9)
    CALL SGET_DEVICE_DATA_RDONLY (ZGWS9, YL_ZGWS9)
    CALL SGET_DEVICE_DATA_RDONLY (ZOROGL_SR, YL_ZOROGL_SR)
    CALL SGET_DEVICE_DATA_RDONLY (ZOROGM_SR, YL_ZOROGM_SR)
    CALL SGET_DEVICE_DATA_RDONLY (ZUST1, YL_ZUST1)
    CALL SGET_DEVICE_DATA_RDONLY (ZVST1, YL_ZVST1)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_SL2_DBBC1, YDCPG_SL2%F_DBBC1)
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDCPG_SL2_DPHI1, YDCPG_SL2%F_DPHI1)
    CALL SGET_DEVICE_DATA_RDWR (Z_YDVARS_SVD_T1, YDVARS%SVD%FT1)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LRDBBC:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LAPINEB_PARALLEL:LRDBBC:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YFXTRAN_ACDC_STACK, ZDBBC9, ZDPHI9, ZGWS9,  & 
    !$OMP &          ZOROGL_SR, ZOROGM_SR, ZUST1, ZVST1, Z_YDCPG_SL2_DBBC1, Z_YDCPG_SL2_DPHI1,  & 
    !$OMP &          Z_YDVARS_SVD_T1)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
