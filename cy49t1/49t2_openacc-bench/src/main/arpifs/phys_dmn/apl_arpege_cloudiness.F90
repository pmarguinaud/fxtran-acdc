...
REAL(KIND=JPRB),                INTENT(OUT)   :: PCLCT_RAD(YDCPG_OPTS%KLON) ! total cloud cover for radiation
...
!$ACDC PARALLEL, TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACNEBCOND {

CALL ACNEBCOND (YDMODEL%YRCST, YDMODEL%YRML_GCONF%YRRIP, YDMODEL%YRML_PHY_MF, KIDIA, KFDIA,                                 &
& KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, KFLEVG, LDREDPR, YDSTA, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI,                       &
& YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP,          &
& YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, PQV, PQI, PQL, PMSC_QW,                      &
...
IF(YDMODEL%YRML_PHY_MF%YRPHY%LNEBN) THEN
  CALL ACNEBN (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, KIDIA, KFDIA, KLON,                                  &
  & YDMODEL%YRML_PHY_MF%YRTOPH%NTNEBU, KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF,                      &
  & YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP, PQV, PQL, PQI, PFLU_QSAT, YDMF_PHYS_BASE_STATE%T, PPFL_FPLCH, &
  & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP,                    &
  & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, PUNEBH, PNEBS0, PQLIS0, PQLI_CVP, PNEB_CVPP, PQLI_CVPP,      &
  & PAIPCMT, YDCPG_MISC%NEB, PNEBC0, YDCPG_MISC%QICE, YDCPG_MISC%QLI, YDSTA)
...
CALL ACNPART(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, KIDIA, KFDIA, KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTNEBU,                   &
& KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF,   &
& PDECRD, YDCPG_MISC%NEB, YDMF_PHYS%OUT%CLCH, YDMF_PHYS%OUT%CLCM, YDMF_PHYS%OUT%CLCL, YDCPG_MISC%CLCT, PCLCT_RAD,         &
