...
INTEGER (KIND=JPIM)::JL_CUADJTQ000
REAL (KIND=JPRB)::ZQP
...
INTEGER (KIND=JPIM)::JL_CUADJTQ001
    ...
    DO JLL=1, JLM
      ...
      IF (PQU (JLON, JK)/=ZQOLD ) THEN
        ...
        IF (KLAB (JLON, JK+1)==2) THEN
          ...
          IF (PKINEU (JLON, JK)>0.0_JPRB.AND.PMFU (JLON, JK)>0.0_JPRB.AND.(ZBUO (JLON, JK)>-2._JPRB.OR.(PTEN&
            & (JLON, JK-1)-PTEN (JLON, JK))/(ZRG*(PGEO (JLON, JK-1)-PGEO (JLON, JK)))<-3.E-3_JPRB)) THEN
            KCTOP (JLON)=JK
            LLO1 =.TRUE.
          ELSE
            KLAB (JLON, JK)=0
    ...
    DO JL=KIDIA, KFDIA
      ...
      IF (LLO1 ) THEN
        ...
        IF (PLU (JLON, JK)>ZDNOPRC) THEN
          PWU (JLON, JK)=MIN (15._JPRB, SQRT (2.0_JPRB*MAX (0.5_JPRB, PKINEU (JLON, JK+1))))
          ...
          ZPRCON=ZXPRCDGW/(0.75_JPRB*PWU (JLON, JK))*ZZCO
      ...
      DO JL=KIDIA, KFDIA
        ...
        IF (LLO1 ) THEN
          ...
          IF (PLRAIN (JLON, JK)>0.0_JPRB) THEN
            ...
            ZC=ZPRECIP 
            PWU (JLON, JK)=MIN (15._JPRB, SQRT (2.0_JPRB*MAX (0.1_JPRB, PKINEU (JLON, JK))))
            ZD=ZVV/PWU (JLON, JK)
            ZINT=EXP (-ZD)
