SUBROUTINE APL_ARPEGE_PRECIPITATION_ACUPTQ (LDPTQ, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_BASE_STATE&
&, YDMF_PHYS_SURF, YDMODEL, YDSTA, YDVARS, PFLU_NEIJ, PNEBS, PNEB_CVPP, PQC_DET_PCMT, PQI, PQL&
&, PQLIS, PQLI_CVPP, PQR, PQS, PQV, PSEDIQI, PSEDIQL, PTENHA, PTENQVA)
...
IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PRECIPITATION_ACUPTQ', 0, ZHOOK_HANDLE)
ZFRSO (:,:)=0.0_JPRB
ZFRTH (:,:)=0.0_JPRB
CALL ACUPTQ (YDMODEL%YRCST, YDCPG_OPTS%KLON, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS&
&%KFLEVG, 0, LDPTQ, ZFRSO, ZFRTH, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT&
&%DIFTQ, YDMF_PHYS%OUT%DIFTS, YDMF_PHYS%OUT%FCCQL, YDMF_PHYS%OUT%FCCQN, YDMF_PHYS%OUT%FPLCL&
&, YDMF_PHYS%OUT%FPLCN, YDMF_PHYS%OUT%FPEVPCL, YDMF_PHYS%OUT%FPEVPCN, YDMF_PHYS%OUT%FPFPCL&
&, YDMF_PHYS%OUT%FPFPCN, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%RDELP, YDMF_PHYS_BASE_STATE%T&
...
CALL ACPLUIZ (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDCPG_OPTS&
&%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, ZTMIC, ZQMIC, PQL, PQI, PQR, PQS,&
& YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE&
&%YCPG_DYN%RCP%CP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R, PNEBS, PQLIS, PNEB_CVPP, PQLI_CVPP, PQC_DET_PCMT&
&, PTENHA, PTENQVA,.TRUE., YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YGSP_RR%T, PFLU_NEIJ&
&, YDMF_PHYS_SURF%GSD_VF%PLSM, YDVARS%GEOMETRY%GM%T0, YDSTA, YDMF_PHYS%OUT%FCSQL, YDMF_PHYS%OUT&
&%FCSQN, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPEVPSL, YDMF_PHYS%OUT%FPEVPSN&
&, YDMF_PHYS%OUT%FPFPSL, YDMF_PHYS%OUT%FPFPSN, PSEDIQL, PSEDIQI)
IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_PRECIPITATION_ACUPTQ', 1, ZHOOK_HANDLE)
ENDSUBROUTINE
