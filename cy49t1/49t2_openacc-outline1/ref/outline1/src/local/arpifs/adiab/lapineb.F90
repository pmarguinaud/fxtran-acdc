...
DO JFLD=1, SIZE (YDCPG_GMV9%GMV)
  CALL LAPINEB_ZERO_GMV9 (YDCPG_GMV9, JFLD)
ENDDO
...
CALL LAPINEB_ROTUVGEO_1 (YDVARS, YDMODEL, YDA_PCCO, YDGEOMETRY, YDCPG_BNDS, ZP, ZQ)
IHVIO=0
...
IF (YDCPG_OPTS%LRPLANE) THEN
  ...
  CALL LAPINEB_LRPLANE (ZDGUX, ZDLUN, YDVARS, YDA_PSCO, ZDLUX, ZDGUN, YDA_PCCO&
  &, YDMODEL, YDGEOMETRY, YDCPG_BNDS, ZGMDTY, ZGMDTX, ZINEZV)
ENDIF

CALL LAPINEB_ZVWEI (YDGEOMETRY, YDCPG_BNDS, ZVWEI)
...
IF (YDMODEL%YRML_DYN%YRDYN%NVLAG==2) THEN
  ...
  ELSE
    CALL LAPINEB_NVLAG2 (YDVARS, YDGEOMETRY, YDCPG_BNDS, ZVWEI, ZC9, ZDP9)
  ENDIF

ELSEIF (YDMODEL%YRML_DYN%YRDYN%NVLAG==3) THEN
  CALL LAPINEB_NVLAG3 (YDVARS, YDCPG_SL2, YDMODEL, YDGEOMETRY&
  &, YDCPG_BNDS, ZVWEI, ZC9, ZSPTO, ZC9_SI, ZSPT1, ZDP9)
ENDIF
...
ELSE
  CALL LAPINEB_ROTUV (LDSLPHY, YDVARS, YDCPG_GMV9, YDCPG_SL2, YDA_PUP9, YDMODEL&
  &, YDCPG_GMV9_SI, YDGEOMETRY, YDA_PVP9, YDCPG_BNDS, ZP, ZUP9, ZVP9, ZQ)
ENDIF

CALL LAPINEB_TEMP_EQ (YDVARS, YDCPG_GMV9, YDCPG_SL2, YDMODEL, YDGEOMETRY, YDCPG_GMV9_SI, YDCPG_BNDS)
...
DO JFLD=1, SIZE (YDVARS%GFL_PTR)
  ...
  IF (YDMODEL%YRML_DYN%YRDYNA%LSLDIA.AND.(YDVARS%GFL_PTR (JFLD)%YCOMP%CNAME&
    ...
    CALL LAPINEB_DPDETA0 (YDVARS, JFLD, ZCMSLP, YDGEOMETRY, YDCPG_BNDS, ZDP9)
  ENDIF


  IF (YDVARS%GFL_PTR (JFLD)%YCOMP%LADV) THEN
    CALL LAPINEB_CVGQ (YDVARS, JFLD, YDCPG_GFL9, YDGEOMETRY, YDCPG_BNDS)
  ENDIF
...
IF (YDMODEL%YRML_GCONF%YGFL%YCVGQ%LSP) THEN
  CALL LAPINEB_YCVGQ (YDVARS, YDGEOMETRY, YDCPG_BNDS)
ENDIF
...
IF (YDMODEL%YRML_DYN%YRDYNA%L_RDRY_VD) THEN
  CALL LAPINEB_L_RDRY_VD (YDVARS, YDMODEL, YDGEOMETRY, YDCPG_BNDS, ZRT1)
ELSE
...
IF (YDMODEL%YRML_DYN%YRDYN%LADVF.AND..NOT.(YDMODEL%YRML_DYN%YRDYNA&
  ...
  CALL LAPINEB_LACONE (YDVARS, YDCPG_GMV9, YDCPG_OPTS, ZDSTRET&
  &, YDA_PCCO, YDMODEL, YDGEOMETRY, YDCPG_BNDS, ZP, ZQ)
ENDIF
...
IF (YDMODEL%YRML_DYN%YRDYN%L2TLFF.AND..NOT.(YDMODEL%YRML_DYN%YRDYNA&
  ...
  ELSE
    CALL LAPINEB_LARCHE (YDVARS, YDA_PSCO, YDCPG_OPTS, ZDSTRET, YDA_PCCO, YDGEOMETRY&
    &, YDCPG_GMV9, ZDEPI, YDMODEL, YDCPG_BNDS, ZP, ZUT1, ZQ, ZUZ9, ZVZ9, ZVT1)
  ENDIF

  CALL LAPINEB_ROTUVGEO_2 (YDVARS, YDCPG_GMV9, YDA_PCCO, YDMODEL, YDGEOMETRY, YDCPG_BNDS)
ENDIF
...
IF (YDMODEL%YRML_DYN%YRDYNA%LRDBBC) THEN
  CALL LAPINEB_LRDBBC (YDVARS, YDCPG_SL2, YDGEOMETRY, YDCPG_BNDS,&
  & ZVST1, ZOROGL_SR, ZDPHI9, ZDBBC9, ZUST1, ZOROGM_SR, ZGWS9)
ENDIF
