...
LNCELLCU = ALLOCATED (SELF%NCELLCU)
IF (LNCELLCU) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%NCELLCU)
  !$OMP TARGET UPDATE TO (SELF%NCELLCU)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%NCELLCU)
ENDIF

LNFERTCU = ALLOCATED (SELF%NFERTCU)
IF (LNFERTCU) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%NFERTCU)
  !$OMP TARGET UPDATE TO (SELF%NFERTCU)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%NFERTCU)
ENDIF

LNBLS = ALLOCATED (SELF%NBLS)
IF (LNBLS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%NBLS)
  !$OMP TARGET UPDATE TO (SELF%NBLS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%NBLS)
ENDIF

LNBSS = ALLOCATED (SELF%NBSS)
IF (LNBSS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%NBSS)
  !$OMP TARGET UPDATE TO (SELF%NBSS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%NBSS)
ENDIF

LNRNBLS = ALLOCATED (SELF%NRNBLS)
IF (LNRNBLS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%NRNBLS)
  !$OMP TARGET UPDATE TO (SELF%NRNBLS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%NRNBLS)
ENDIF

LNRNBSS = ALLOCATED (SELF%NRNBSS)
IF (LNRNBSS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%NRNBSS)
  !$OMP TARGET UPDATE TO (SELF%NRNBSS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%NRNBSS)
ENDIF

LNBEW = ALLOCATED (SELF%NBEW)
IF (LNBEW) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%NBEW)
  !$OMP TARGET UPDATE TO (SELF%NBEW)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%NBEW)
ENDIF

LNBNS = ALLOCATED (SELF%NBNS)
IF (LNBNS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%NBNS)
  !$OMP TARGET UPDATE TO (SELF%NBNS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%NBNS)
ENDIF

LRCUCONVCA = ALLOCATED (SELF%RCUCONVCA)
IF (LRCUCONVCA) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RCUCONVCA)
  !$OMP TARGET UPDATE TO (SELF%RCUCONVCA)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RCUCONVCA)
ENDIF

LRNLCONVCA = ALLOCATED (SELF%RNLCONVCA)
IF (LRNLCONVCA) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RNLCONVCA)
  !$OMP TARGET UPDATE TO (SELF%RNLCONVCA)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RNLCONVCA)
ENDIF

LRCAPECONVCA = ALLOCATED (SELF%RCAPECONVCA)
IF (LRCAPECONVCA) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RCAPECONVCA)
  !$OMP TARGET UPDATE TO (SELF%RCAPECONVCA)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RCAPECONVCA)
ENDIF

LRWASALIVE = ALLOCATED (SELF%RWASALIVE)
IF (LRWASALIVE) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RWASALIVE)
  !$OMP TARGET UPDATE TO (SELF%RWASALIVE)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RWASALIVE)
ENDIF

LRWGHTCU = ALLOCATED (SELF%RWGHTCU)
IF (LRWGHTCU) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RWGHTCU)
  !$OMP TARGET UPDATE TO (SELF%RWGHTCU)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RWGHTCU)
ENDIF

LRLATLONNBLS = ALLOCATED (SELF%RLATLONNBLS)
IF (LRLATLONNBLS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RLATLONNBLS)
  !$OMP TARGET UPDATE TO (SELF%RLATLONNBLS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RLATLONNBLS)
ENDIF

LRLATLONNBSS = ALLOCATED (SELF%RLATLONNBSS)
IF (LRLATLONNBSS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RLATLONNBSS)
  !$OMP TARGET UPDATE TO (SELF%RLATLONNBSS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RLATLONNBSS)
ENDIF

LRLONDEP = ALLOCATED (SELF%RLONDEP)
IF (LRLONDEP) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RLONDEP)
  !$OMP TARGET UPDATE TO (SELF%RLONDEP)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RLONDEP)
ENDIF

LRLATDEP = ALLOCATED (SELF%RLATDEP)
IF (LRLATDEP) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RLATDEP)
  !$OMP TARGET UPDATE TO (SELF%RLATDEP)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RLATDEP)
ENDIF

LSLLONG = ALLOCATED (SELF%SLLONG)
IF (LSLLONG) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SLLONG)
  !$OMP TARGET UPDATE TO (SELF%SLLONG)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SLLONG)
ENDIF

LSLLAT = ALLOCATED (SELF%SLLAT)
IF (LSLLAT) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SLLAT)
  !$OMP TARGET UPDATE TO (SELF%SLLAT)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SLLAT)
ENDIF

LSLDLONG = ALLOCATED (SELF%SLDLONG)
IF (LSLDLONG) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SLDLONG)
  !$OMP TARGET UPDATE TO (SELF%SLDLONG)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SLDLONG)
ENDIF

LSLDLAT = ALLOCATED (SELF%SLDLAT)
IF (LSLDLAT) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SLDLAT)
  !$OMP TARGET UPDATE TO (SELF%SLDLAT)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SLDLAT)
ENDIF

LSLDDLAT = ALLOCATED (SELF%SLDDLAT)
IF (LSLDDLAT) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SLDDLAT)
  !$OMP TARGET UPDATE TO (SELF%SLDDLAT)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SLDDLAT)
ENDIF
...
LNCELLCU = ALLOCATED (SELF%NCELLCU)
IF (LNCELLCU) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%NCELLCU)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%NCELLCU)
ENDIF

LNFERTCU = ALLOCATED (SELF%NFERTCU)
IF (LNFERTCU) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%NFERTCU)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%NFERTCU)
ENDIF

LNBLS = ALLOCATED (SELF%NBLS)
IF (LNBLS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%NBLS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%NBLS)
ENDIF

LNBSS = ALLOCATED (SELF%NBSS)
IF (LNBSS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%NBSS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%NBSS)
ENDIF

LNRNBLS = ALLOCATED (SELF%NRNBLS)
IF (LNRNBLS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%NRNBLS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%NRNBLS)
ENDIF

LNRNBSS = ALLOCATED (SELF%NRNBSS)
IF (LNRNBSS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%NRNBSS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%NRNBSS)
ENDIF

LNBEW = ALLOCATED (SELF%NBEW)
IF (LNBEW) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%NBEW)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%NBEW)
ENDIF

LNBNS = ALLOCATED (SELF%NBNS)
IF (LNBNS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%NBNS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%NBNS)
ENDIF

LRCUCONVCA = ALLOCATED (SELF%RCUCONVCA)
IF (LRCUCONVCA) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RCUCONVCA)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RCUCONVCA)
ENDIF

LRNLCONVCA = ALLOCATED (SELF%RNLCONVCA)
IF (LRNLCONVCA) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RNLCONVCA)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RNLCONVCA)
ENDIF

LRCAPECONVCA = ALLOCATED (SELF%RCAPECONVCA)
IF (LRCAPECONVCA) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RCAPECONVCA)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RCAPECONVCA)
ENDIF

LRWASALIVE = ALLOCATED (SELF%RWASALIVE)
IF (LRWASALIVE) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RWASALIVE)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RWASALIVE)
ENDIF

LRWGHTCU = ALLOCATED (SELF%RWGHTCU)
IF (LRWGHTCU) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RWGHTCU)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RWGHTCU)
ENDIF

LRLATLONNBLS = ALLOCATED (SELF%RLATLONNBLS)
IF (LRLATLONNBLS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RLATLONNBLS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RLATLONNBLS)
ENDIF

LRLATLONNBSS = ALLOCATED (SELF%RLATLONNBSS)
IF (LRLATLONNBSS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RLATLONNBSS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RLATLONNBSS)
ENDIF

LRLONDEP = ALLOCATED (SELF%RLONDEP)
IF (LRLONDEP) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RLONDEP)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RLONDEP)
ENDIF

LRLATDEP = ALLOCATED (SELF%RLATDEP)
IF (LRLATDEP) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RLATDEP)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RLATDEP)
ENDIF

LSLLONG = ALLOCATED (SELF%SLLONG)
IF (LSLLONG) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SLLONG)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SLLONG)
ENDIF

LSLLAT = ALLOCATED (SELF%SLLAT)
IF (LSLLAT) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SLLAT)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SLLAT)
ENDIF

LSLDLONG = ALLOCATED (SELF%SLDLONG)
IF (LSLDLONG) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SLDLONG)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SLDLONG)
ENDIF

LSLDLAT = ALLOCATED (SELF%SLDLAT)
IF (LSLDLAT) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SLDLAT)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SLDLAT)
ENDIF

LSLDDLAT = ALLOCATED (SELF%SLDDLAT)
IF (LSLDDLAT) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SLDDLAT)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SLDDLAT)
ENDIF
