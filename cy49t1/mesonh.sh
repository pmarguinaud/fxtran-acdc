...
do
  echo "==> $f <=="
  openacc.pl --interface --stack84 --version --inline-contained --cycle 49 --style MesoNH --dir src/local/mpa/openacc $(resolve --user $f)
done
...
do
  echo "==> $f <=="
  openacc.pl --interface --stack84 --version --cycle 49 --style MesoNH --dir src/local/mpa/openacc --modi $(resolve --user $f)
done
