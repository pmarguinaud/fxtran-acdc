...
!$OMP TARGET DATA MAP (TOFROM:  A(1:LDA, 1:K), B(1:LDB, 1:K), C(1:LDC)) IF (LDACC)
...
#endif
#ifdef _FXTRAN_USE_OPENMPTARGET
!$OMP TARGET DATA MAP (TOFROM: A (1:LDA,1:K), B(1:LDB,1:K), C(1:LDC)) 
!$OMP TARGET DATA USE_DEVICE_ADDR (A, B, C)
    ...
    CALL CHECKCUBLAS (&
     ...
     CALL FXTRAN_ACDC_CHECK_BLAS (&
       & FXTRAN_ACDC_DGEMM (GETROCBLASHANDLE (), FXTRAN_ACDC_OP_N, FXTRAN_ACDC_OP_T, M, N, K, &
       &         ALPHA, A (1, 1),   LDA, &
       &                B (LDB, 1), LDB, &
       &         BETA,  C (1),      LDC))
#endif
