...
IF (KQM==-1) THEN
  ...
  IF (.NOT.PRESENT (PRDETAR))  THEN
      CALL ABOR1_ACC (' LAITRI_WENO : MISSING ARGUMENT PRDETAR')
  ENDIF


  IF (.NOT.PRESENT (LDQM3DCONS))  THEN
      CALL ABOR1_ACC (' LAITRI_WENO : MISSING ARGUMENT LDQM3DCONS')
  ENDIF
...
IF (CDTYPE=='SCALAR') THEN
  ...
  DO JLEV=1, KFLEV
    ...
    DO JROF=KST, KEND
      ...
      Z (JROF, 1)=FCWEIGHT (PCLA (JROF, JLEV, 1), PCLA (JROF,&
      ...
      Z (JROF, 4)=FCWEIGHT (PCLA (JROF, JLEV, 1), PCLA (JROF,&
      & JLEV, 2), PCLA (JROF, JLEV, 3), Z01, Z10, Z20, Z31)
    ENDDO
    ...
    DO JROF=KST, KEND
      ...
      Z (JROF, 2)=FCWEIGHT (PCLA (JROF, JLEV, 1), PCLA (JROF,&
      & JLEV, 2), PCLA (JROF, JLEV, 3), Z01, Z10, Z20, Z31)
      ...
      Z (JROF, 3)=FCWEIGHT (PCLA (JROF, JLEV, 1), PCLA (JROF,&
      & JLEV, 2), PCLA (JROF, JLEV, 3), Z01, Z10, Z20, Z31)
    ...
    DO JROF=KST, KEND
      ...
      IF (YDDYN%LWENOBC.OR.(KNOWENO (JROF, JLEV)==0)) THEN
        ...
        DO JJ=1, 3
          ...
          ZFX (JJ)=Z (JROF, J0)+PVINTW (JROF, JLEV, 1+3*(JJ-1))*(Z (JROF, J0+1)-Z&
          & (JROF, J0))+PVINTW (JROF, JLEV, 2+3*(JJ-1))*(Z (JROF, J0+2)-Z (JROF, J0&
          &))+PVINTW (JROF, JLEV, 3+3*(JJ-1))*(Z (JROF, J0+3)-Z (JROF, J0))
          ZBETA (JJ)=YDDYN%RBETAK(JJ, 1)*Z (JROF, J0+3)*Z (JROF, J0+3)+YDDYN%RBETAK(JJ, 2)*Z (JROF&
          &, J0+2)*Z (JROF, J0+2)+YDDYN%RBETAK(JJ, 3)*Z (JROF, J0+1)*Z (JROF, J0+1)+YDDYN%RBETAK&
          &(JJ, 4)*Z (JROF, J0)*Z (JROF, J0)+YDDYN%RBETAK(JJ, 5)*Z (JROF, J0+3)*Z (JROF, J0+2)+YDDYN&
          &%RBETAK(JJ, 6)*Z (JROF, J0+3)*Z (JROF, J0+1)+YDDYN%RBETAK(JJ, 7)*Z (JROF, J0+3)*Z (JROF&
          &, J0)+YDDYN%RBETAK(JJ, 8)*Z (JROF, J0+2)*Z (JROF, J0+1)+YDDYN%RBETAK(JJ, 9)*Z (JROF,&
          & J0+2)*Z (JROF, J0)+YDDYN%RBETAK(JJ, 10)*Z (JROF, J0+1)*Z (JROF, J0)
      ...
      ELSE
        ZXF (JROF, JLEV)=FCWEIGHT (PVINTW (JROF, JLEV, 1), PVINTW (JROF, JLEV, 2), PVINTW&
        & (JROF, JLEV, 3), Z (JROF, 1), Z (JROF, 2), Z (JROF, 3), Z (JROF, 4))
      ENDIF
...
ELSEIF (CDTYPE=='VECTOR') THEN
  ...
  DO JLEV=1, KFLEV
    ...
    DO JROF=KST, KEND
      ...
      Z (JROF, 1)=ZZ00+PCLA (JROF, JLEV, 1)*(ZZ10-ZZ00)+PCLA (JROF&
      &, JLEV, 2)*(ZZ20-ZZ00)+PCLA (JROF, JLEV, 3)*(ZZ30-ZZ00)
      Z (JROF, 2)=ZZ01+PCLA (JROF, JLEV, 1)*(ZZ11-ZZ01)+PCLA (JROF&
      &, JLEV, 2)*(ZZ21-ZZ01)+PCLA (JROF, JLEV, 3)*(ZZ31-ZZ01)
      Z (JROF, 3)=ZZ02+PCLA (JROF, JLEV, 1)*(ZZ12-ZZ02)+PCLA (JROF&
      &, JLEV, 2)*(ZZ22-ZZ02)+PCLA (JROF, JLEV, 3)*(ZZ32-ZZ02)
      Z (JROF, 4)=ZZ03+PCLA (JROF, JLEV, 1)*(ZZ13-ZZ03)+PCLA (JROF&
      &, JLEV, 2)*(ZZ23-ZZ03)+PCLA (JROF, JLEV, 3)*(ZZ33-ZZ03)
    ...
    DO JROF=KST, KEND
      ZFX (1)=Z (JROF, 1)+PVINTW (JROF, JLEV, 1)*(Z (JROF, 2)-Z (JROF, 1))+PVINTW (JROF, JLEV&
      &, 2)*(Z (JROF, 3)-Z (JROF, 1))+PVINTW (JROF, JLEV, 3)*(Z (JROF, 4)-Z (JROF, 1))

      IF (YDDYN%LWENOBC.OR.(KNOWENO (JROF, JLEV)==0)) THEN
        ZBETA (1)=YDDYN%RBETAK(1, 1)*Z (JROF, 4)*Z (JROF, 4)+YDDYN%RBETAK(1, 2)*Z (JROF, 3)*Z (JROF&
        &, 3)+YDDYN%RBETAK(1, 3)*Z (JROF, 2)*Z (JROF, 2)+YDDYN%RBETAK(1, 4)*Z (JROF, 1)*Z (JROF, 1&
        &)+YDDYN%RBETAK(1, 5)*Z (JROF, 4)*Z (JROF, 3)+YDDYN%RBETAK(1, 6)*Z (JROF, 4)*Z (JROF, 2)+YDDYN&
        &%RBETAK(1, 7)*Z (JROF, 4)*Z (JROF, 1)+YDDYN%RBETAK(1, 8)*Z (JROF, 3)*Z (JROF, 2)+YDDYN%RBETAK&
        &(1, 9)*Z (JROF, 3)*Z (JROF, 1)+YDDYN%RBETAK(1, 10)*Z (JROF, 2)*Z (JROF, 1)
        ZWW (1)=(ZBETA (1)+ZEPS)**ZALPHA
        ZWW (1)=SIGN (MAX (ABS (ZWW (1)), ZEPS_MAX), ZWW (1))
        ZWW (1)=PCW (JROF, JLEV, 1)/ZWW (1)
        J0=2+MIN (0, KNOWENO (JROF, JLEV))
        ZFX (2)=Z (JROF, J0)+PVINTW (JROF, JLEV, 4)*(Z (JROF, J0+1)-Z (JROF, J0))+PVINTW (JROF, JLEV&
        &, 5)*(Z (JROF, J0+2)-Z (JROF, J0))+PVINTW (JROF, JLEV, 6)*(Z (JROF, J0+3)-Z (JROF, J0))
        ZBETA (2)=YDDYN%RBETAK(2, 1)*Z (JROF, J0+3)*Z (JROF, J0+3)+YDDYN%RBETAK(2, 2)*Z (JROF, J0+2)*Z (JROF&
        &, J0+2)+YDDYN%RBETAK(2, 3)*Z (JROF, J0+1)*Z (JROF, J0+1)+YDDYN%RBETAK(2, 4)*Z (JROF, J0)*Z (JROF, J0&
        &)+YDDYN%RBETAK(2, 5)*Z (JROF, J0+3)*Z (JROF, J0+2)+YDDYN%RBETAK(2, 6)*Z (JROF, J0+3)*Z (JROF, J0+1)&
        &+YDDYN%RBETAK(2, 7)*Z (JROF, J0+3)*Z (JROF, J0)+YDDYN%RBETAK(2, 8)*Z (JROF, J0+2)*Z (JROF, J0+1)+YDDYN&
        &%RBETAK(2, 9)*Z (JROF, J0+2)*Z (JROF, J0)+YDDYN%RBETAK(2, 10)*Z (JROF, J0+1)*Z (JROF, J0)
        ZWW (2)=(ZBETA (2)+ZEPS)**ZALPHA
        ZWW (2)=SIGN (MAX (ABS (ZWW (2)), ZEPS_MAX), ZWW (2))
        ZWW (2)=PCW (JROF, JLEV, 2)/ZWW (2)
        J0=MAX (0, KNOWENO (JROF, JLEV))
        ZFX (3)=Z (JROF, J0)+PVINTW (JROF, JLEV, 7)*(Z (JROF, J0+1)-Z (JROF, J0))+PVINTW (JROF, JLEV&
        &, 8)*(Z (JROF, J0+2)-Z (JROF, J0))+PVINTW (JROF, JLEV, 9)*(Z (JROF, J0+3)-Z (JROF, J0))
        ZBETA (3)=YDDYN%RBETAK(3, 1)*Z (JROF, J0+3)*Z (JROF, J0+3)+YDDYN%RBETAK(3, 2)*Z (JROF, J0+2)*Z (JROF&
        &, J0+2)+YDDYN%RBETAK(3, 3)*Z (JROF, J0+1)*Z (JROF, J0+1)+YDDYN%RBETAK(3, 4)*Z (JROF, J0)*Z (JROF, J0&
        &)+YDDYN%RBETAK(3, 5)*Z (JROF, J0+3)*Z (JROF, J0+2)+YDDYN%RBETAK(3, 6)*Z (JROF, J0+3)*Z (JROF, J0+1)&
        &+YDDYN%RBETAK(3, 7)*Z (JROF, J0+3)*Z (JROF, J0)+YDDYN%RBETAK(3, 8)*Z (JROF, J0+2)*Z (JROF, J0+1)+YDDYN&
        &%RBETAK(3, 9)*Z (JROF, J0+2)*Z (JROF, J0)+YDDYN%RBETAK(3, 10)*Z (JROF, J0+1)*Z (JROF, J0)
...
ELSE
  CALL ABOR1_ACC ('LAITRI_WENO: UNEXPECTED CDTYPE')
ENDIF
...
ENDSUBROUTINE LAITRI_WENO_OPENACC
