...
DO JBLK = 1, KGPBLKS
  ...
  DO JL = KIDIA, MERGE (KLON, KFDIA, JBLK < KGPBLKS)
    ...
    CALL CUININ_OPENACC (YDCST, YDTHF, YDML_PHY_SLIN%YREPHLI, YDML_PHY_EC%YRECUMF, JL, JL, KLON, KLEV&
    &, PTEN(LBOUND (PTEN, 1), LBOUND (PTEN, 2), JBLK), PQEN(LBOUND (PQEN, 1), LBOUND (PQEN, 2), JBLK&
    &), PQSEN(LBOUND (PQSEN, 1), LBOUND (PQSEN, 2), JBLK), PUEN(LBOUND (PUEN, 1), LBOUND (PUEN, 2)&
    &, JBLK), PVEN(LBOUND (PVEN, 1), LBOUND (PVEN, 2), JBLK), PGEO(LBOUND (PGEO, 1), LBOUND (PGEO,&
    & 2), JBLK), PAPH(LBOUND (PAPH, 1), LBOUND (PAPH, 2), JBLK), ILAB(LBOUND (ILAB, 1), LBOUND (ILAB&
    &, 2), JBLK), ZTENH(LBOUND (ZTENH, 1), LBOUND (ZTENH, 2), JBLK), ZQENH(LBOUND (ZQENH, 1), LBOUND&
    & (ZQENH, 2), JBLK), ZQSENH(LBOUND (ZQSENH, 1), LBOUND (ZQSENH, 2), JBLK), PGEOH(LBOUND (PGEOH&
    &, 1), LBOUND (PGEOH, 2), JBLK), PTU(LBOUND (PTU, 1), LBOUND (PTU, 2), JBLK), PQU(LBOUND (PQU,&
    & 1), LBOUND (PQU, 2), JBLK), ZTD(LBOUND (ZTD, 1), LBOUND (ZTD, 2), JBLK), ZQD(LBOUND (ZQD, 1)&
    &, LBOUND (ZQD, 2), JBLK), ZUU(LBOUND (ZUU, 1), LBOUND (ZUU, 2), JBLK), ZVU(LBOUND (ZVU, 1), LBOUND&
    & (ZVU, 2), JBLK), ZUD(LBOUND (ZUD, 1), LBOUND (ZUD, 2), JBLK), ZVD(LBOUND (ZVD, 1), LBOUND (ZVD&
    &, 2), JBLK), PLU(LBOUND (PLU, 1), LBOUND (PLU, 2), JBLK), YDSTACK=YLSTACK)
  ...
  DO JBLK = 1, KGPBLKS
    ...
    DO JL = KIDIA, MERGE (KLON, KFDIA, JBLK < KGPBLKS)
      ...
      IF (LDCUM (JL, JBLK)) THEN
        ...
        ZMF_SHAL (JL, JBLK)=0.07_JPRB*(YDCST%RG/PTEN (JL, KLEV, JBLK&
        &)*ZDZ*MAX (0.0_JPRB, ZKHVFL (JL, JBLK)))**.3333_JPRB
...
DO JBLK = 1, KGPBLKS
  ...
  !$ACC LOOP VECTOR &
  !$ACC&PRIVATE (IKB, JL, ZDH, ZDQMIN, ZMFMAX, ZQUMQE) 
...
DO JBLK = 1, KGPBLKS
  ...
  DO JL = KIDIA, MERGE (KLON, KFDIA, JBLK < KGPBLKS)
    ...
    IF (YDML_PHY_EC%YRECUMF%LMFDD) THEN
      CALL CUDLFSN_OPENACC (YDTHF, YDCST, YDML_PHY_SLIN%YREPHLI, YDML_PHY_EC%YRECUMF, JL, JL, KLON, KLEV,&
      & KCBOT(LBOUND (KCBOT, 1), JBLK), KCTOP(LBOUND (KCTOP, 1), JBLK), LDCUM(LBOUND (LDCUM, 1), JBLK), ZTENH&
      &(LBOUND (ZTENH, 1), LBOUND (ZTENH, 2), JBLK), ZQENH(LBOUND (ZQENH, 1), LBOUND (ZQENH, 2), JBLK), PTEN&
      &(LBOUND (PTEN, 1), LBOUND (PTEN, 2), JBLK), PQSEN(LBOUND (PQSEN, 1), LBOUND (PQSEN, 2), JBLK), PGEO&
      &(LBOUND (PGEO, 1), LBOUND (PGEO, 2), JBLK), PGEOH(LBOUND (PGEOH, 1), LBOUND (PGEOH, 2), JBLK), PAPH&
      &(LBOUND (PAPH, 1), LBOUND (PAPH, 2), JBLK), PTU(LBOUND (PTU, 1), LBOUND (PTU, 2), JBLK), PQU(LBOUND&
      & (PQU, 1), LBOUND (PQU, 2), JBLK), ZMFUB(LBOUND (ZMFUB, 1), JBLK), ZRFL(LBOUND (ZRFL, 1), JBLK), ZTD&
      &(LBOUND (ZTD, 1), LBOUND (ZTD, 2), JBLK), ZQD(LBOUND (ZQD, 1), LBOUND (ZQD, 2), JBLK), PMFD(LBOUND&
      & (PMFD, 1), LBOUND (PMFD, 2), JBLK), ZMFDS(LBOUND (ZMFDS, 1), LBOUND (ZMFDS, 2), JBLK), ZMFDQ(LBOUND&
      & (ZMFDQ, 1), LBOUND (ZMFDQ, 2), JBLK), ZDMFDP(LBOUND (ZDMFDP, 1), LBOUND (ZDMFDP, 2), JBLK), IDTOP&
      &(LBOUND (IDTOP, 1), JBLK), LLDDRAF(LBOUND (LLDDRAF, 1), JBLK), YDSTACK=YLSTACK)
      CALL CUDDRAFN_OPENACC (YDCST, YDTHF, YDML_PHY_SLIN%YREPHLI, YDML_PHY_EC%YRECUMF, JL, JL, KLON&
      &, KLEV, LLDDRAF(LBOUND (LLDDRAF, 1), JBLK), ZTENH(LBOUND (ZTENH, 1), LBOUND (ZTENH, 2), JBLK&
      &), ZQENH(LBOUND (ZQENH, 1), LBOUND (ZQENH, 2), JBLK), PQSEN(LBOUND (PQSEN, 1), LBOUND (PQSEN&
      &, 2), JBLK), PGEO(LBOUND (PGEO, 1), LBOUND (PGEO, 2), JBLK), PGEOH(LBOUND (PGEOH, 1), LBOUND&
      & (PGEOH, 2), JBLK), PAPH(LBOUND (PAPH, 1), LBOUND (PAPH, 2), JBLK), ZRFL(LBOUND (ZRFL, 1), JBLK&
      &), ZTD(LBOUND (ZTD, 1), LBOUND (ZTD, 2), JBLK), ZQD(LBOUND (ZQD, 1), LBOUND (ZQD, 2), JBLK),&
      & PMFU(LBOUND (PMFU, 1), LBOUND (PMFU, 2), JBLK), PMFD(LBOUND (PMFD, 1), LBOUND (PMFD, 2), JBLK&
      &), ZMFDS(LBOUND (ZMFDS, 1), LBOUND (ZMFDS, 2), JBLK), ZMFDQ(LBOUND (ZMFDQ, 1), LBOUND (ZMFDQ&
      &, 2), JBLK), ZDMFDP(LBOUND (ZDMFDP, 1), LBOUND (ZDMFDP, 2), JBLK), ZDMFDE(LBOUND (ZDMFDE, 1)&
      &, LBOUND (ZDMFDE, 2), JBLK), PMFDDE_RATE(LBOUND (PMFDDE_RATE, 1), LBOUND (PMFDDE_RATE, 2), JBLK&
      &), ZKINED(LBOUND (ZKINED, 1), LBOUND (ZKINED, 2), JBLK), YDSTACK=YLSTACK)
    ENDIF
...
DO JBLK = 1, KGPBLKS
  ...
  !$ACC LOOP VECTOR &
  !$ACC&PRIVATE (JK, JL, LLO1, LLO3, ZDZ) 
...
DO JBLK = 1, KGPBLKS
  ...
  !$ACC LOOP VECTOR &
  !$ACC&PRIVATE (IK, IKB, IKD, JL, LLO1, ZDUTEN, ZDZ, ZIND, ZTAURES) 
...
DO JBLK = 1, KGPBLKS
  ...
  !$ACC LOOP VECTOR &
  !$ACC&PRIVATE (IKB, JK, JL, ZDZ, ZMFMAX) 
...
DO JBLK = 1, KGPBLKS
  ...
  DO JL = KIDIA, MERGE (KLON, KFDIA, JBLK < KGPBLKS)
    ...
    CALL CUFLXN_OPENACC (YDTHF, YDCST, YDML_PHY_SLIN%YREPHLI, YDML_PHY_EC%YRECUMF, JL, JL, KLON, KLEV, PTSPHY&
    &, PTEN(LBOUND (PTEN, 1), LBOUND (PTEN, 2), JBLK), PQEN(LBOUND (PQEN, 1), LBOUND (PQEN, 2), JBLK), PQSEN&
    &(LBOUND (PQSEN, 1), LBOUND (PQSEN, 2), JBLK), ZTENH(LBOUND (ZTENH, 1), LBOUND (ZTENH, 2), JBLK), ZQENH&
    &(LBOUND (ZQENH, 1), LBOUND (ZQENH, 2), JBLK), PAPH(LBOUND (PAPH, 1), LBOUND (PAPH, 2), JBLK), PAP(LBOUND&
    & (PAP, 1), LBOUND (PAP, 2), JBLK), PGEOH(LBOUND (PGEOH, 1), LBOUND (PGEOH, 2), JBLK), LDLAND(LBOUND&
    & (LDLAND, 1), JBLK), LDCUM(LBOUND (LDCUM, 1), JBLK), LDTDKMF, KCBOT(LBOUND (KCBOT, 1), JBLK), KCTOP&
    &(LBOUND (KCTOP, 1), JBLK), IDTOP(LBOUND (IDTOP, 1), JBLK), ITOPM2, KTYPE(LBOUND (KTYPE, 1), JBLK),&
    & LLDDRAF(LBOUND (LLDDRAF, 1), JBLK), PMFU(LBOUND (PMFU, 1), LBOUND (PMFU, 2), JBLK), PMFD(LBOUND (PMFD&
    &, 1), LBOUND (PMFD, 2), JBLK), ZMFUS(LBOUND (ZMFUS, 1), LBOUND (ZMFUS, 2), JBLK), ZMFDS(LBOUND (ZMFDS&
    &, 1), LBOUND (ZMFDS, 2), JBLK), ZMFUQ(LBOUND (ZMFUQ, 1), LBOUND (ZMFUQ, 2), JBLK), ZMFDQ(LBOUND (ZMFDQ&
    &, 1), LBOUND (ZMFDQ, 2), JBLK), ZMFUL(LBOUND (ZMFUL, 1), LBOUND (ZMFUL, 2), JBLK), PLUDE(LBOUND (PLUDE&
    &, 1), LBOUND (PLUDE, 2), JBLK), PLUDELI(LBOUND (PLUDELI, 1), LBOUND (PLUDELI, 2), LBOUND (PLUDELI,&
    & 3), JBLK), PLRAIN(LBOUND (PLRAIN, 1), LBOUND (PLRAIN, 2), JBLK), PSNDE(LBOUND (PSNDE, 1), LBOUND &
    &(PSNDE, 2), LBOUND (PSNDE, 3), JBLK), ZDMFUP(LBOUND (ZDMFUP, 1), LBOUND (ZDMFUP, 2), JBLK), ZDMFDP&
    &(LBOUND (ZDMFDP, 1), LBOUND (ZDMFDP, 2), JBLK), ZDPMEL(LBOUND (ZDPMEL, 1), LBOUND (ZDPMEL, 2), JBLK&
    &), PLGLAC(LBOUND (PLGLAC, 1), LBOUND (PLGLAC, 2), JBLK), PMFLXR(LBOUND (PMFLXR, 1), LBOUND (PMFLXR&
    &, 2), JBLK), PMFLXS(LBOUND (PMFLXS, 1), LBOUND (PMFLXS, 2), JBLK), PRAIN(LBOUND (PRAIN, 1), JBLK),&
    & PMFUDE_RATE(LBOUND (PMFUDE_RATE, 1), LBOUND (PMFUDE_RATE, 2), JBLK), PMFDDE_RATE(LBOUND (PMFDDE_RATE&
    &, 1), LBOUND (PMFDDE_RATE, 2), JBLK), YDSTACK=YLSTACK)
  ENDDO
...
DO JBLK = 1, KGPBLKS
  ...
  !$ACC LOOP VECTOR &
  !$ACC&PRIVATE (JK, JL, ZDZ, ZFAC, ZMFA) 
...
DO JBLK = 1, KGPBLKS
  ...
  DO JL = KIDIA, MERGE (KLON, KFDIA, JBLK < KGPBLKS)
    ...
    &, YDML_PHY_EC%YREPHY, JL, JL, KLON, KLEV, ITOPM2, KTYPE(LBOUND (KTYPE, 1), JBLK), KCTOP(LBOUND (KCTOP&
    &, 1), JBLK), KCBOT(LBOUND (KCBOT, 1), JBLK), IDTOP(LBOUND (IDTOP, 1), JBLK), LDTDKMF, LDCUM(LBOUND&
    & (LDCUM, 1), JBLK), LLDDRAF(LBOUND (LLDDRAF, 1), JBLK), LLSCVFLAG(LBOUND (LLSCVFLAG, 1), JBLK), PTSPHY&
    &, PAPH(LBOUND (PAPH, 1), LBOUND (PAPH, 2), JBLK), PAP(LBOUND (PAP, 1), LBOUND (PAP, 2), JBLK), PGEOH&
    &(LBOUND (PGEOH, 1), LBOUND (PGEOH, 2), JBLK), PGEO(LBOUND (PGEO, 1), LBOUND (PGEO, 2), JBLK), PTEN&
    &(LBOUND (PTEN, 1), LBOUND (PTEN, 2), JBLK), ZTENH(LBOUND (ZTENH, 1), LBOUND (ZTENH, 2), JBLK), PQEN&
    &(LBOUND (PQEN, 1), LBOUND (PQEN, 2), JBLK), ZQENH(LBOUND (ZQENH, 1), LBOUND (ZQENH, 2), JBLK), PQSEN&
    &(LBOUND (PQSEN, 1), LBOUND (PQSEN, 2), JBLK), PLGLAC(LBOUND (PLGLAC, 1), LBOUND (PLGLAC, 2), JBLK&
    &), PLUDE(LBOUND (PLUDE, 1), LBOUND (PLUDE, 2), JBLK), PLUDELI(LBOUND (PLUDELI, 1), LBOUND (PLUDELI&
    &, 2), LBOUND (PLUDELI, 3), JBLK), PSNDE(LBOUND (PSNDE, 1), LBOUND (PSNDE, 2), LBOUND (PSNDE, 3), JBLK&
    &), PMFU(LBOUND (PMFU, 1), LBOUND (PMFU, 2), JBLK), PMFD(LBOUND (PMFD, 1), LBOUND (PMFD, 2), JBLK)&
    &, ZMFUS(LBOUND (ZMFUS, 1), LBOUND (ZMFUS, 2), JBLK), ZMFDS(LBOUND (ZMFDS, 1), LBOUND (ZMFDS, 2), JBLK&
    &), ZMFUQ(LBOUND (ZMFUQ, 1), LBOUND (ZMFUQ, 2), JBLK), ZMFDQ(LBOUND (ZMFDQ, 1), LBOUND (ZMFDQ, 2),&
    & JBLK), ZMFUL(LBOUND (ZMFUL, 1), LBOUND (ZMFUL, 2), JBLK), ZDMFUP(LBOUND (ZDMFUP, 1), LBOUND (ZDMFUP&
    &, 2), JBLK), ZDPMEL(LBOUND (ZDPMEL, 1), LBOUND (ZDPMEL, 2), JBLK), PMFLXR(LBOUND (PMFLXR, 1), LBOUND&
    & (PMFLXR, 2), JBLK), PMFLXS(LBOUND (PMFLXS, 1), LBOUND (PMFLXS, 2), JBLK), PTENT(LBOUND (PTENT, 1&
    &), LBOUND (PTENT, 2), JBLK), PTENQ(LBOUND (PTENQ, 1), LBOUND (PTENQ, 2), JBLK), PENTH(LBOUND (PENTH&
    &, 1), LBOUND (PENTH, 2), JBLK), YDSTACK=YLSTACK)
  ENDDO
...
DO JBLK = 1, KGPBLKS
  ...
  DO JL = KIDIA, MERGE (KLON, KFDIA, JBLK < KGPBLKS)
    ...
    IF (YDML_PHY_EC%YRECUMF%LMFDUDV) THEN
      ...
      &, KTYPE(LBOUND (KTYPE, 1), JBLK), KCBOT(LBOUND (KCBOT, 1), JBLK), KCTOP(LBOUND (KCTOP, 1), JBLK), LDCUM&
      &(LBOUND (LDCUM, 1), JBLK), PTSPHY, PAPH(LBOUND (PAPH, 1), LBOUND (PAPH, 2), JBLK), PAP(LBOUND (PAP,&
      & 1), LBOUND (PAP, 2), JBLK), PUEN(LBOUND (PUEN, 1), LBOUND (PUEN, 2), JBLK), PVEN(LBOUND (PVEN, 1),&
      & LBOUND (PVEN, 2), JBLK), ZMFUUS(LBOUND (ZMFUUS, 1), LBOUND (ZMFUUS, 2), JBLK), ZMFDUS(LBOUND (ZMFDUS&
      &, 1), LBOUND (ZMFDUS, 2), JBLK), PMFU(LBOUND (PMFU, 1), LBOUND (PMFU, 2), JBLK), PMFD(LBOUND (PMFD,&
      & 1), LBOUND (PMFD, 2), JBLK), ZUU(LBOUND (ZUU, 1), LBOUND (ZUU, 2), JBLK), ZUD(LBOUND (ZUD, 1), LBOUND&
      & (ZUD, 2), JBLK), ZVU(LBOUND (ZVU, 1), LBOUND (ZVU, 2), JBLK), ZVD(LBOUND (ZVD, 1), LBOUND (ZVD, 2)&
      &, JBLK), PGP2DSPP(LBOUND (PGP2DSPP, 1), LBOUND (PGP2DSPP, 2), JBLK), PTENU(LBOUND (PTENU, 1), LBOUND&
      & (PTENU, 2), JBLK), PTENV(LBOUND (PTENV, 1), LBOUND (PTENV, 2), JBLK), YDSTACK=YLSTACK)
  ...
  DO JBLK = 1, KGPBLKS
    ...
    DO JL = KIDIA, MERGE (KLON, KFDIA, JBLK < KGPBLKS)
      ...
      IF (YDML_PHY_EC%YREPHY%LMFSCAV) THEN
        CALL CUCTRACER_OPENACC (YDCST, YDML_PHY_SLIN%YRCUMFS, YDML_PHY_SLIN%YRECUMF2, YDML_PHY_EC%YRECUMF, JL&
        &, JL, KLON, KLEV, KTRAC, KCTOP(LBOUND (KCTOP, 1), JBLK), IDTOP(LBOUND (IDTOP, 1), JBLK), KTYPE(LBOUND&
        & (KTYPE, 1), JBLK), LLDCUM(LBOUND (LLDCUM, 1), JBLK), LLDDRAF3(LBOUND (LLDDRAF3, 1), JBLK), PTSPHY,&
        & PAPH(LBOUND (PAPH, 1), LBOUND (PAPH, 2), JBLK), PAP(LBOUND (PAP, 1), LBOUND (PAP, 2), JBLK), ZMFUUS&
        &(LBOUND (ZMFUUS, 1), LBOUND (ZMFUUS, 2), JBLK), ZMFDUS(LBOUND (ZMFDUS, 1), LBOUND (ZMFDUS, 2), JBLK&
        &), PMFU(LBOUND (PMFU, 1), LBOUND (PMFU, 2), JBLK), PMFD(LBOUND (PMFD, 1), LBOUND (PMFD, 2), JBLK), ZMFUDR&
        &(LBOUND (ZMFUDR, 1), LBOUND (ZMFUDR, 2), JBLK), ZMFDDR(LBOUND (ZMFDDR, 1), LBOUND (ZMFDDR, 2), JBLK&
        &), ZDMFUPC(LBOUND (ZDMFUPC, 1), LBOUND (ZDMFUPC, 2), JBLK), ZDMFDPC(LBOUND (ZDMFDPC, 1), LBOUND (ZDMFDPC&
        &, 2), JBLK), PCEN(LBOUND (PCEN, 1), LBOUND (PCEN, 2), LBOUND (PCEN, 3), JBLK), PTENC(LBOUND (PTENC,&
        & 1), LBOUND (PTENC, 2), LBOUND (PTENC, 3), JBLK), PSCAV, YDSTACK=YLSTACK)
      ELSE
        ...
        &), ZDMFUPC(LBOUND (ZDMFUPC, 1), LBOUND (ZDMFUPC, 2), JBLK), ZDMFDPC(LBOUND (ZDMFDPC, 1), LBOUND (ZDMFDPC&
        &, 2), JBLK), PCEN(LBOUND (PCEN, 1), LBOUND (PCEN, 2), LBOUND (PCEN, 3), JBLK), PTENC(LBOUND (PTENC,&
        & 1), LBOUND (PTENC, 2), LBOUND (PTENC, 3), JBLK), PSCAV0, YDSTACK=YLSTACK)
      ENDIF
...
DO JBLK = 1, KGPBLKS
  ...
  !$ACC LOOP VECTOR &
  !$ACC&PRIVATE (IK, IKD, JK, JL, ZDUTEN, ZDVTEN, ZRO) 
  ...
  DO JL = KIDIA, MERGE (KLON, KFDIA, JBLK < KGPBLKS)
    ...
    DO JK=1, KLEV
      ...
      IF (LDCUM (JL, JBLK).AND.JK>=KCTOP (JL, JBLK)-1) THEN
        ...
        ZDUTEN=PTENU (JL, JK, JBLK)-ZTENU (JL, JK, JBLK)
        ZDVTEN=PTENV (JL, JK, JBLK)-ZTENV (JL, JK, JBLK)
        PDISS (JL, JK, JBLK)=ABS (PUEN (JL, JK, JBLK)*ZDUTEN+PVEN (JL, JK, JBLK)*ZDVTEN)**0.3333_JPRB
