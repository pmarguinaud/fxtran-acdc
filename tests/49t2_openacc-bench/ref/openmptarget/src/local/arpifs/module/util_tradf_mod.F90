...
LEMTD = ALLOCATED (SELF%EMTD)
IF (LEMTD) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%EMTD)
  !$OMP TARGET UPDATE TO (SELF%EMTD)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%EMTD)
ENDIF

LTRSW = ALLOCATED (SELF%TRSW)
IF (LTRSW) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%TRSW)
  !$OMP TARGET UPDATE TO (SELF%TRSW)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%TRSW)
ENDIF

LEMTC = ALLOCATED (SELF%EMTC)
IF (LEMTC) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%EMTC)
  !$OMP TARGET UPDATE TO (SELF%EMTC)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%EMTC)
ENDIF

LTRSC = ALLOCATED (SELF%TRSC)
IF (LTRSC) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%TRSC)
  !$OMP TARGET UPDATE TO (SELF%TRSC)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%TRSC)
ENDIF

LEMTU = ALLOCATED (SELF%EMTU)
IF (LEMTU) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%EMTU)
  !$OMP TARGET UPDATE TO (SELF%EMTU)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%EMTU)
ENDIF

LTAUAER = ALLOCATED (SELF%TAUAER)
IF (LTAUAER) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%TAUAER)
  !$OMP TARGET UPDATE TO (SELF%TAUAER)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%TAUAER)
ENDIF

LSRSWD = ALLOCATED (SELF%SRSWD)
IF (LSRSWD) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWD)
  !$OMP TARGET UPDATE TO (SELF%SRSWD)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWD)
ENDIF

LSRLWDC = ALLOCATED (SELF%SRLWDC)
IF (LSRLWDC) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRLWDC)
  !$OMP TARGET UPDATE TO (SELF%SRLWDC)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRLWDC)
ENDIF

LSRLWD = ALLOCATED (SELF%SRLWD)
IF (LSRLWD) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRLWD)
  !$OMP TARGET UPDATE TO (SELF%SRLWD)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRLWD)
ENDIF

LSRSWDC = ALLOCATED (SELF%SRSWDC)
IF (LSRSWDC) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWDC)
  !$OMP TARGET UPDATE TO (SELF%SRSWDC)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWDC)
ENDIF

LSRSWDCS = ALLOCATED (SELF%SRSWDCS)
IF (LSRSWDCS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWDCS)
  !$OMP TARGET UPDATE TO (SELF%SRSWDCS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWDCS)
ENDIF

LSRLWDCS = ALLOCATED (SELF%SRLWDCS)
IF (LSRLWDCS) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRLWDCS)
  !$OMP TARGET UPDATE TO (SELF%SRLWDCS)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRLWDCS)
ENDIF

LSRSWDV = ALLOCATED (SELF%SRSWDV)
IF (LSRSWDV) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWDV)
  !$OMP TARGET UPDATE TO (SELF%SRSWDV)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWDV)
ENDIF

LSRSWDUV = ALLOCATED (SELF%SRSWDUV)
IF (LSRSWDUV) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWDUV)
  !$OMP TARGET UPDATE TO (SELF%SRSWDUV)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWDUV)
ENDIF

LEDRO = ALLOCATED (SELF%EDRO)
IF (LEDRO) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%EDRO)
  !$OMP TARGET UPDATE TO (SELF%EDRO)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%EDRO)
ENDIF

LSRSWPAR = ALLOCATED (SELF%SRSWPAR)
IF (LSRSWPAR) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWPAR)
  !$OMP TARGET UPDATE TO (SELF%SRSWPAR)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWPAR)
ENDIF

LSRSWUVB = ALLOCATED (SELF%SRSWUVB)
IF (LSRSWUVB) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWUVB)
  !$OMP TARGET UPDATE TO (SELF%SRSWUVB)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWUVB)
ENDIF

LSRSWPARC = ALLOCATED (SELF%SRSWPARC)
IF (LSRSWPARC) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWPARC)
  !$OMP TARGET UPDATE TO (SELF%SRSWPARC)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWPARC)
ENDIF

LSRSWTINC = ALLOCATED (SELF%SRSWTINC)
IF (LSRSWTINC) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRSWTINC)
  !$OMP TARGET UPDATE TO (SELF%SRSWTINC)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRSWTINC)
ENDIF

LSRFDIR = ALLOCATED (SELF%SRFDIR)
IF (LSRFDIR) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRFDIR)
  !$OMP TARGET UPDATE TO (SELF%SRFDIR)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRFDIR)
ENDIF

LSRCDIR = ALLOCATED (SELF%SRCDIR)
IF (LSRCDIR) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%SRCDIR)
  !$OMP TARGET UPDATE TO (SELF%SRCDIR)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%SRCDIR)
ENDIF

LRMOON = ALLOCATED (SELF%RMOON)
IF (LRMOON) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%RMOON)
  !$OMP TARGET UPDATE TO (SELF%RMOON)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%RMOON)
ENDIF

LDERIVATIVELW = ASSOCIATED (SELF%DERIVATIVELW)
IF (LDERIVATIVELW) THEN
  !$OMP TARGET ENTER DATA MAP (ALLOC: SELF%DERIVATIVELW)
  !$OMP TARGET UPDATE TO (SELF%DERIVATIVELW)
  !$OMP TARGET ENTER DATA MAP (TO: SELF%DERIVATIVELW)
ENDIF
...
LEMTD = ALLOCATED (SELF%EMTD)
IF (LEMTD) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%EMTD)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%EMTD)
ENDIF

LTRSW = ALLOCATED (SELF%TRSW)
IF (LTRSW) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%TRSW)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%TRSW)
ENDIF

LEMTC = ALLOCATED (SELF%EMTC)
IF (LEMTC) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%EMTC)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%EMTC)
ENDIF

LTRSC = ALLOCATED (SELF%TRSC)
IF (LTRSC) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%TRSC)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%TRSC)
ENDIF

LEMTU = ALLOCATED (SELF%EMTU)
IF (LEMTU) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%EMTU)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%EMTU)
ENDIF

LTAUAER = ALLOCATED (SELF%TAUAER)
IF (LTAUAER) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%TAUAER)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%TAUAER)
ENDIF

LSRSWD = ALLOCATED (SELF%SRSWD)
IF (LSRSWD) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWD)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWD)
ENDIF

LSRLWDC = ALLOCATED (SELF%SRLWDC)
IF (LSRLWDC) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRLWDC)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRLWDC)
ENDIF

LSRLWD = ALLOCATED (SELF%SRLWD)
IF (LSRLWD) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRLWD)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRLWD)
ENDIF

LSRSWDC = ALLOCATED (SELF%SRSWDC)
IF (LSRSWDC) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWDC)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWDC)
ENDIF

LSRSWDCS = ALLOCATED (SELF%SRSWDCS)
IF (LSRSWDCS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWDCS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWDCS)
ENDIF

LSRLWDCS = ALLOCATED (SELF%SRLWDCS)
IF (LSRLWDCS) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRLWDCS)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRLWDCS)
ENDIF

LSRSWDV = ALLOCATED (SELF%SRSWDV)
IF (LSRSWDV) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWDV)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWDV)
ENDIF

LSRSWDUV = ALLOCATED (SELF%SRSWDUV)
IF (LSRSWDUV) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWDUV)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWDUV)
ENDIF

LEDRO = ALLOCATED (SELF%EDRO)
IF (LEDRO) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%EDRO)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%EDRO)
ENDIF

LSRSWPAR = ALLOCATED (SELF%SRSWPAR)
IF (LSRSWPAR) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWPAR)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWPAR)
ENDIF

LSRSWUVB = ALLOCATED (SELF%SRSWUVB)
IF (LSRSWUVB) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWUVB)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWUVB)
ENDIF

LSRSWPARC = ALLOCATED (SELF%SRSWPARC)
IF (LSRSWPARC) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWPARC)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWPARC)
ENDIF

LSRSWTINC = ALLOCATED (SELF%SRSWTINC)
IF (LSRSWTINC) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRSWTINC)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRSWTINC)
ENDIF

LSRFDIR = ALLOCATED (SELF%SRFDIR)
IF (LSRFDIR) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRFDIR)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRFDIR)
ENDIF

LSRCDIR = ALLOCATED (SELF%SRCDIR)
IF (LSRCDIR) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%SRCDIR)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%SRCDIR)
ENDIF

LRMOON = ALLOCATED (SELF%RMOON)
IF (LRMOON) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%RMOON)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%RMOON)
ENDIF

LDERIVATIVELW = ASSOCIATED (SELF%DERIVATIVELW)
IF (LDERIVATIVELW) THEN
  !$OMP TARGET EXIT DATA MAP (RELEASE: SELF%DERIVATIVELW)
  !$OMP TARGET EXIT DATA MAP (DELETE: SELF%DERIVATIVELW)
ENDIF
