#!/usr/bin/perl -w

#
# Copyright 2025 Meteo-France
# All rights reserved
# philippe.marguinaud@meteo.fr
#

use strict;

use Data::Dumper;
use Getopt::Long;
use File::Path;
use File::Spec;
use File::Basename;
use FileHandle;

use FindBin qw ($Bin);
use lib "$Bin/../lib";
use local::lib;

use fxtran;
use fxtran::xpath;

use Fxtran;
use Fxtran::Canonic;
use Fxtran::Style;
use Fxtran::Bt;
use Fxtran::Finder;
use Fxtran::Util;

my %intf;

sub findActualArgumentIntent
{
  my ($p, %opts) = @_;

  my $find = $opts{find};

  my $stmt = &Fxtran::stmt ($p);

  my ($name, $rank) = &F ('./arg-N', $p, 1);

  unless ($name)
    {
      my @arg = &F ('./arg-spec/arg', $stmt);

      for ($rank = 0; $rank < scalar (@arg); $rank++)
        {
          last if ($arg[$rank]->unique_key eq $p->unique_key);
        }

      die if ($rank == scalar (@arg));
    }
  
  my ($proc) = &F ('./procedure-designator', $stmt, 1);

  unless ($intf{$proc})
    {
      my $intf = $find->getInterface (name => $proc);
      
      die unless ($intf);
      
      $intf = &Fxtran::Util::slurp ($intf);
      
      ($intf) = &parse (fragment => $intf, fopts => [qw (-construct-tag -no-cpp -line-length 500 -canonic)]);
      
      ($intf) = &F ('.//program-unit[./subroutine-stmt[string(./subroutine-N)="?"]]', $proc, $intf);
      
      &Fxtran::Canonic::makeCanonic ($intf);

      $intf{$proc} = $intf;
    }

  unless ($name)
    {
      my @arg = &F ('./subroutine-stmt/dummy-arg-LT/arg-N', $intf{$proc});
      $name = $arg[$rank]->textContent;
    }

  my ($dp) = &F ('./specification-part/declaration-part', $intf{$proc});

  my ($intent) = &F ('./T-decl-stmt[./EN-decl-LT/EN-decl[string(./EN-N)="?"]]/attribute/intent-spec', $name, $dp, 1);

  return $intent;
}

my @I =
qw (
/tmp/IAL-bundle/build.GPUCC80_O1_NVHPC25.9_CUDA12.9_HPCX2.24/.fxtran/generated
/tmp/IAL-bundle/build.GPUCC80_O1_NVHPC25.9_CUDA12.9_HPCX2.24/ial_dp/arpifs/interfaces/include
/tmp/IAL-bundle/build.GPUCC80_O1_NVHPC25.9_CUDA12.9_HPCX2.24/ial_dp/generated_interfaces
/tmp/IAL-bundle/build.GPUCC80_O1_NVHPC25.9_CUDA12.9_HPCX2.24/ial_dp/satrad/interfaces/include
);

local $ENV{CMAKE_BUILD_DIRECTORY} = '/tmp/IAL-bundle/build.GPUCC80_O1_NVHPC25.9_CUDA12.9_HPCX2.24';

my $find = 'Fxtran::Finder'->new (I => \@I);

my $F90 = shift;

my $d = &parse (location => $F90, fopts => [qw (-construct-tag -no-cpp -line-length 500 -canonic)]);

&Fxtran::Canonic::makeCanonic ($d);

my @pu = &F ('.//program-unit', $d);

my ($pu) = @pu;

my ($dp) = &F ('./specification-part/declaration-part', $pu);
my ($ep) = &F ('./execution-part', $pu);

my @arg = &F ('./T-decl-stmt[./_T-spec_/derived-T-spec][./attribute[string(./intent-spec)="INOUT"]]/EN-decl-LT/EN-decl/EN-N', $dp, 1);
my %arg = map { ($_, 1) } @arg;

my %N;

for my $expr (&F ('.//named-E', $ep))
  {
    my ($N) = &F ('./N', $expr, 1);

    next unless ($arg{$N});

    my $p = $expr->parentNode;

    my $do = 0;

    if ($p->nodeName eq 'E-1')
      {
        $do = 1;
      }
    elsif ($p->nodeName eq 'arg')
      {
        my $intent = &findActualArgumentIntent ($p, find => $find);
        $do = $intent ne 'IN';
      }

    my $stmt = &Fxtran::stmt ($p);

    if ($do)
      {
        $stmt->parentNode->insertAfter ($_, $stmt) for (&s ("LLDO_$N = .TRUE."), &t ("\n"));
        $N{$N}++;
      }
   
  }


$dp->appendChild (&t ("\n"));
$ep->insertBefore (&t ("\n"), $ep->firstChild);

for my $N (sort keys (%N))
  {
    $dp->appendChild ($_) for (&t ("\n"), &s ("LOGICAL :: LLDO_$N"));
    $ep->insertBefore ($_, $ep->firstChild) for (&t ("\n"), &s ("LLDO_$N = .FALSE."));

    my ($if) = &parse (fragment => << "EOF");
IF (LLDO_$N) THEN
CALL WIPE ($N)
CALL COPY ($N)
ENDIF
EOF

    $ep->appendChild ($_) for (&t ("\n"), $if);

  }

$dp->appendChild (&t ("\n"));


print $d->textContent;
