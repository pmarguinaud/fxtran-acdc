...
IF (LPARALLELMETHOD ('OPENMP','CUASCN_PARALLEL:0')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CUASCN_PARALLEL:0')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CUASCN_PARALLEL:0:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&PRESENT (KCBOT, KTYPE, LDCUM, LLKLAB, LSCVFLAG, PMFUB, PQU, PWMEAN, YSTACK, ZDPMEAN, &
    !$ACC&         ZLUOLD, ZOENTR) &
    !$ACC&PRIVATE (JBLK) &
...
IF (LPARALLELMETHOD ('OPENMP','CUASCN_PARALLEL:1')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CUASCN_PARALLEL:1')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CUASCN_PARALLEL:1:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         PMFUL, PMFUQ, PMFUS, PQU, PTU, PWU, PWUBASE, YSTACK, ZBUO) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
IF (LPARALLELMETHOD ('OPENMP','CUASCN_PARALLEL:2')) THEN
  ...
  DO JBLK = YDCPG_OPTS%JBLKMIN, YDCPG_OPTS%JBLKMAX
    ...
    DO JK=KLEV-1, 3,-1
      ...
      DO JL_CUBASMCN=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA

        IF (.NOT.LDCUM (JL_CUBASMCN, JBLK).AND.KLAB (JL_CUBASMCN, IK+1, JBLK)==0) THEN

          IF (YDECUMF%LMFMID.AND.IK>YDECUMF%NJKT7.AND.PQEN (JL_CUBASMCN&
            &, IK, JBLK)>0.80_JPRB*PQSEN (JL_CUBASMCN, IK, JBLK)) THEN
            PTU (JL_CUBASMCN, IK+1, JBLK)=(YDCST%RCPD*PTEN (JL_CUBASMCN, IK, JBLK)+PGEO &
            &(JL_CUBASMCN, IK, JBLK)-PGEOH (JL_CUBASMCN, IK+1, JBLK))*ZORCPD_CUBASMCN
            PQU (JL_CUBASMCN, IK+1, JBLK)=PQEN (JL_CUBASMCN, IK, JBLK)
            PLU (JL_CUBASMCN, IK+1, JBLK)=0.0_JPRB
            ZZZMB=MAX (1.E-6_JPRB,-PVERVEL (JL_CUBASMCN, IK, JBLK)*ZRG_CUBASMCN)
            ZZZMB=MIN (ZZZMB, YDECUMF%RMFLIA)
            PMFUB (JL_CUBASMCN, JBLK)=ZZZMB
            PMFU (JL_CUBASMCN, IK+1, JBLK)=PMFUB (JL_CUBASMCN, JBLK)
            PMFUS (JL_CUBASMCN, IK+1, JBLK)=PMFUB (JL_CUBASMCN, JBLK)*(YDCST%RCPD&
            &*PTU (JL_CUBASMCN, IK+1, JBLK)+PGEOH (JL_CUBASMCN, IK+1, JBLK))
            PMFUQ (JL_CUBASMCN, IK+1, JBLK)=PMFUB (JL_CUBASMCN, JBLK)*PQU (JL_CUBASMCN, IK+1, JBLK)
            PMFUL (JL_CUBASMCN, IK+1, JBLK)=0.0_JPRB
            PDMFUP (JL_CUBASMCN, IK+1, JBLK)=0.0_JPRB
            PLRAIN (JL_CUBASMCN, IK+1, JBLK)=0.0_JPRB
            KCBOT (JL_CUBASMCN, JBLK)=IK
            KLAB (JL_CUBASMCN, IK+1, JBLK)=1
            KTYPE (JL_CUBASMCN, JBLK)=3
          ENDIF
      ...
      IF (LLO3) THEN
        ...
        DO JL_CUENTR=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
          ZDMFEN (JL_CUENTR, JBLK)=0.0_JPRB
          ZDMFDE (JL_CUENTR, JBLK)=0.0_JPRB
          ZENTR (JL_CUENTR, JBLK)=0.0
        ENDDO


        DO JL_CUENTR=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA

          IF (LDCUM (JL_CUENTR, JBLK)) THEN
            ZDZ=(PGEOH (JL_CUENTR, IK, JBLK)-PGEOH (JL_CUENTR, IK+1, JBLK))*ZRG_CUENTR
            ZMF=PMFU (JL_CUENTR, IK+1, JBLK)*ZDZ
            LLO1_CUENTR=IK<KCBOT (JL_CUENTR, JBLK)

            IF (LLO1_CUENTR) THEN
              
              ZXDETRPEN=YDECUMF%DETRPEN
              
              ZDMFEN (JL_CUENTR, JBLK)=ZENTR (JL_CUENTR, JBLK)*ZMF
              ZDMFDE (JL_CUENTR, JBLK)=ZXDETRPEN*ZMF
            ENDIF
      ...
      IF (LLO3) THEN
        ...
        IF (YDECUMF%LSCVLIQ) THEN
          ...
          IF (.NOT.YDEPHLI%LPHYLIN) THEN
            ...
            LLFLAG_CUADJTQ000 (YLCPG_BNDS%KIDIA:YLCPG_BNDS%KFDIA, JBLK&
            &)=LLFLAG (YLCPG_BNDS%KIDIA:YLCPG_BNDS%KFDIA, JBLK)
            
            
            LLFLAG_CUADJTQ000 (YLCPG_BNDS%KIDIA:YLCPG_BNDS%KFDIA, JBLK)=LLFLAG_CUADJTQ000 (YLCPG_BNDS%KIDIA&
            &:YLCPG_BNDS%KFDIA, JBLK).AND..NOT.LSCVFLAG (YLCPG_BNDS%KIDIA:YLCPG_BNDS%KFDIA, JBLK)

            

            

            DO JL=YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA
        ...
        ELSE
          ...
          IF (.NOT.YDEPHLI%LPHYLIN) THEN
            ...
            LLFLAG_CUADJTQ001 (YLCPG_BNDS%KIDIA:YLCPG_BNDS%KFDIA, JBLK&
            &)=LLFLAG (YLCPG_BNDS%KIDIA:YLCPG_BNDS%KFDIA, JBLK)
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CUASCN_PARALLEL:2')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CUASCN_PARALLEL:2:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         PTEN, PTENH, PTU, PVERVEL, PWMEAN, PWU, YSTACK, ZBUO, ZDMFDE, ZDMFEN, ZDPMEAN, &
    !$ACC&         ZENTR, ZLUOLD, ZOENTR, ZPH, ZPRECIP, ZQOLD) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
    ...
    !$ACC LOOP VECTOR &
    !$ACC&PRIVATE (IK, IKB, IS, JK, JL, JLON, JL_CUBASMCN, JL_CUENTR, LLO1_CUENTR, LLO3, LLPERT_DETRPEN, &
    !$ACC&         YLCPG_BNDS, Z1S, Z2S, ZALFAW, ZBC, ZBE, ZBUOC, ZC, ZCBF, ZCHANGE, ZCOND, &
    !$ACC&         ZCOND1, ZCOR, ZD, ZDFI, ZDKBUO, ZDKEN, ZDT, ZDZ, ZF, ZFAC, ZFOEEWI, ZFOEEWL, &
    !$ACC&         ZI, ZINT, ZKEDKE, ZL, ZLCRIT, ZLEEN, ZLNEW, ZMF, ZMFMAX, ZMFTEST, ZMFULK, &
    !$ACC&         ZMFUN, ZMFUQK, ZMFUSK, ZOCUDET, ZOEALFA, ZOEALFAP, ZOEALFA_CUADJTQ001, &
    !$ACC&         ZORCPD_CUBASMCN, ZPRCON, ZQEEN, ZQMAX, ZQP, ZQSAT, ZQUDE, ZRG_CUBASMCN, &
    !$ACC&         ZRG_CUENTR, ZRNEW, ZROLD, ZSCDE, ZSEEN, ZTARG, ZVI, ZVV, ZVW, ZXDETRPEN, &
    !$ACC&         ZXE, ZXENTRORG, ZXENTSHALP, ZXPRCDGW, ZXS, ZZCO, ZZZMB) 
...
IF (LPARALLELMETHOD ('OPENMP','CUASCN_PARALLEL:3')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CUASCN_PARALLEL:3')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CUASCN_PARALLEL:3:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC PARALLEL LOOP GANG &
    !$ACC&PRESENT (KCBOT, KCTOP, LDCUM, PWMEAN, YSTACK, ZDPMEAN) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
