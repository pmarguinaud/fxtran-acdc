...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LSOLV.AND.(.NOT.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE)) THEN
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG')) THEN
    ...
    CALL SGET_HOST_DATA_RDONLY (PGWDCS, YD_PGWDCS)
    ...
    CALL SGET_HOST_DATA_RDONLY (PWWILT, YD_PWWILT)
    ...
    CALL SGET_HOST_DATA_RDONLY (Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN, YDMF_PHYS_SURF%GSD_VV%F_RSMIN)
    ...
    IF (FXTRAN_ACDC_LSYNCHOST ('APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (PGWDCS, YD_PGWDCS)
      CALL SGET_HOST_DATA_RDWR (PHQ, YD_PHQ)
      CALL SGET_HOST_DATA_RDWR (PHTR, YD_PHTR)
      CALL SGET_HOST_DATA_RDWR (PHU, YD_PHU)
      ...
      CALL SGET_HOST_DATA_RDWR (PWLMX, YD_PWLMX)
      CALL SGET_HOST_DATA_RDWR (PWWILT, YD_PWWILT)
      ...
      CALL SGET_HOST_DATA_RDWR (Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN, YDMF_PHYS_SURF%GSD_VV%F_RSMIN)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN&
    ...
    CALL SGET_DEVICE_DATA_RDONLY (PGWDCS, YD_PGWDCS)
    CALL SGET_DEVICE_DATA_RDWR (PHQ, YD_PHQ)
    CALL SGET_DEVICE_DATA_RDWR (PHTR, YD_PHTR)
    CALL SGET_DEVICE_DATA_RDWR (PHU, YD_PHU)
    CALL SGET_DEVICE_DATA_RDONLY (PQV, YD_PQV)
    CALL SGET_DEVICE_DATA_RDONLY (PWFC, YD_PWFC)
    CALL SGET_DEVICE_DATA_RDWR (PWLMX, YD_PWLMX)
    CALL SGET_DEVICE_DATA_RDONLY (PWWILT, YD_PWWILT)
    ...
    CALL SGET_DEVICE_DATA_RDWR (Z_YDMF_PHYS_SURF_GSD_VV_PHV, YDMF_PHYS_SURF%GSD_VV%F_HV)
    ...
    CALL SGET_DEVICE_DATA_RDONLY (Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN, YDMF_PHYS_SURF%GSD_VV%F_RSMIN)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:PCHROV, PFLU_NEIJ, PFLU_QSAT, PFLU_QSATS, PFLU_VEG, PGWDCS, PHQ,  & 
    !$OMP &          PHTR, PHU, PQV, PWFC, PWLMX, PWWILT, YDCPG_OPTS, YDMODEL, YFXTRAN_ACDC_STACK,  & 
    !$OMP &          Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_FC,  & 
    !$OMP &          Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q, Z_YDMF_PHYS_OUT_FRSO,  & 
    !$OMP &          Z_YDMF_PHYS_SURF_GSD_VF_PLSM, Z_YDMF_PHYS_SURF_GSD_VV_PD2,  & 
    !$OMP &          Z_YDMF_PHYS_SURF_GSD_VV_PHV, Z_YDMF_PHYS_SURF_GSD_VV_PIVEG,  & 
    !$OMP &          Z_YDMF_PHYS_SURF_GSD_VV_PLAI, Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN)  & 
    !$OMP & PRIVATE (JBLK)  & 
