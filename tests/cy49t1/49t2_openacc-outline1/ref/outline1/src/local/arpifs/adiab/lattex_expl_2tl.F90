  ...
  DO JGFL=1, SIZE (YDCPG_SL1%GFL)
    ...
    IF (YDVARS%GFL_PTR (JGFL)%YCOMP%LADV) THEN
      ...
      IF (YDML_DYN%YRDYN%LSPLTHOIGFL.AND.(YDML_DYN%YRDYN%NSPLTHOI==0&
        ...
        CALL LATTEX_EXPL_2TL_OUTLINE_000 (YDVARS, YDCPG_SL1, JGFL, YDGEOMETRY, YDCPG_BNDS)
      ELSE
        CALL LATTEX_EXPL_2TL_OUTLINE_001 (YDVARS, YDCPG_SL1, JGFL, YDGEOMETRY, YDCPG_BNDS)
      ENDIF
...
ELSEIF (.NOT.LDCT) THEN
  ...
  DO JGFL=1, SIZE (YDCPG_SL1%GFL)
    ...
    IF (YDVARS%GFL_PTR (JGFL)%YCOMP%LADV) THEN
      ...
      IF (YDML_DYN%YRDYN%LSPLTHOIGFL.AND.(YDML_DYN%YRDYN%NSPLTHOI==0&
        &).AND.(.NOT.YDVARS%GFL_PTR (JGFL)%YCOMP%LTDIABLIN)) THEN
        CALL LATTEX_EXPL_2TL_OUTLINE_002 (YDVARS, YDCPG_SL1, JGFL, YDGEOMETRY, YDCPG_BNDS)
      ELSE
        CALL LATTEX_EXPL_2TL_OUTLINE_003 (YDVARS, YDCPG_SL1, JGFL, YDGEOMETRY, YDCPG_BNDS)
      ENDIF
...
IF (YDVARS%CVGQ%YCOMP%LGP.AND.(.NOT.LDCTC)) THEN
  ...
  IF ((YDML_DYN%YRDYN%NSPLTHOI/=0).OR.LLTDIABLIN) THEN
    CALL LATTEX_EXPL_2TL_OUTLINE_004 (YDCPG_SL1, YDGEOMETRY, YDCPG_BNDS)
  ENDIF

  CALL LATTEX_EXPL_2TL_OUTLINE_005 (YDCPG_SL1, YDGEOMETRY, YDCPG_BNDS)
  LLTDIABLIN=YDVARS%CVGQ%YCOMP%LTDIABLIN.AND.YDVARS%Q%YCOMP%LTDIABLIN

  IF ((YDML_DYN%YRDYN%NSPLTHOI==0).AND.LLTDIABLIN) THEN
    CALL LATTEX_EXPL_2TL_OUTLINE_006 (YDCPG_SL1, YDGEOMETRY, YDCPG_BNDS)
  ENDIF
