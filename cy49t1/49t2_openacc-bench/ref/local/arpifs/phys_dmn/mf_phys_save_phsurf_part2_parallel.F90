...
IF (LPARALLELMETHOD ('OPENMP','MF_PHYS_SAVE_PHSURF_PART2_PARALLEL:PART2')) THEN
  ...
  IF (LSYNCHOST ('MF_PHYS_SAVE_PHSURF_PART2_PARALLEL:PART2')) THEN
    IF (LHOOK) CALL DR_HOOK ('MF_PHYS_SAVE_PHSURF_PART2_PARALLEL&
...
ELSEIF (LPARALLELMETHOD ('OPENMPSINGLECOLUMN','MF_PHYS_SAVE_PHSURF_PART2_PARALLEL:PART2')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','MF_PHYS_SAVE_PHSURF_PART2_PARALLEL:PART2')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('MF_PHYS_SAVE_PHSURF_PART2_PARALLEL:PART2:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         PUAL_T0, PUEN_T0, PUNEBH_T0, PUOM_T0, YDCPG_OPTS, YDMODEL, YSTACK) &
    !$ACC&PRIVATE (JBLK) &
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      YLSTACK%U4 = stack_u4 (YSTACK, JBLK-YDCPG_OPTS%JBLKMIN+1, YDCPG_OPTS%KGPBLKS)
        ...
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPRO.OR.YDMODEL%YRML_PHY_MF%YRPHY%LGPCMT) THEN
          CALL ACPY (JLON, PUAL_T0 (:,:, JBLK), PSAV_UDAL (:,:, JBLK))
          CALL ACPY (JLON, PUOM_T0 (:,:, JBLK), PSAV_UDOM (:,:, JBLK))

          IF (YDMODEL%YRML_PHY_MF%YRPHY%LCDDPRO) THEN
            CALL ACPY (JLON, PDAL_T0 (:,:, JBLK), PSAV_DDAL (:,:, JBLK))
            CALL ACPY (JLON, PDOM_T0 (:,:, JBLK), PSAV_DDOM (:,:, JBLK))
          ENDIF
        ...
        IF (YDMODEL%YRML_GCONF%YGFL%YUNEBH%LACTIVE)  THEN
            CALL ACPY (JLON, PUNEBH_T0 (:,:, JBLK), PSAV_UNEBH (:,:, JBLK))
        ENDIF
  

        IF (YDMODEL%YRML_GCONF%YGFL%YUEN%LACTIVE)  THEN
            CALL ACPY (JLON, PUEN_T0 (:,:, JBLK), PSAV_ENTCH (:,:, JBLK))
        ENDIF
...
ENDSUBROUTINE MF_PHYS_SAVE_PHSURF_PART2_PARALLEL
