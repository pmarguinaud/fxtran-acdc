...
ELSEIF (YDMODEL%YRML_PHY_MF%YRPHY%LO3FL .AND. (YDMODEL%YRML_PHY_RAD%YRERAD%NOZOCL == 1) .AND. YDMODEL%YRML_PHY_MF%YRPHY%LRAYFM) THEN
  IF (MOD(YDCPG_OPTS%NSTEP,YDMODEL%YRML_PHY_RAD%YRERAD%NRADFR) == 0) THEN
    CALL RADOZCMF(YDMODEL%YRCST,YDMODEL%YRML_GCONF%YRRIP%YREOZOC, KIDIA, KFDIA, KLON, KFLEVG,&
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, YDVARS%GEOMETRY%GEMU%T0, ZROZ)
    DO JLON = KIDIA, KFDIA
      PQO3(JLON,0)=1.E-9_JPRB
    ENDDO
    DO JLEV=YDCPG_OPTS%KTDIA,KFLEVG
...
ELSE
  CALL SUOZON(KIDIA, KFDIA, KLON, KFLEVG, PQO3, .FALSE.,                                                             &
