...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LASSIE_PARALLEL:SITNU')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('LASSIE_PARALLEL:SITNU:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
  CALL SGET_DEVICE_DATA_RDWR (PGAGT0L, YD_PGAGT0L)
  CALL SGET_DEVICE_DATA_RDWR (PGAGT0M, YD_PGAGT0M)
  CALL SGET_DEVICE_DATA_RDWR (PSDIV0, YD_PSDIV0)
  CALL SGET_DEVICE_DATA_RDWR (PTOD0, YD_PTOD0)
  ...
  !$OMP & MAP(PRESENT:PGAGT0L, PGAGT0M, PSDIV0, PTOD0, YDCPG_OPTS, YDCST, YDDYN, YDGEOMETRY,  & 
  !$OMP &          YFXTRAN_ACDC_STACK, Z_YDVARS_DIV_T0, Z_YDVARS_SP_DL, Z_YDVARS_SP_DM,  & 
  !$OMP &          Z_YDVARS_T_DL, Z_YDVARS_T_DM)  & 
  !$OMP & PRIVATE (JBLK)  & 
...
IF (YDDYNA%LSPRT) THEN
  ...
  IF (YDDYNA%LTWOTL) THEN
    ...
    IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LASSIE_PARALLEL:GPRCP')) THEN
      ...
      CALL SGET_HOST_DATA_RDWR (PTOD0, YD_PTOD0)
      CALL SGET_HOST_DATA_RDONLY (ZR9, YL_ZR9)
      ...
      IF (FXTRAN_ACDC_LSYNCHOST ('LASSIE_PARALLEL:GPRCP')) THEN
        ...
        CALL SGET_HOST_DATA_RDWR (PTOD0, YD_PTOD0)
        CALL SGET_HOST_DATA_RDWR (ZR9, YL_ZR9)
    ...
    ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LASSIE_PARALLEL:GPRCP')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK ('LASSIE_PARALLEL:GPRCP:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
      CALL SGET_DEVICE_DATA_RDWR (PTOD0, YD_PTOD0)
      CALL SGET_DEVICE_DATA_RDONLY (ZR9, YL_ZR9)
      IF (LHOOK) CALL DR_HOOK ('LASSIE_PARALLEL:GPRCP:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK ('LASSIE_PARALLEL:GPRCP:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
      
      !$OMP TARGET TEAMS DISTRIBUTE  & 
      !$OMP & MAP(PRESENT:PTOD0, YDCPG_OPTS, YDCST, YDGEOMETRY, YFXTRAN_ACDC_STACK, ZR9)  & 
      !$OMP & PRIVATE (JBLK)  & 
      !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
