...
IF (LHOOK) CALL DR_HOOK ('LARCINB_LTDIABLIN', 0, ZHOOK_HANDLE)
CALL LAITRE_GFL (YDGEOMETRY, YDML_GCONF%YGFL, YDML_DYN, KASLB1, YDGEOMETRY%YRDIM%NPROMA, YDCPG_BNDS&
&%KIDIA, YDCPG_BNDS%KFDIA, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIMV%NFLSA, YDGEOMETRY%YRDIMV&
&%NFLEN, KHVI, KWK, CLINT, YDVARS%GFL_PTR (JFLD)%YCOMP%WENO_ALPHA, LDMOM, KNOWENO, KL0, PLSCAW&
&, PRSCAW, YDCPG_SL1%GFL (JFLD)%P, ZGFLF, YDCPG_SL1%GFL (JFLD)%P_SPF)
...
IF (LHOOK) CALL DR_HOOK ('LARCINB_LTDIABLIN', 1, ZHOOK_HANDLE)
ENDSUBROUTINE
