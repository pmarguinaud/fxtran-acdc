...
!$ACDC PARALLEL, TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=CPTEND_NEW {
...
CALL CPTEND_NEW(YDMODEL%YRCST, YDMODEL, KLON, KIDIA, KFDIA, KFLEVG,           &
...
& YDMF_PHYS%OUT%FCQGNG, YDMF_PHYS%OUT%FCQNG, YDMF_PHYS%OUT%FRMH, PMSC_FRMQ, PFCHQ, YDMF_PHYS%OUT%FRSO, YDMF_PHYS%OUT%FRTH,&
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%NDPSFI == 1) THEN
  CALL CPMVVPS(YDMODEL%YRCST, YDGEOMETRY%YRVERT_GEOM%YRVAB, KLON, KIDIA, KFDIA,                        &
  & KFLEVG, YDCPG_OPTS%ZDTPHY, ZPFL_FP, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD(:, KFLEVG),               &
  & YDMF_PHYS%OUT%FEVL (:, 1), YDMF_PHYS%OUT%FEVN (:, 1), YDCPG_DYN0%CTY%EVEL, YDCPG_DYN0%CTY%PSDVBC,  &
  & YDMF_PHYS_NEXT_STATE%SP%P)
ENDIF
...
CALL CPUTQY_APLPAR_EXPL(YDMODEL%YRCST, YDCPG_BNDS, YDCPG_OPTS, YDMODEL%YRML_DYN%YRDYNA, YDMF_PHYS_NEXT_STATE, &
