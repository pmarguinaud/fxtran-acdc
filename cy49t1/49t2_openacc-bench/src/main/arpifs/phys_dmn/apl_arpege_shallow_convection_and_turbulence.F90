...
IF(YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF) THEN
   CALL ACVPPKF(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YDCPG_BNDS, YDCPG_OPTS, YDMODEL%YRML_PHY_MF%YRTOPH%NTCVIM,              &
...
IF(YDMODEL%YRML_PHY_MF%YRPHY%LEDKF) THEN
  ...
  CALL ARP_SHALLOW_MF(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPARAR%PHYEX, YDCPG_OPTS, YDCPG_BNDS, YDMODEL%YRML_PHY_MF%YRPHY2%TSPHY,     &
  ...
  & PQV, PQL, PQI, PQR, PQS, YDMF_PHYS_BASE_STATE%TKE, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, &
  & PEDMFQ, PEDMFS, PEDMFU, PEDMFV, YDMF_PHYS_SURF, PPRODTH_CVPP, PQLI_CVPP, PNEB_CVPP, KNLAB_CVPP, PMF_UP)
...
IF (YDMODEL%YRML_PHY_MF%YRPHY%LECTPRODCVP) THEN
  CALL ACTKE1 (YDMODEL%YRCST, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH, YDMODEL%YRML_PHY_MF, KIDIA,              &
  & KFDIA, KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET,                                     &
  & KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD,   &
  & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,    &
  & YDMF_PHYS_BASE_STATE%T, YDMF_PHYS_BASE_STATE%U, YDMF_PHYS_BASE_STATE%V, PQV,                                           &
  ...
  & PKUROV, PXTROV, PXUROV, PNBVNO, PNEBS, PQLIS, PNEBS0, PQLIS0, PCOEFN,                                                  &
  & PUSLE, PKCLS, PECTCLS, PPRODTH)
  PKUROV2(:,:) = PKUROV(:,:)
ELSE
  CALL ACTKE (YDMODEL%YRCST, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH, YDMODEL%YRML_PHY_MF, KIDIA,             &
  & KFDIA, KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET,                                   &
  & KFLEVG, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%PHIF, YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYD, &
  & YDMF_PHYS_BASE_STATE%YCPG_PHY%PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%DELP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%R,  &
...
ENDIF
DO JLON = 1, KIDIA, KFDIA
  YDMF_PHYS%OUT%CLPH(JLON)=MIN(YDMODEL%YRML_PHY_MF%YRPHY0%XMAXLM,MAX(YDMODEL%YRML_PHY_MF%YRPHY0%XMINLM,YDMF_PHYS%OUT%CLPH(JLON)))
ENDDO
