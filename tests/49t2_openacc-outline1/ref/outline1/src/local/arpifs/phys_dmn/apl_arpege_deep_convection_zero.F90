...
IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_DEEP_CONVECTION_ZERO', 0, ZHOOK_HANDLE)

DO JLEV=1, YDCPG_OPTS%KFLEVG

  DO JLON=YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA
    PTENT (JLON, JLEV)=PTENHA (JLON, JLEV)/YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%CP (JLON, JLEV)
    PTENQ (JLON, JLEV)=PTENQVA (JLON, JLEV)
    PTENU (JLON, JLEV)=0.0_JPRB
    PTENV (JLON, JLEV)=0.0_JPRB
    PTENDT (JLON, JLEV)=PRDG_CVGT (JLON, JLEV)
    PTENDQ (JLON, JLEV)=PRDG_CVGQ (JLON, JLEV)
  ENDDO

ENDDO

IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_DEEP_CONVECTION_ZERO', 1, ZHOOK_HANDLE)
ENDSUBROUTINE
