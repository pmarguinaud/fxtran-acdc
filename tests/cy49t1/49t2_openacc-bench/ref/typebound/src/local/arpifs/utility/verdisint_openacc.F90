...
temp (REAL (KIND=JPRD), ZOUTS, (KPROMA))
...
IF (ANY (CDOPER (1:4)==(/'INGW','ITOP','IBOT'/))) THEN
  ...
  IF (.NOT.PRESENT (POUT)) THEN
    ...
    IF (.NOT.PRESENT (POUTS))  THEN
      CALL ABOR1_ACC ("ERROR: NEEDS AT LEAST 1 OF POUT/POUTS")
    ENDIF


    IF (ITYPE==1)  THEN
      CALL ABOR1_ACC ("ERROR POUTS: ITYPE=1 NOT POSSIBLE")
    ENDIF


    IF (ILEVOUT==KFLEV)  THEN
      CALL ABOR1_ACC ("ERROR POUTS: DERIVATIVE NOT POSSIBLE")
    ENDIF
  ...
  ELSEIF (PRESENT (POUTS)) THEN
    ...
    IF (ITYPE==1)  THEN
      CALL ABOR1_ACC ("ERROR KFLEV+1: ITYPE=1 NOT POSSIBLE")
    ENDIF


    IF (PRESENT (PINS))  THEN
      CALL ABOR1_ACC ("ERROR: PINS MEANINGLESS WITH POUTS")
    ENDIF
...
ENDSUBROUTINE VERDISINT_OPENACC
