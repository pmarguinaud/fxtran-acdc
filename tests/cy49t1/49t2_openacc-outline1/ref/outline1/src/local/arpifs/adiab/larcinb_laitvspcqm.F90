SUBROUTINE LARCINB_LAITVSPCQM (LDMOM, YDVARS, YDCPG_SL1, JFLD, KHVI, YDCPG_GFLF, KASLB1,&
...
IF (LHOOK) CALL DR_HOOK ('LARCINB_LAITVSPCQM', 0, ZHOOK_HANDLE)
CALL LAITRE_GFL (YDGEOMETRY, YDML_GCONF%YGFL, YDML_DYN, KASLB1, YDGEOMETRY%YRDIM%NPROMA&
&, YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA, YDGEOMETRY%YRDIMV%NFLEVG, YDGEOMETRY%YRDIMV%NFLSA&
&, YDGEOMETRY%YRDIMV%NFLEN, KHVI, KWKGFL, YDVARS%GFL_PTR (JFLD)%YCOMP%CSLINT, YDVARS&
&%GFL_PTR (JFLD)%YCOMP%WENO_ALPHA, LDMOM, KNOWENO, KL0, PLSCAW, PRSCAW, YDCPG_SL1%GFL&
& (JFLD)%P, YDCPG_GFLF%GFL (JFLD)%P, PXSPSL=YDCPG_SL1%GFL (JFLD)%P_SP)
IF (LHOOK) CALL DR_HOOK ('LARCINB_LAITVSPCQM', 1, ZHOOK_HANDLE)
ENDSUBROUTINE
