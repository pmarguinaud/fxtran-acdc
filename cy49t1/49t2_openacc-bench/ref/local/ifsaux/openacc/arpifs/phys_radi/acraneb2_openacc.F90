...
temp (REAL (KIND=JPRB), ZNEBC, (KLON, KLEV))
temp (REAL (KIND=JPRB), ZB1C, (KLON, KLEV))
...
temp (REAL (KIND=JPRB), ZFRSODIRT, (KLON))
REAL (KIND=JPRB)::ZTAUCUN 
REAL (KIND=JPRB)::ZTAUC 
temp (REAL (KIND=JPRB), ZFRSODIRS_UN, (KLON))
temp (REAL (KIND=JPRB), ZFRSODIRS_TRUE, (KLON))
temp (REAL (KIND=JPRB), ZFRSODIRS_CUN, (KLON))
temp (REAL (KIND=JPRB), ZFRSODIRS_C, (KLON))
temp (REAL (KIND=JPRB), ZCLCT, (KLON))
...
INTEGER (KIND=JPIM)::IIDIA (KJN)
INTEGER (KIND=JPIM)::IFDIA (KJN)
...
IF (KIND (ZFRSODIRS_TRUE) == 8) THEN
    alloc8 (ZFRSODIRS_TRUE)
ELSEIF (KIND (ZFRSODIRS_TRUE) == 4) THEN
    alloc4 (ZFRSODIRS_TRUE)
ELSE
    STOP 1
ENDIF
...
IF (KIND (ZFRSODIRS_CUN) == 8) THEN
    alloc8 (ZFRSODIRS_CUN)
ELSEIF (KIND (ZFRSODIRS_CUN) == 4) THEN
    alloc4 (ZFRSODIRS_CUN)
ELSE
    STOP 1
ENDIF
...
IF (ICALS==0) THEN
  ...
  &, KFDIA, KLON, KTDIA, KLEV, KJN, IIDIA, IFDIA, IAUCR, PAPRS, PAPRSF, PDELP&
...
&, KLON, KTDIA, KLEV, KJN, IIDIA, IFDIA, IAUCR, PDELP, PNEB, ZQICE, ZQLI, PR, PAPRSF, PT&
...
CALL ACRANEB_COEFS_OPENACC (YDML_PHY_MF%YRPHY3, KLON, KTDIA, KLEV, KJN, IIDIA, IFDIA, IAUCR, PAPRSF&
...
IF (KNFRRC/=0) THEN
  ...
  IF (MOD (KSTEP, KNFRRC)==0) THEN
    ...
    ZNEBC (JLON,:)=0._JPRB
    ZB1C (JLON,:)=1._JPRB
    CALL ACRANEB_SOLVS_OPENACC (YDML_PHY_MF%YRPHY, LLNUMX, KLON, KTDIA, KLEV, KJN, IIDIA, IFDIA&
...
& KJN, IIDIA, IFDIA, IAUCR, PALB, PALBDIR, PCLCT, ZFRSODIRT, ZFRSODIRS_C, ZFRSODIRS_CUN, PNEB&
...
ENDSUBROUTINE ACRANEB2_OPENACC
