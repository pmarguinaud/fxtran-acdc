...
REAL(KIND=JPRB) :: ZKTROV(YDCPG_OPTS%KLON,0:YDCPG_OPTS%KFLEVG),ZNBVNO(YDCPG_OPTS%KLON,0:YDCPG_OPTS%KFLEVG)
REAL(KIND=JPRB) :: ZKUROV(YDCPG_OPTS%KLON,0:YDCPG_OPTS%KFLEVG),ZKUROV2(YDCPG_OPTS%KLON,0:YDCPG_OPTS%KFLEVG)
...
REAL(KIND=JPRB) :: ZPRODTH(YDCPG_OPTS%KLON,YDCPG_OPTS%KFLEVG)         ! Flux de production de TKE 
...
REAL(KIND=JPRB)     :: ZFLU_CHN (YDCPG_OPTS%KLON)
...
& YDSPP_CONFIG=>YDMODEL%YRML_GCONF%YRSPP_CONFIG, YDEPHY=> YDMODEL%YRML_PHY_EC%YREPHY,                                        &
...
& LGCHECKMV=>YDPHY%LGCHECKMV, LECTPRODCVP=>YDPHY%LECTPRODCVP, LCVTDK=>YDPHY%LCVTDK, LCVPPKF=>YDPHY%LCVPPKF,       &
& TSTEP=>YDRIP%TSTEP, LEDKF=>YDPHY%LEDKF, LEDMFI=>YDPHY%LEDMFI)
...
IF (YDCPG_OPTS%LCONFX) THEN
  ZDTMSE=0.01_JPRB
  ZSTATI=RSTATI+(ZDTMSE-TSTEP)*0.5_JPRB
ELSE
  ZDTMSE=TSPHY
  ZSTATI=RSTATI
ENDIF
...
CALL CPPHINP(YDGEOMETRY, YDMODEL, KLON, KFLEVG, KIDIA, KFDIA, YDVARS%GEOMETRY%GEMU%T0,  YDVARS%GEOMETRY%GELAM%T0, &
& YDVARS%U%T0, YDVARS%V%T0, YDVARS%T%DL, YDVARS%T%DM, YDVARS%Q%T0, YDVARS%Q%DL, YDVARS%Q%DM, YDVARS%CVGQ%DL,              &
& YDVARS%CVGQ%DM, YDCPG_PHY0%XYB%RDELP, YDCPG_DYN0%CTY%EVEL, YDVARS%CVGQ%T0, ZRDG_MU0, ZRDG_SOLO, ZRDG_MU0LU, ZRDG_MU0M,  &
& ZRDG_MU0N, ZRDG_CVGQ, ZRDG_CVGT)
...
DO JLEV = YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG
  DO JLON = YDCPG_BNDS%KIDIA, YDCPG_BNDS%KFDIA
    ZQCO2 (JLON,JLEV) = YDMODEL%YRML_PHY_MF%YRPHY3%QCO2
  ENDDO
ENDDO
...
IF (YDCPG_OPTS%LCONFX) THEN
  ...
  & YDMF_PHYS_SURF%GSD_VH%PSPSH, YDMF_PHYS_SURF%GSD_VK%PUDGRO, YDMF_PHYS_SURF%GSD_VV%PHV,               &
  & YDMF_PHYS_SURF%GSD_VV%PZ0H, YDVARS%DAL%T0, YDVARS%DOM%T0, YDVARS%UAL%T0, YDVARS%UEN%T0,             &
  & YDVARS%UNEBH%T0, YDVARS%UOM%T0, YDMODEL)

ENDIF


!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=APLPAR_INIT {

CALL APLPAR_INIT (YDCPG_OPTS%LAROME, KIDIA, KFDIA, KLON, KFLEVG, &
& NTSSG, YDCPG_OPTS%YRSURF_DIMS%YSP_SBD%NLEVS, YDMF_PHYS_SURF%GSD_VF%PVEG, ZMSC_FRMQ, ZFCHQ, ZDSA_CPS,       &
& ZDSA_LHS, ZDSA_RS, ZMSC_LH, ZMSC_LSCPE, ZFLU_QSAT, ZMSC_QW, ZMSC_TW, ZFLU_CD, ZFLU_CDN, ZFLU_CH, ZFLU_CHN, &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=PLSM {
...
IF(LGCHECKMV) THEN
!$ACDC PARALLEL,NAME=CHECKMV {
  CALL CHECKMV(YDCPG_OPTS%NINDAT, YDMODEL%YRCST, YDRIP, YDPHY0, YDPHY2, KIDIA, KFDIA,          &
  & KLON, KFLEVG, YDCPG_OPTS%NSTEP, PAPHI, PAPHIF, PAPRS, PAPRSF, YDVARS%GEOMETRY%GELAM%T0,    &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACTQSAT {

CALL ACTQSAT (YDMODEL%YRCST, YDPHY, KIDIA, KFDIA, KLON, NTQSAT, KFLEVG, &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACSOL {

  IF ( LSOLV ) THEN
    LLHMT=.FALSE.
    CALL ACSOL (YDCPG_OPTS%YRCLI, YDMODEL%YRCST, YDPHY, YDPHY1, KIDIA, KFDIA, KLON,                                        &
    ...
    & YDMF_PHYS_BASE_STATE%YGSP_SG%A (:,1), YDMF_PHYS_BASE_STATE%YGSP_SG%R (:,1), YDMF_PHYS_SURF%GSD_VV%PSAB,              &
    & YDMF_PHYS_BASE_STATE%YGSP_SG%F (:,1), YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VF%PVEG,                    &
    & YDMF_PHYS_BASE_STATE%YGSP_SB%Q (:,1), YDMF_PHYS_BASE_STATE%YGSP_SB%TL (:,1), YDMF_PHYS_BASE_STATE%YGSP_RR%W,         &
    & YDMF_PHYS_BASE_STATE%YGSP_RR%IC, LLHMT, ZDSA_C1, ZDSA_C2, ZC3, ZCG, ZCN, YDMF_PHYS%OUT%CT, ZNEIJG, ZNEIJV, ZWFC,     &
    & ZWPMX, ZWSEQ, ZWSMX, ZWWILT)
  ELSE
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ZDPHIV {
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACHMTLS {

  CALL ACHMTLS (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, KIDIA, KFDIA, KLON,                            &
  & KFLEVG, PAPHI, PAPHIF, PAPRS, PAPRSF, PR, PT, PU, PV, YDMF_PHYS_BASE_STATE%YGSP_RR%T,          &
  ...
  DO JLON = KIDIA, KFDIA
    ZDPHI(JLON)=PAPHIF(JLON,KFLEVG)-PAPHI(JLON,KFLEVG)
    ZPRS(JLON)=YDMODEL%YRCST%RD+ZRVMD*YDCPG_MISC%QS(JLON)
    ZRTI(JLON)=2.0_JPRB/(PR(JLON,KFLEVG)*PT(JLON,KFLEVG)+YDMODEL%YRCST%RKAPPA*ZDPHI(JLON)&
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACHMT {

  CALL ACHMT (YDCPG_OPTS%YRCLI, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY1,     &
  & YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, KIDIA, KFDIA, KLON, KFLEVG,                                             &
  ...
  & ZNEIJV, YDMF_PHYS_BASE_STATE%YGSP_SG%F (:,1), YDMF_PHYS_BASE_STATE%YGSP_RR%T, YDMF_PHYS_SURF%GSD_VF%PVEG,          &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACCLPH {
...
DO JLON = KIDIA, KFDIA
  ...
  & *(YDMODEL%YRCST%RATM/PAPRS(JLON,KFLEVG))**YDMODEL%YRCST%RKAPPA  
ENDDO

CALL ACCLPH (YDMODEL%YRCST, YDPHY0, YDPHY2, KIDIA, KFDIA, KLON, YDCPG_OPTS%KTDIA, &
& KFLEVG, ZTHETAV, PAPHI, PAPHIF, PU, PV, ZTHETAVS, IMOC_CLPH, YDMF_PHYS%OUT%CLPH, YDMF_PHYS%OUT%VEIN,  &
& ZUGST, ZVGST)
...
& YDCPG_MISC, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL, YDDDH, INLAB_CVPP, ZCDROV, ZCHROV, ZCOEFN, ZCONDCVPPI,      &
& ZCONDCVPPL, ZDIFCVPPQ, ZDIFCVPPS, YDMODEL%YRML_PHY_MF%YRPHY0%REPS, ZFLU_CD, ZFLU_CH, ZKQLROV,               &
& ZKQROV, ZKTROV, ZKUROV, ZKUROV2, ZMSC_LSCPE, ZNBVNO, ZNEBS, ZNEBS0, ZNEB_CVPP, ZPFL_FPLCH, ZPFL_FTKE,       &
& ZPFL_FTKEI, ZPRODTH_CVPP, ZQI, ZQIC, ZQL, ZQLC, ZQLIS, ZQLIS0, ZQLI_CVPP, ZQV, ZQR, ZQS, ZTKE1, ZXTROV,     &
& ZXUROV, ZUSLE, ZKCLS, ZECTCLS, ZPRODTH, ZEDMFQ, ZEDMFS, ZEDMFU, ZEDMFV, ZMF_UP)
...
& ZNEB_CVPP, ZPFL_FPLCH, ZQI, ZQL, ZQLIS, ZQLIS0, ZQLI_CVP, ZQLI_CVPP, ZQV, ZUNEBH, ZCLCT_RAD, YDSTA)
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=DIFTQ {
...
IF (LCVPPKF.OR.(LEDKF .AND. .NOT. LEDMFI)) THEN
   IF (LCVPPKF) THEN
    ...
    ELSEIF(LEDKF) THEN  
      DO JLEV=YDCPG_OPTS%KTDIA,KFLEVG
         DO JLON = KIDIA, KFDIA
            YDMF_PHYS%OUT%DIFTQ   (JLON,JLEV) = YDMF_PHYS%OUT%DIFTQ(JLON,JLEV) + ZEDMFQ(JLON,JLEV)
            YDMF_PHYS%OUT%DIFTS   (JLON,JLEV) = YDMF_PHYS%OUT%DIFTS(JLON,JLEV) + ZEDMFS(JLON,JLEV)
            YDMF_PHYS%OUT%STRTU   (JLON,JLEV) = YDMF_PHYS%OUT%STRTU(JLON,JLEV) + ZEDMFU(JLON,JLEV)
            YDMF_PHYS%OUT%STRTV   (JLON,JLEV) = YDMF_PHYS%OUT%STRTV(JLON,JLEV) + ZEDMFV(JLON,JLEV)
         ENDDO
      ENDDO
    ENDIF
ENDIF  
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=FRSOPT {

!DEC$ IVDEP
DO JLON = KIDIA, KFDIA
  IF(.NOT.LMSE)YDMF_PHYS%OUT%FRTHDS(JLON)=YDMF_PHYS%OUT%FRTH(JLON,KFLEVG,1)/ZFLU_EMIS(JLON)+YDMODEL%YRCST%RSIGMA*YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON)**4
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACDRAG {

IF(LMCC03)THEN
  CALL ACDNSHF(YDMODEL%YRCST, YDPHY, YDPHY1, KIDIA, KFDIA, KLON, YDCPG_OPTS%KTDIA,                                &
  & KFLEVG, ZFLU_EMIS, YDMF_PHYS_SURF%GSD_VF%PLSM, ZFLU_NEIJ, ZQV, YDCPG_MISC%QS, YDMF_PHYS_BASE_STATE%YGSP_RR%T, &
...
IF ( LSTRAS ) THEN
  CALL ACPLUIS (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, KIDIA, KFDIA, KLON,                                          &
  & NTPLUI, KFLEVG, PAPRSF, PDELP, ZNEBS, ZQV, ZQLIS, ZMSC_QW, PR, PT, YDMF_PHYS%OUT%FCSQL,                      &
...
IF (LCVTDK) THEN  
  ...
  & YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDMODEL, ZFPCOR, ZQV, ZTENHA, ZTENQVA, ZTENT, ZRDG_CVGQ, ZRDG_CVGT)
ENDIF

IF (LECTPRODCVP) THEN

!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=TURBULENCE_PART2 {

  CALL APL_ARPEGE_TURBULENCE_PART2(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, KIDIA, KFDIA,                            &
  & KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET, KFLEVG,                         &
  & PAPHI, PAPHIF, PAPRS, PAPRSF, PDELP, PR, PCP, PT, YDMF_PHYS_BASE_STATE%Q, PU, PV, YDMF_PHYS_BASE_STATE%TKE, &
  & ZPRODTH, ZKUROV2, YDMF_PHYS%OUT%DIFCQ, YDMF_PHYS%OUT%DIFCS, YDMF_PHYS%OUT%STRCU, YDMF_PHYS%OUT%STRCV,       &
  & ZUSLE, ZKCLS, ZECTCLS, ZPFL_FTKE, ZTKE1, YDMF_PHYS%OUT%EDR)
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=QNGCOR {
...
IF(LNEBN.OR.LRRGUST) THEN
  DO JLEV=YDCPG_OPTS%KTDIA-1,KFLEVG
    DO JLON = KIDIA, KFDIA
...
CALL QNGCOR (YDMODEL%YRCST, YDPHY2, KIDIA, KFDIA, KLON, NTPLUI, KFLEVG, &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACDRME {

CALL ACDRME ( YDMODEL%YRCST, YDSTA, YDPHY2, YDTOPH, KIDIA, KFDIA, KLON,                            &
& NTDRME, KFLEVG, PCP, PDELP, PT, ZQV, PU, PV, YDMF_PHYS%OUT%FRMH, ZMSC_FRMQ, YDMF_PHYS%OUT%STRMU, &
& YDMF_PHYS%OUT%STRMV)
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACEVADCAPE {

! DCAPE due to precipitation evaporation.

  CALL ACEVADCAPE(YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, KIDIA, KFDIA, KLON,                &
  & KFLEVG, YDMF_PHYS%OUT%FPLSL, YDMF_PHYS%OUT%FPLSN, YDMF_PHYS%OUT%FPLCL, YDMF_PHYS%OUT%FPLCN, &
...
! Gusts.
  ...
  & YDTOPH, KIDIA, KFDIA, KLON, KFLEVG, YDMF_PHYS%OUT%UCLS,                                                             &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACVISIH {

ZQGM(1:KLON,1:KFLEVG)=ZEPSNEB

CALL ACVISIH(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI, KIDIA, KFDIA, KLON,           &
& YDCPG_OPTS%KTDIA, KFLEVG, PAPHI, PAPHIF, PAPRSF, PT, PR, YDCPG_MISC%QLI, YDCPG_MISC%QICE,  &
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ZBAY_QRCONV {
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ZDE2MR {
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=PPWETPOINT {

CALL PPWETPOINT(YDMODEL%YRCST, YDPHY, KIDIA, KFDIA, KLON, 1, ZPCLS,                          &
& YDMF_PHYS%OUT%TCLS,  YDMF_PHYS_BASE_STATE%Q(:, KFLEVG), YDMF_PHYS_BASE_STATE%L(:, KFLEVG), &
& YDMF_PHYS_BASE_STATE%I(:, KFLEVG),  YDMF_PHYS%OUT%TPWCLS)
...
!$ACDC PARALLEL {
  ...
  CALL CPQSOL(YDMODEL%YRCST, YDGEOMETRY%YRDIMV, YDPHY, KLON, KIDIA, KFDIA, &
...
& ZDIFEXT, ZDSA_CPS, ZMSC_FRMQ, ZPFL_FEFB1, ZPFL_FEFB2, ZPFL_FEFB3, ZPFL_FTKE, ZPFL_FTKEI, ZFCHQ)
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=BAYRAD {
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=EDR {
...
!$ACDC PARALLEL,TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=PRECIPS {
