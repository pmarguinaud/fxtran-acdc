...
DO JK=1,IKT 
  ...
  ENDDO
ENDDO
CALL COMPUTE_FRAC_ICE(NEBN%CFRAC_ICE_SHALLOW_MF,CST,NEBN,ZFRAC_ICE(:,:),ZWK(:,:), IERR(:,:))
...
DO JK=1,IKT 
  DO JIJ=IIJB,IIJE 
    ...
    & ((1.+CST%XRV / CST%XRD *PRM(JIJ,JK,1))/(1.+ZRTM(JIJ,JK))) 
  ENDDO
ENDDO
! 
!!! 2. Compute updraft
...
IF(PRESENT(BUCONF)) THEN
 IF( BUCONF%LBUDGET_U ) THEN
    DO JK=1,IKT 
      DO JIJ=IIJB,IIJE 
        ZWORK(JIJ,JK)=PRHODJ(JIJ,JK)*PDUDT_MF(JIJ,JK)
      ENDDO
    ENDDO
...
!
 IF( BUCONF%LBUDGET_V ) THEN
    DO JK=1,IKT 
      DO JIJ=IIJB,IIJE 
        ZWORK(JIJ,JK)=PRHODJ(JIJ,JK)*PDVDT_MF(JIJ,JK)
      ENDDO
    ENDDO 
...
! 
 IF( BUCONF%LBUDGET_TH ) THEN 
    DO JK=1,IKT 
      DO JIJ=IIJB,IIJE 
        ZWORK(JIJ,JK)=PRHODJ(JIJ,JK)*PDTHLDT_MF(JIJ,JK)
      ENDDO
    ENDDO
...
!
 IF( BUCONF%LBUDGET_RV ) THEN
    DO JK=1,IKT 
      DO JIJ=IIJB,IIJE 
        ZWORK(JIJ,JK)=PRHODJ(JIJ,JK)*PDRTDT_MF(JIJ,JK)
      ENDDO
    ENDDO
 ...
 IF( BUCONF%LBUDGET_SV ) THEN
   DO JSV=1,KSV
    IF (ONOMIXLG .AND. JSV >= KSV_LGBEG .AND. JSV<= KSV_LGEND) THEN
        DO JK=1,IKT 
          DO JIJ=IIJB,IIJE 
            ZWORK(JIJ,JK)=MAX(PRSVS(JIJ,JK,JSV) + PRHODJ(JIJ,JK)* &
            PDSVDT_MF(JIJ,JK,JSV),PSVMIN(JSV))
            ZWORK(JIJ,JK)=PRSVS(JIJ,JK,JSV) - ZWORK(JIJ,JK)
          ENDDO
        ENDDO
    ELSE
        DO JK=1,IKT 
          DO JIJ=IIJB,IIJE 
            ZWORK(JIJ,JK)=PRHODJ(JIJ,JK)*PDSVDT_MF(JIJ,JK,JSV)
          ENDDO
        ENDDO
    END IF
