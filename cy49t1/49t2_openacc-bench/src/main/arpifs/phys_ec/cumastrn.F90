...
REAL(KIND=JPRB) ::     ZCAPE(KLON), ZCAPE2(KLON), ZHEAT(KLON), ZCAPPBL(KLON), ZCAPDCYCL(KLON), ZCAPDCYCLLD(KLON), ZCAPDCYCLSE(KLON)
...
REAL(KIND=JPRB) ::   ZCONS2, ZCONS, ZDH, ZDH2,&
 ...
 & ZDUTEN, ZDVTEN, ZTDIS, ZTAURES, ZAR, ZAS, ZBR, ZBS, ZAK, ZRG, ZIND !, ZALPHA
...
ASSOCIATE(LMFCUCA=>YDML_PHY_EC%YRECUMF%LMFCUCA, LMFDD=>YDML_PHY_EC%YRECUMF%LMFDD, &
 ...
 & RTAUFAC=>YDML_PHY_EC%YRECUMF%RTAUFAC, RDXMIN=>YDML_PHY_EC%YRECUMF%RDXMIN, RDXMAX=>YDML_PHY_EC%YRECUMF%RDXMAX, &
...
! ZMFCFL(JL) = 2._JPRB*MIN(2._JPRB,1._JPRB + 2.5E-5_JPRB*ZDX(JL))
  IF(LDCUM(JL).AND.KTYPE(JL) == 1) THEN
    ...
    IF(LLO1.AND.RCAPDCYCL==2.0_JPRB) THEN
      ZIND=MAX(0._JPRB,MIN(1._JPRB,(RDXMAX-ZDX(JL))/(RDXMAX-RDXMIN)))
