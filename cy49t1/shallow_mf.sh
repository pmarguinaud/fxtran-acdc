...
do
  echo "==> $f <=="
  openacc.pl --interface --stack84  --interfaces --inline-contained \
     --cycle 49 --dir src/local/mpa/openacc $(resolve --user $f)
done
...
do
  echo "==> $f <=="
  openacc.pl --interface --stack84  --interfaces --inline-contained \
     --cycle 49 --dir src/local/mpa/openacc --modi $(resolve --user $f)
done
