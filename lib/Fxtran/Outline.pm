...
sub outlineSection
{
  ...
  unless ($inc_target)
    {
      $inc_target = &Fxtran::Scope::getNoExec ($d);
    }
...
EOF
  ...
  &Fxtran::Associate::resolveAssociates ($s, outer => 1);
  ...
  for my $expr (@expr)
    {
      my ($n) = &F ('./N/n', $expr, 1);
      next if (&Fxtran::Intrinsic::isIntrinsic ($n) || $PARKIND{$n});
