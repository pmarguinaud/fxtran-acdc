...
DO JGFL=1, SIZE (YDCPG_SL1%GFL)
  ...
  IF (YDCPG_SL1%GFL (JGFL)%YCOMP%LADV.AND.YDCPG_SL1%GFL (JGFL)%YCOMP%CSLINT=='LAITVSPCQM  ') THEN
    ...
    IF ((YDML_DYN%YRDYN%NSPLTHOI/=0).OR.LLTDIABLIN) THEN
      CALL LATTEX_EXPL_VSPLTRANS_OUTLINE_000 (YDCPG_SL1, JGFL, YDML_DYN, YDGEOMETRY, YDCPG_BNDS)
    ENDIF

    CALL LATTEX_EXPL_VSPLTRANS_OUTLINE_001 (YDCPG_SL1, JGFL, YDML_DYN, YDGEOMETRY, YDCPG_BNDS)
  ENDIF
