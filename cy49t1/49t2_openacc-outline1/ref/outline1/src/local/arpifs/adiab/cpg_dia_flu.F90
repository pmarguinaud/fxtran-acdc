...
IF (LLCALL_CPCFU.OR.LLCALL_CPXFU.OR.LLCALL_CUMCPLDM) THEN
  CALL CPG_DIA_FLU_OUTLINE_000 (YDCPG_OPTS, YDMF_PHYS_OUT, YDCPG_BNDS&
  &, ZSTRCV, ZSTRCU, ZSTRDV, ZSTRDU, ZSTRTV, ZSTRTU)
ENDIF


IF (LLCALL_CPXFU.OR.YDMODEL%YRML_PHY_G%YRDPHY%LDIRCLSMOD) THEN
  CALL CPG_DIA_FLU_OUTLINE_001 (YDCPG_OPTS, YDMF_PHYS_OUT, YDCPG_BNDS&
  &, ZNUCLS, ZVGST, ZVCLS, ZUCLS, ZNVCLS, ZUGST)
ENDIF
...
IF (YDCPG_OPTS%LDIAB.OR.(YDMODEL%YRML_PHY_MF%YRSIMPHL%LSIMPH&
  ...
  IF (LLCALL_CPCFU.OR.LLCALL_CPXFU.OR.LLCALL_CUMCPLDM) THEN
    CALL CPG_DIA_FLU_OUTLINE_002 (YDVARS, YDCPG_OPTS, YDCPG_BNDS&
    &, ZSTRCV, ZSTRCU, ZSTRDV, ZSTRDU, ZSTRTV, ZSTRTU)
  ENDIF


  IF (LLCALL_CPXFU.OR.YDMODEL%YRML_PHY_G%YRDPHY%LDIRCLSMOD) THEN
    CALL CPG_DIA_FLU_OUTLINE_003 (YDVARS, YDCPG_OPTS, YDCPG_BNDS&
    &, ZNUCLS, ZVGST, ZVCLS, ZUCLS, ZNVCLS, ZUGST)
  ENDIF
...
IF (LLCALL_CPXFU) THEN
  CALL CPG_DIA_FLU_OUTLINE_004 (YDMF_PHYS_SURF, YDCPG_OPTS&
  &, YDMODEL, YDCPG_BNDS, ZDPRECIPS, ZDPRECIPS2)
