...
IF (YDMODEL%YRML_DYN%YRDYN%NCURRENT_ITER==0) THEN
  ...
  IF (YDMODEL%YRML_DYN%YRDYNA%LTWOTL) THEN
    CALL CPG_GP_GPTF2_EXPL_2TL (YDVARS, YDCPG_OPTS, YDGEOMETRY, YDCPG_BNDS)
  ELSE
...
CALL CPG_GP_MAP_FACTOR (YDVARS, YDCPG_DYN0, YDMODEL, YDGEOMETRY&
&, YDCPG_BNDS, Z_OROGMM_T0, Z_OROGLM_T0, Z_OROGLL_T0)
...
CALL CPG_GP_GP_SPV (YDVARS, YDCPG_DYN0, YDCPG_DYN9, YDMODEL, YDGEOMETRY, YDCPG_BNDS)
...
IF (YDMODEL%YRML_DYN%YRDYN%NCURRENT_ITER==0) THEN
  ...
  IF (YDCPG_OPTS%LDIAB.OR.YDMODEL%YRML_PHY_SLIN%YRPHLC%LSPHLC&
    ...
    CALL CPG_GP_SETZERO (YDCPG_MISC, YDCPG_BNDS)

    IF (YDCPG_OPTS%LFSTEP.AND..NOT.YDMODEL%YRML_DYN%YRDYNA%LTWOTL) THEN
      CALL CPG_GP_SET9TO0 (YDCPG_OPTS, YDMF_PHYS_SURF, YDCPG_BNDS)
    ENDIF
...
CALL CPG_GP_GPINISLB_PART1_EXPL (YDVARS, YDCPG_MISC, YDCPG_DYN9&
...
CALL CPG_GP_GPINISLB_PART3_EXPL (YDCPG_DYN0, YDMODEL, YDCPG_SL2&
&, YDGEOMETRY, YDCPG_BNDS, Z_GWHT_T0, Z_GDW_T0)
...
IF (YDMODEL%YRML_DYN%YRDYN%NCURRENT_ITER==0) THEN
  ...
  IF (YDCPG_OPTS%LDIAB.OR.YDMODEL%YRML_PHY_MF%YRSIMPHL%LSIMPH) THEN
    ...
    IF (YDMODEL%YRML_DYN%YRDYNA%LTWOTL) THEN
      CALL CPG_GP_SET1TO0 (YDCPG_OPTS, YDMF_PHYS_SURF, YDCPG_BNDS)
    ELSE
      CALL CPG_GP_SET1TO9 (YDCPG_OPTS, YDMF_PHYS_SURF, YDCPG_BNDS)
    ENDIF
