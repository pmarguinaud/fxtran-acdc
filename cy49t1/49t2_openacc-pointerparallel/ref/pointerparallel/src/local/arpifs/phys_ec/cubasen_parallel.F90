...
IF (LPARALLELMETHOD ('OPENMP','CUBASEN_PARALLEL:0')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CUBASEN_PARALLEL:0')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CUBASEN_PARALLEL:0:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&PRESENT (KDPL, LLFIRST, LLGO_ON, PWUBASE, YSTACK) &
    !$ACC&PRIVATE (JBLK) &
...
IF (LPARALLELMETHOD ('OPENMP','CUBASEN_PARALLEL:1')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CUBASEN_PARALLEL:1')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CUBASEN_PARALLEL:1:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC PARALLEL LOOP GANG &
    !$ACC&PRESENT (ILAB, KLAB, PGEO, PGEOH, PLU, PQENH, PQU, PTEN, PTENH, PTU, PWU2H, YSTACK, &
...
IF (LPARALLELMETHOD ('OPENMP','CUBASEN_PARALLEL:2')) THEN
    ...
    DO JKK=KLEV, JKT1,-1
      ...
      DO JK=JKK-1, JKT2,-1
        ...
        IF (.NOT.YDEPHLI%LPHYLIN) THEN
          ...
          LLFLAG (YLCPG_BNDS%KIDIA:YLCPG_BNDS%KFDIA, JBLK)=LLGO_ON (YLCPG_BNDS%KIDIA:YLCPG_BNDS%KFDIA, JBLK)
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CUBASEN_PARALLEL:2')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CUBASEN_PARALLEL:2:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         PTENH, PTU, PWU2H, PWUBASE, YSTACK, ZBUOH, ZCAPE, ZCBASE, ZDZ, ZLU, ZMIX, &
    !$ACC&         ZPH, ZQENH, ZQEX, ZQOLD, ZQU, ZSENH, ZSUH, ZTEX, ZTU, ZWU2H) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
    ...
    !$ACC LOOP VECTOR &
    !$ACC&PRIVATE (IK, IS, JK, JKB, JKK, JKT, JL, JLON, LLRESET, YLCPG_BNDS, Z1S, Z2S, ZALFAW, ZBUOF, &
...
IF (LPARALLELMETHOD ('OPENMP','CUBASEN_PARALLEL:3')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','CUBASEN_PARALLEL:3')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('CUBASEN_PARALLEL:3:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC PARALLEL LOOP GANG &
    !$ACC&PRESENT (PCAPE, YSTACK, ZCAPE) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON) 
...
ENDSUBROUTINE CUBASEN_PARALLEL
