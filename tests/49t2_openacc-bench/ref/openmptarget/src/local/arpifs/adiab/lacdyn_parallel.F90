...
IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LACDYN_PARALLEL:LASURE')) THEN
  ...
  CALL SGET_HOST_DATA_RDWR (ZBDT, YL_ZBDT)
  CALL SGET_HOST_DATA_RDWR (ZREDIV, YL_ZREDIV)
  CALL SGET_HOST_DATA_RDONLY (Z_YDVARS_GEOMETRY_GMAPPA_T0, YDVARS%GEOMETRY%GMAPPA%FT0)
  ...
  IF (FXTRAN_ACDC_LSYNCHOST ('LACDYN_PARALLEL:LASURE')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (ZBDT, YL_ZBDT)
    CALL SGET_HOST_DATA_RDWR (ZREDIV, YL_ZREDIV)
    CALL SGET_HOST_DATA_RDWR (Z_YDVARS_GEOMETRY_GMAPPA_T0, YDVARS%GEOMETRY%GMAPPA%FT0)
...
ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LACDYN_PARALLEL:LASURE')) THEN
  ...
  IF (LHOOK) CALL DR_HOOK ('LACDYN_PARALLEL:LASURE:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
  CALL SGET_DEVICE_DATA_RDWR (ZBDT, YL_ZBDT)
  CALL SGET_DEVICE_DATA_RDWR (ZREDIV, YL_ZREDIV)
  CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_GMAPPA_T0, YDVARS%GEOMETRY%GMAPPA%FT0)
  CALL SGET_DEVICE_DATA_RDONLY (Z_YDVARS_GEOMETRY_GM_T0, YDVARS%GEOMETRY%GM%FT0)
  IF (LHOOK) CALL DR_HOOK ('LACDYN_PARALLEL:LASURE:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
  IF (LHOOK) CALL DR_HOOK ('LACDYN_PARALLEL:LASURE:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
  
  !$OMP TARGET TEAMS DISTRIBUTE  & 
  !$OMP & MAP(PRESENT:YDCPG_OPTS, YDGEOMETRY, YDMODEL, YFXTRAN_ACDC_STACK, ZBDT, ZREDIV,  & 
  !$OMP &          Z_YDVARS_GEOMETRY_GMAPPA_T0, Z_YDVARS_GEOMETRY_GM_T0)  & 
  !$OMP & PRIVATE (JBLK)  & 
...
ELSE
  ...
  IF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENMP','LACDYN_PARALLEL:ZMIXNL')) THEN
    ...
    CALL SGET_HOST_DATA_RDWR (ZMIXNL, YL_ZMIXNL)
  ...
  ELSEIF (FXTRAN_ACDC_LPARALLELMETHOD ('OPENACCSINGLECOLUMN','LACDYN_PARALLEL:ZMIXNL')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK ('LACDYN_PARALLEL:ZMIXNL:GET_DATA',0,ZHOOK_HANDLE_FIELD_API)
    CALL SGET_DEVICE_DATA_RDWR (ZMIXNL, YL_ZMIXNL)
    IF (LHOOK) CALL DR_HOOK ('LACDYN_PARALLEL:ZMIXNL:GET_DATA',1,ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK ('LACDYN_PARALLEL:ZMIXNL:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    
    !$OMP TARGET TEAMS DISTRIBUTE  & 
    !$OMP & MAP(PRESENT:YDCPG_OPTS, YFXTRAN_ACDC_STACK, ZMIXNL)  & 
    !$OMP & PRIVATE (JBLK)  & 
    !$OMP & THREAD_LIMIT (YDCPG_OPTS%KLON)       
