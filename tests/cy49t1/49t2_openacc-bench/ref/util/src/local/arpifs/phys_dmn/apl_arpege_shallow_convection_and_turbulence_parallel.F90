...
IF (LPARALLELMETHOD ('OPENMP','APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE')) THEN
  ...
  IF (LSYNCHOST ('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE')) THEN
    IF (LHOOK) CALL DR_HOOK ('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL&
...
ELSEIF (LPARALLELMETHOD ('OPENMPSINGLECOLUMN','APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL&
  &:ACTKE')) THEN
...
ELSEIF (LPARALLELMETHOD ('OPENACCSINGLECOLUMN','APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL&
  &:ACTKE')) THEN
  ...
  &:ACTKE:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
    ...
    !$ACC&         YDMODEL, YSTACK, ZCDROV_SAVE, ZCHROV_SAVE, ZKTROV_SAVE, ZKUROV_SAVE, &
    !$ACC&         ZNLAB, ZNLABCVP, ZTPRDY, Z_YDCPG_MISC_QICE, Z_YDCPG_MISC_QLI, &
    ...
    DO JLON = 1, MIN (YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1) * YDCPG_OPTS%KLON)
      ...
      CALL ACPY (JLON, Z_YDCPG_MISC_QICE (:,:, JBLK), PQI (:,:, JBLK))
      CALL ACPY (JLON, Z_YDCPG_MISC_QLI (:,:, JBLK), PQL (:,:, JBLK))
      ZNLABCVP (JLON,:, JBLK)=1.0_JPRB
      ...
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LECTPRODCVP) THEN
        ...
        &:,:, JBLK), PQLIS0(:,:, JBLK), PCOEFN(:,:, JBLK), PUSLE(:,:, JBLK), PKCLS(:, JBLK), PECTCLS(:,&
        & JBLK), PPRODTH(:,:, JBLK),YDSTACK=YLSTACK)
        CALL ACPY (JLON, PKUROV2 (:,:, JBLK), PKUROV (:,:, JBLK))
      ELSE
        CALL ACTKE_OPENACC (YDMODEL%YRCST, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH, YDMODEL%YRML_PHY_MF&
        &, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF, YDMODEL%YRML_PHY_MF&
        &%YRTOPH%NTCOET, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:,:, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF&
      ...
      CALL ACPY (JLON, ZKTROV_SAVE (:,:, JBLK), PKTROV (:,:, JBLK))
      CALL ACPY (JLON, ZKUROV_SAVE (:,:, JBLK), PKUROV (:,:, JBLK))
      ZCDROV_SAVE (JLON, JBLK)=PCDROV (JLON, JBLK)
      ZCHROV_SAVE (JLON, JBLK)=PCHROV (JLON, JBLK)
    ENDDO
...
ENDSUBROUTINE APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL
