...
!$ACDC PARALLEL, TARGET=OpenMP/OpenMPSingleColumn/OpenACCSingleColumn,NAME=ACFLUSO {

  DO JLON = KIDIA, KFDIA
    ZCRTI(JLON) = 1.0_JPRB/(YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON)*PDSA_RS(JLON))
  ENDDO
  CALL ACFLUSO (YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, KIDIA, KFDIA, KLON, KFLEVG, &
  ...
  & YDMF_PHYS_BASE_STATE%V, PDPHIT, PDPHIV, YDMF_PHYS%OUT%GZ0, YDMF_PHYS_SURF%GSD_VF%PLSM, PFLU_QSATS, ZCRTI,               &
  & YDMF_PHYS_BASE_STATE%YGSP_RR%T, LDHMT, PFLU_CD, PFLU_CDN, PCDROV, ZCE, PCEROV, PFLU_CH, PCHROV, YDMF_PHYS%OUT%QCLS,     &
  & YDMF_PHYS%OUT%RHCLS, YDMF_PHYS%OUT%TCLS, YDMF_PHYS%OUT%UCLS, YDMF_PHYS%OUT%VCLS, YDMF_PHYS%OUT%UGST, YDMF_PHYS%OUT%VGST)
