...
IF (OENTR_DETR) THEN
  ...
  ! thetav at mass and flux levels
      ZTHVM_F(JIJ,JK)=ZTHM_F(JIJ,JK)* &
      &((1.+ZRVORD*ZRVM_F(JIJ,JK))/(1.+ZRTM_F(JIJ,JK)))
      ZTHVM(JIJ,JK)=PTHM(JIJ,JK)* &
      &((1.+ZRVORD*PRVM(JIJ,JK))/(1.+PRTM(JIJ,JK)))
  ...
  ! compute updraft thevav and buoyancy term at KKB level
    PTHV_UP(JIJ,IKB) = ZTH_UP(JIJ,IKB)*&
    & ((1+ZRVORD*PRV_UP(JIJ,IKB))/(1+PRT_UP(JIJ,IKB)))
  ...
  ! Compute Buoyancy flux at the ground
    ZWTHVSURF(JIJ) = (ZTHVM_F(JIJ,IKB)/ZTHM_F(JIJ,IKB))*PSFTH(JIJ)+     &
  ...
  DO JIJ=IIJB,IIJE 
    IF (ZWTHVSURF(JIJ)>0.)THEN
      ...
      ((ZG_O_THVREF(JIJ,IKB))*ZWTHVSURF(JIJ)*ZLUP(JIJ))**(1./3.)
...
DO JK=IKB,IKE-IKL,IKL
  ...
  IF (OENTR_DETR) THEN
    IF(JK/=IKB) THEN
      DO JIJ=IIJB,IIJE 
        ZRC_MIX(JIJ,JK) = ZRC_MIX(JIJ,JK-IKL) ! guess of Rc of mixture
        ZRI_MIX(JIJ,JK) = ZRI_MIX(JIJ,JK-IKL) ! guess of Ri of mixture
      ENDDO
    ENDIF
    ...
    !       Computation of updraft characteristics at level JK+KKL
      IF(GTEST(JIJ))THEN
        ZMIX1(JIJ)=0.5*(PZZ(JIJ,JK+IKL)-PZZ(JIJ,JK))*&
  ...
  DO JSV=1,KSV 
    ...
    DO JIJ=IIJB,IIJE 
      IF(GTEST(JIJ)) THEN
        PSV_UP(JIJ,JK+IKL,JSV) = (PSV_UP(JIJ,JK,JSV)*(1-0.5*ZMIX2(JIJ)) + &
  ...
  IF (OENTR_DETR) THEN
    ...
    ! Mass flux is updated with the new updraft fraction
      IF (OENTR_DETR) PEMF(JIJ,JK+IKL)=PFRAC_UP(JIJ,JK+IKL)*SQRT(ZW_UP2(JIJ,JK+IKL))* &
      &ZRHO_F(JIJ,JK+IKL)
    ENDDO
...
DO JIJ=IIJB,IIJE 
  ZG_O_THVREF_ED(JIJ)=CST%XG/PTHVM(JIJ,KK)
  
  ZFRAC_ICE(JIJ)=PFRAC_ICE(JIJ) ! to not modify fraction of ice
...
DO JIJ=IIJB,IIJE 
  IF(KK/=KKB)THEN
    ZCOEFF_MINUS_HALF(JIJ)=((PTHVM(JIJ,KK)-PTHVM(JIJ,KK-KKL))/PDZZ(JIJ,KK))
    ZTHV_MINUS_HALF(JIJ) = PTHVM(JIJ,KK) - &
    & ZCOEFF_MINUS_HALF(JIJ)*0.5*(PZZ(JIJ,KK+KKL)-PZZ(JIJ,KK))
  ELSE
    ZCOEFF_MINUS_HALF(JIJ)=0.
    ZTHV_MINUS_HALF(JIJ) = PTHVM(JIJ,KK)
  ENDIF
  ZCOEFF_PLUS_HALF(JIJ)  = ((PTHVM(JIJ,KK+KKL)-PTHVM(JIJ,KK))/PDZZ(JIJ,KK+KKL))
  ZTHV_PLUS_HALF(JIJ)  = PTHVM(JIJ,KK) + &
  & ZCOEFF_PLUS_HALF(JIJ)*0.5*(PZZ(JIJ,KK+KKL)-PZZ(JIJ,KK))
ENDDO
...
DO JIJ=IIJB,IIJE 
  ZRCMIX(JIJ)=PRC_UP(JIJ)
  ZRIMIX(JIJ)=PRI_UP(JIJ)
ENDDO
CALL TH_R_FROM_THL_RT(CST,NEBN,D%NIJT,NEBN%CFRAC_ICE_SHALLOW_MF,ZFRAC_ICE,&
             ...
             PBUF=ZBUF, KB=D%NIJB, KE=D%NIJE)
DO JIJ=IIJB,IIJE 
  ZTHV_UP_F2(JIJ) = ZTHMIX(JIJ)*(1.+ZRVORD*ZRVMIX(JIJ))/(1.+PRT_UP(JIJ))
ENDDO
...
!  Compute cons then non cons. var. of mixture at the flux level KK+KKL  with initial ZKIC
  ZMIXTHL(JIJ) = ZKIC_INIT * 0.5*(PTHLM(JIJ,KK)+PTHLM(JIJ,KK+KKL))+&
  & (1. - ZKIC_INIT)*PTHL_UP(JIJ)
  ZMIXRT(JIJ)  = ZKIC_INIT * 0.5*(PRTM(JIJ,KK)+PRTM(JIJ,KK+KKL))+&
  & (1. - ZKIC_INIT)*PRT_UP(JIJ)
ENDDO
CALL TH_R_FROM_THL_RT(CST,NEBN,D%NIJT,NEBN%CFRAC_ICE_SHALLOW_MF,ZFRAC_ICE,&
